...
class t3lib_pageSelect {
	...
	function getPage($uid, $disableGroupAccessCheck = FALSE) {
		...
		if (is_array($this->cache_getPage[$uid][$cacheKey])) {
			...
			// Explicitly add pages that are accessible in a draft workspace but not in the live workspace. Otherwise they can't be previewed.
		$wsVersion = $this->getWorkspaceVersionOfRecord($this->versioningWorkspaceId, 'pages', $uid);
		if (is_array($wsVersion)) {
			$wsVersionAccess = $GLOBALS['TYPO3_DB']->exec_SELECTgetSingleRow('uid', 'pages', 'uid=' . intval($wsVersion['uid']) . $this->where_hid_del . $accessCheck);
			if (is_array($wsVersionAccess)) {
				$accessCheck = '';
			}
		}
	...
	function getMenu($uid, $fields = '*', $sortField = 'sorting', $addWhere = '', $checkShortcuts = 1) {
		...
		$output = Array();
		$query = 'pid=' . intval($uid) . $this->where_hid_del . $this->where_groupAccess . ' ' . $addWhere;
			// Explicitly add pages that are accessible in a draft workspace but not in the live workspace
		if ($this->versioningWorkspaceId != 0) {
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid', 'pages', 'pid=' . intval($uid) . $this->where_hid_del . ' ' . $addWhere, '', $sortField);
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				 $wsRow = $this->getWorkspaceVersionOfRecord($this->versioningWorkspaceId,'pages',$row['uid']);
				 $realUid = is_array($wsRow) ? $wsRow['uid'] : $row['uid'];
				 $result = $GLOBALS['TYPO3_DB']->exec_SELECTgetSingleRow('uid', 'pages', 'uid=' . intval($realUid) . $this->where_hid_del . $this->where_groupAccess . ' ' . $addWhere, '', $sortField);
				 if (is_array($result)) {
				 		$recordArray[] = $row['uid'];
				 }
			}
