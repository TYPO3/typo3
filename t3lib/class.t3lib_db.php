...
class t3lib_DB {
	 ...
	 * @var array
	 */
	protected $cache_autoIncFields = array();
	 ...
	 * @var array
	 */
	protected $cache_fieldType = array();
	 ...
	 * @var array
	 */
	protected $cache_primaryKeys = array();
		...
		foreach ($parsedExtSQL as $table => $tdef) {
			if (is_array($tdef['fields'])) {
				foreach ($tdef['fields'] as $field => $fdef) {
					...
					$this->cache_fieldType[$table][$field]['metaType'] = $this->mapMySqlTypeToMetaType($fdef['fieldType']);
	 ...
	 */
	public function mapMySqlTypeToMetaType($type) {
