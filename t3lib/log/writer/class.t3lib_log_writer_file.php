...
class t3lib_log_writer_File extends t3lib_log_writer_Abstract {
	...
	public function writeLog(t3lib_log_Record $record) {
		fwrite(self::$logFileHandle, $record . LF);
	 ...
	 *
	 * @return void
	 * @throws UnexpectedValueException if the log file can't be opened.
	 */
	protected function openLogFile() {
		...
		if (!is_resource(self::$logFileHandle)) {
			throw new UnexpectedValueException('Could not open log file "' . $this->logFile . '"', 1321804422);
		}
	}
	...
	protected function createLogFile() {
		if (!file_exists($this->logFile)) {
			...
			if (!is_dir($logFileDirectory)) {
				...
				t3lib_div::fixPermissions($logFileDirectory);
