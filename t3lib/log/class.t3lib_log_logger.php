...
class t3lib_log_Logger {
	...
	/**
	 * The writer to log in case we cannot log to the configured writers
	 *
	 * @var t3lib_log_writer_Abstract
	 */
	...
	public function addWriter($minimumLevel, t3lib_log_writer_Writer $writer) {
		...
		for ($logLevelWhichTriggersWriter = t3lib_log_Level::EMERGENCY; $logLevelWhichTriggersWriter <= $minimumLevel; $logLevelWhichTriggersWriter++) {
			if (!is_array($this->writers[$logLevelWhichTriggersWriter])) {
	...
	/**
	 * Returns all configured writers
	 *
	 * @return array
	 */
	...
	/**
	 * Returns the fallback writer
	 *
	 * @return t3lib_log_writer_Writer
	 */
	public function getFallbackWriter()
	{
		if (!is_object($this->fallbackWriter)) {
			$this->fallbackWriter = t3lib_div::makeInstance('t3lib_log_writer_PhpErrorLog');
		}
		return $this->fallbackWriter;
	}

	/**
	 * Sets the fallback writer, to which information about failed attempts to write
	 * log messages are written
	 *
	 * @param t3lib_log_writer_Writer $writer
	 * @return t3lib_log_Logger
	 */
	public function setFallbackWriter(t3lib_log_writer_Writer $writer) {
	...
	public function addProcessor($minimumLevel, t3lib_log_processor_Processor $processor) {
		...
		for ($logLevelWhichTriggersProcessor = t3lib_log_Level::EMERGENCY; $logLevelWhichTriggersProcessor <= $minimumLevel; $logLevelWhichTriggersProcessor++) {
			if (!is_array($this->processors[$logLevelWhichTriggersProcessor])) {
	 ...
	 * @return void
	 */
	protected function notifyWriters(t3lib_log_Record $record) {
		if (!empty($this->writers[$record->getLevel()])) {

				// Track the success of writing the log entries
			$allWritersFailed = TRUE;
			$exceptions = array();

			foreach ($this->writers[$record->getLevel()] as $writer) {
				try {
					$writer->writeLog($record);
					$allWritersFailed = FALSE;
				} catch (Exception $e) {
					$exceptions[get_class($writer)] = $e;
				}
			}

				// In case of none of our writers was able to write the entry,
				// we go into fallback procedure
			if ($allWritersFailed) {
				$this->handleAllWritersFailed($record, $exceptions);
			}
		}
	}

	/**
	 * Handles the situation, when writing a log record failed completely.
	 *
	 * @param t3lib_log_Record $originalLogRecord Log record
	 * @param array $exceptions Array of exceptions collected while writing
	 * @return void
	 */
	protected function handleAllWritersFailed(t3lib_log_Record $originalLogRecord, array $exceptions) {
		foreach ($exceptions as $writer => $exception) {
			/** @var $additionalLogRecord t3lib_log_Record */
			$additionalLogRecord = t3lib_div::makeInstance(
				't3lib_log_Record',
				__CLASS__,
				t3lib_log_Level::WARNING,
				'LogWriter ' . $writer . ' failed to write log entry.'
			);
			$additionalLogRecord->setData(
				array(
					'logEntry' => $additionalLogRecord,
					'exception' => $exception,
				)
			);

			$this->writeToFallbackWriter($additionalLogRecord);
		}
			// Write the original log record
		$this->writeToFallbackWriter($originalLogRecord);
	}
	 ...
	 * @return void
	 */
	public function writeToFallbackWriter(t3lib_log_Record $record) {
		$this->getFallbackWriter()->writeLog($record);
	}
