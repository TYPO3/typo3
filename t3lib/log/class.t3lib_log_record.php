...
class t3lib_log_Record implements ArrayAccess {
	...
	/** @var string Unique ID of the request */
	protected $requestId;

	/** @var float Creation timestamp with microseconds */
	protected $created;

	/** @var string The component where the record was created */
	protected $component;

	/** @var int Severity level */
	protected $level = t3lib_log_Level::INFO;

	/** @var string Log message one-liner */
	protected $message = '';

	/** @var array Additional log data */
	protected $data = array();

	/** @var array gttable properties for ArrayAccess */
	private $getableProperties = array(
		'requestId',
		'created',
		'component',
		'level',
	...
	/** @var array setable properties for ArrayAccess */
	private $setableProperties = array(
		'level',
		'message',
		'data'
	);
	...
	public function __construct($component = '', $level, $message, array $data = array()) {
		...
		$this->requestId = Typo3_Bootstrap::getInstance()->getRequestId();
	...
	public function offsetExists($offset) {
		...
		if (in_array($offset, $this->getableProperties, TRUE) && isset($this->$offset)) {
			$offsetExists = TRUE;
		}
	...
	public function offsetGet($offset) {
		if (!in_array($offset, $this->getableProperties, TRUE)) {
			return NULL;
		}
	...
	public function offsetSet($offset, $value) {
		if (in_array($offset, $this->setableProperties, TRUE)) {
			$this->$offset = $offset;
		}
	}
