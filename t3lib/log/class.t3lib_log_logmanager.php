...
class t3lib_log_LogManager implements t3lib_Singleton {
	 ...
	 *
	 * t3lib_log_LogManager::getLogger('main.sub.subsub');
	 *
	...
	public static function getLogger($name = '') {
		...
		if (!is_null(self::$loggers[$name])) {
			...
			self::setWritersForLogger($logger);
			self::setProcessorsForLogger($logger);
		}
	 ...
	 * @static
	 */
	public static function registerLogger($name) {
	 ...
	 * @static
	 */
	protected static function setWritersForLogger(t3lib_log_Logger $logger) {
		$config = self::getConfigurationForLogger(self::CONFIGURATION_TYPE_WRITER, $logger->getName());

		foreach ($config as $severityLevel => $writer) {
			foreach ($writer as $logWriterClassName => $logWriterOptions) {
				...
				try {
					...
					$logger->writeToFallbackWriter(
						new t3lib_log_Record(
							__CLASS__,
							t3lib_log_Level::WARNING,
							'Instantiation of LogWriter "' . $logWriterClassName . '" failed for ' . $logger->getName() . '.',
							$logWriterOptions
						)
					);
				}
	 ...
	 * @static
	 */
	protected static function setProcessorsForLogger(t3lib_log_Logger $logger) {
		$config = self::getConfigurationForLogger(self::CONFIGURATION_TYPE_PROCESSOR, $logger->getName());

		foreach ($config as $severityLevel => $processor) {
			foreach ($processor as $logProcessorClassName => $logProcessorOptions) {
				...
				try {
					...
					$logger->warning('Instantiation of LogProcessor "' . $logProcessorClassName . '" failed for ' . $logger->getName() . ' (' . $e->getMessage() . ')');
				}
			}
		}
	}
	 ...
	 * @static
	 */
	protected static function getConfigurationForLogger($configurationType, $loggerName) {
		...
		$configKey = $configurationType . 'Configuration';

		$config = $GLOBALS['TYPO3_CONF_VARS']['LOG'];

		$result = (!empty($config[$configKey]) ? $config[$configKey] : array());
		...
		foreach ($explodedName as $partOfClassName) {
			if (!empty($config[$partOfClassName][$configKey])) {
				$result = $config[$partOfClassName][$configKey];
			}
			$config = $config[$partOfClassName];
		}
		...
		foreach ($result as $level => $unused) {
			try {
				...
				throw new RangeException(
					'The given severity level "' . htmlspecialchars($level) . '" for ' . $configKey . ' of logger "' .
