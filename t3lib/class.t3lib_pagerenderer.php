	...
	protected function renderMainJavaScriptLibraries() {
		...
		if ($this->addJQuery) {
			...
			switch ($this->jQuerySource) {
				...
				case 'local':
				default:
					$jQueryFileName = $this->backPath . $this->jQueryPath . 'jquery-' . rawurlencode($this->jQueryVersion) . '.js';
			}

				// Include the jQuery Core
			$out .= '<script src="' . htmlspecialchars($this->processJsFile($jQueryFileName)) . '" type="text/javascript"></script>';
