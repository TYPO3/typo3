 ...
 *
 *
 *  182: class t3lib_BEfunc
 *
 *              SECTION: SQL-related, selecting records, searching
 *  203:     function deleteClause($table,$tableAlias='')
 *  226:     function getRecord($table,$uid,$fields='*',$where='')
 *  244:     function getRecordWSOL($table,$uid,$fields='*',$where='')
 *  277:     function getRecordRaw($table,$where='',$fields='*')
 *  299:     function getRecordsByField($theTable,$theField,$theValue,$whereClause='',$groupBy='',$orderBy='',$limit='')
 *  332:     function searchQuery($searchWords,$fields,$table='')
 *  347:     function listQuery($field,$value)
 *  359:     function splitTable_Uid($str)
 *  374:     function getSQLselectableList($in_list,$tablename,$default_tablename)
 *  402:     function BEenableFields($table,$inv=0)
 *
 *              SECTION: SQL-related, DEPRECATED functions
 *  466:     function mm_query($select,$local_table,$mm_table,$foreign_table,$whereClause='',$groupBy='',$orderBy='',$limit='')
 *  488:     function DBcompileInsert($table,$fields_values)
 *  502:     function DBcompileUpdate($table,$where,$fields_values)
 *
 *              SECTION: Page tree, TCA related
 *  532:     function BEgetRootLine($uid,$clause='',$workspaceOL=FALSE)
 *  588:     function openPageTree($pid,$clearExpansion)
 *  633:     function getRecordPath($uid, $clause, $titleLimit, $fullTitleLimit=0)
 *  676:     function getExcludeFields()
 *  706:     function getExplicitAuthFieldValues()
 *  777:     function getSystemLanguages()
 *  802:     function readPageAccess($id,$perms_clause)
 *  833:     function getTCAtypes($table,$rec,$useFieldNameAsKey=0)
 *  886:     function getTCAtypeValue($table,$rec)
 *  909:     function getSpecConfParts($str, $defaultExtras)
 *  939:     function getSpecConfParametersFromArray($pArr)
 *  967:     function getFlexFormDS($conf,$row,$table,$fieldName='',$WSOL=TRUE)
 *
 *              SECTION: Caching related
 * 1094:     function storeHash($hash,$data,$ident)
 * 1114:     function getHash($hash,$expTime=0)
 *
 *              SECTION: TypoScript related
 * 1150:     function getPagesTSconfig($id,$rootLine='',$returnPartArray=0)
 * 1201:     function updatePagesTSconfig($id,$pageTS,$TSconfPrefix,$impParams='')
 * 1256:     function implodeTSParams($p,$k='')
 *
 *              SECTION: Users / Groups related
 * 1293:     function getUserNames($fields='username,usergroup,usergroup_cached_list,uid',$where='')
 * 1311:     function getGroupNames($fields='title,uid', $where='')
 * 1328:     function getListGroupNames($fields='title,uid')
 * 1347:     function blindUserNames($usernames,$groupArray,$excludeBlindedFlag=0)
 * 1380:     function blindGroupNames($groups,$groupArray,$excludeBlindedFlag=0)
 *
 *              SECTION: Output related
 * 1421:     function daysUntil($tstamp)
 * 1433:     function date($tstamp)
 * 1444:     function datetime($value)
 * 1456:     function time($value)
 * 1472:     function calcAge($seconds,$labels = 'min|hrs|days|yrs')
 * 1498:     function dateTimeAge($tstamp,$prefix=1,$date='')
 * 1516:     function titleAttrib($content='',$hsc=0)
 * 1529:     function titleAltAttrib($content)
 * 1553:     function thumbCode($row,$table,$field,$backPath,$thumbScript='',$uploaddir=NULL,$abs=0,$tparams='',$size='')
 * 1622:     function getThumbNail($thumbScript,$theFile,$tparams='',$size='')
 * 1639:     function titleAttribForPages($row,$perms_clause='',$includeAttrib=1)
 * 1701:     function getRecordIconAltText($row,$table='pages')
 * 1743:     function getLabelFromItemlist($table,$col,$key)
 * 1769:     function getItemLabel($table,$col,$printAllWrap='')
 * 1794:     function getRecordTitle($table,$row,$prep=0)
 * 1832:     function getProcessedValue($table,$col,$value,$fixed_lgd_chars=0,$defaultPassthrough=0,$noRecordLookup=FALSE,$uid=0)
 * 1955:     function getProcessedValueExtra($table,$fN,$fV,$fixed_lgd_chars=0,$uid=0)
 * 1979:     function getFileIcon($ext)
 * 1993:     function getCommonSelectFields($table,$prefix='')
 * 2036:     function makeConfigForm($configArray,$defaults,$dataPrefix)
 *
 *              SECTION: Backend Modules API functions
 * 2111:     function helpTextIcon($table,$field,$BACK_PATH,$force=0)
 * 2133:     function helpText($table,$field,$BACK_PATH,$styleAttrib='')
 * 2185:     function cshItem($table,$field,$BACK_PATH,$wrap='',$onlyIconMode=FALSE, $styleAttrib='')
 * 2223:     function editOnClick($params,$backPath='',$requestUri='')
 * 2242:     function viewOnClick($id,$backPath='',$rootLine='',$anchor='',$altUrl='',$addGetVars='',$switchFocus=TRUE)
 * 2274:     function getModTSconfig($id,$TSref)
 * 2295:     function getFuncMenu($mainParams,$elementName,$currentValue,$menuItems,$script='',$addparams='')
 * 2338:     function getFuncCheck($mainParams,$elementName,$currentValue,$script='',$addparams='',$tagParams='')
 * 2363:     function getFuncInput($mainParams,$elementName,$currentValue,$size=10,$script="",$addparams="")
 * 2384:     function unsetMenuItems($modTSconfig,$itemArray,$TSref)
 * 2407:     function getSetUpdateSignal($set='')
 * 2458:     function getModuleData($MOD_MENU, $CHANGED_SETTINGS, $modName, $type='', $dontValidateList='', $setDefaultList='')
 *
 *              SECTION: Core
 * 2530:     function lockRecords($table='',$uid=0,$pid=0)
 * 2559:     function isRecordLocked($table,$uid)
 * 2599:     function exec_foreign_table_where_query($fieldValue,$field='',$TSconfig=array(),$prefix='')
 * 2680:     function getTCEFORM_TSconfig($table,$row)
 * 2730:     function getTSconfig_pidValue($table,$uid,$pid)
 * 2758:     function getPidForModTSconfig($table,$uid,$pid)
 * 2774:     function getTSCpid($table,$uid,$pid)
 * 2790:     function firstDomainRecord($rootLine)
 * 2812:     function getDomainStartPage($domain, $path='')
 * 2842:     function RTEsetup($RTEprop,$table,$field,$type='')
 * 2861:     function &RTEgetObj()
 * 2900:     function &softRefParserObj($spKey)
 * 2932:     function explodeSoftRefParserList($parserList)
 * 2964:     function isModuleSetInTBE_MODULES($modName)
 * 2987:     function referenceCount($table,$ref,$msg)
 *
 *              SECTION: Workspaces / Versioning
 * 3046:     function selectVersionsOfRecord($table, $uid, $fields='*', $workspace=0)
 * 3092:     function fixVersioningPid($table,&$rr)
 * 3132:     function workspaceOL($table,&$row,$wsid=-99)
 * 3180:     function getWorkspaceVersionOfRecord($workspace, $table, $uid, $fields='*')
 * 3209:     function getLiveVersionOfRecord($table,$uid,$fields='*')
 * 3231:     function isPidInVersionizedBranch($pid, $table='',$returnStage=FALSE)
 * 3254:     function versioningPlaceholderClause($table)
 * 3268:     function countVersionsOfRecordsOnPage($workspace,$pageId, $allTables=FALSE)
 * 3303:     function wsMapId($table,$uid)
 *
 *              SECTION: Miscellaneous
 * 3333:     function typo3PrintError($header,$text,$js='',$head=1)
 * 3377:     function TYPO3_copyRightNotice()
 * 3401:     function displayWarningMessages()
 * 3458:     function getPathType_web_nonweb($path)
 * 3470:     function ADMCMD_previewCmds($pageinfo)
 * 3492:     function processParams($params)
 * 3518:     function getListOfBackendModules($name,$perms_clause,$backPath='',$script='index.php')
 *
 * TOTAL FUNCTIONS: 98
