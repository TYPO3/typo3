	...
	function updateIndex($testOnly, $cli_echo = FALSE) {
		...
		foreach ($GLOBALS['TCA'] as $tableName => $cfg) {
			...
			if (!is_array($allRecs)) {
				...
				t3lib_div::sysLog(sprintf('Table "%s" exists in $TCA but does not exist in the database. You should run the Database Analyzer in the Install tool to fix this.', $tableName), 'core', 3);
				continue;
			}
