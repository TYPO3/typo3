...
class t3lib_install {
	...
	var $fontTag2='<font face="verdana,sans-serif" size=2 color=black>';
	var $fontTag1='<font face="verdana,sans-serif" size=1 color=black>';
	...
	var $allowUpdateLocalConf=0;	// If set, this class will allow the user to update the localconf.php file. Is set true in the init.php file.
	var $dumpImCommands=1;			// If set, the image Magick commands are always outputted in the image processing checker
	var $deletedPrefixKey="zzz_deleted_";	//
	var $backPath="../";
...
//		debug($GLOBALS["HTTP_COOKIE_VARS"]);
	...
	function stepOutput()	{
		...
		$error_missingConnect='<br>
			...
			<br>
			<strong>Go to Step 1</strong> and enter a proper username/password!</font>
			<br>
			<br>
		';
		$error_missingDB='<br>
		...
		$dbInfo='
					<table border=0 cellpadding=1 cellspacing=0>
					   	...
					   	<tr>
					   		...
					   		<td valign="top" nowrap><strong>'.$this->fontTag1.''.TYPO3_db_username.'</font></strong></td>
						</tr>
					   	<tr>
					   		<td valign="top" nowrap>'.$this->fontTag1.'Password:</font></td>
					   		<td valign="top" nowrap><strong>'.$this->fontTag1.''.TYPO3_db_password.'</font></strong></td>
						</tr>
					   	<tr>
					   		<td valign="top" nowrap>'.$this->fontTag1.'Host:</font></td>
					   		<td valign="top" nowrap><strong>'.$this->fontTag1.''.TYPO3_db_host.'</font></strong></td>
						</tr>
					   	<tr>
					   		<td valign="top" nowrap>'.$this->fontTag1.'Database:</font></td>
					   		<td valign="top" nowrap><strong>'.$this->fontTag1.''.TYPO3_db.'</font></strong></td>
						</tr>
					   	<tr>
					   		<td valign="top" nowrap>'.$this->fontTag1.'# of tables:</font></td>
					   		<td valign="top" nowrap><strong>'.$this->fontTag1.''.(count($whichTables)?'<font color=red>'.count($whichTables).'</font>':count($whichTables)).'</font></strong></td>
						</tr>
					</table>
		';
		$error_emptyDB='<br>
			...
			case 2:
				if ($result = @mysql_pconnect(TYPO3_db_host, TYPO3_db_username, TYPO3_db_password))	{
					...
					if (!$dbIncluded && TYPO3_db)	{
						$options.='<option value="'.htmlspecialchars(TYPO3_db).'" selected>'.htmlspecialchars(TYPO3_db).' (NO ACCESS!)</option>';
					}
	...
	function typo3conf_edit()	{
		...
		while(list($k,$file)=each($typo3conf_files))	{
			if ($this->INSTALL["typo3conf_files"] && !strcmp($this->INSTALL["typo3conf_files"],$file))	{
				$wrap=array('<strong><font color=navy>','</font></strong>');
				$fileFound = 1;
			} else {$wrap=array();}
			$lines[]='<tr><td nowrap><a href="'.$this->action.'&TYPO3_INSTALL[typo3conf_files]='.rawurlencode($file).($this->allowFileEditOutsite_typo3conf_dir?'&TYPO3_INSTALL[FILE][EDIT_path]='.rawurlencode($this->INSTALL["FILE"]["EDIT_path"]):"").'">'.$this->fw($wrap[0].basename($file).$wrap[1]."&nbsp;&nbsp;&nbsp;").'</a></td><td>'.$this->fw(t3lib_div::formatSize(filesize($file))).'</td></tr>';
		}
	...
	function getSelectorOptions($deleteType,$tt="")	{
		...
		if (is_array($deleteType))	{
			...
			while(list($v,$l)=each($deleteType))	{
				$out.='<option value="'.htmlspecialchars($v).'"'.(!strcmp($v,$tt)?" selected":"").'>'.htmlspecialchars($l).'</option>';
			}
		}
		return $out;
	}
					...
					while(list($vk,$value)=each($va))	{
						if (!is_array($value) && $this->checkForBadString($value))	{
							...
							if (strstr($value,chr(10)))	{
								...
								$form.='<input type="checkbox" name="TYPO3_INSTALL[extConfig]['.$k.']['.$vk.']"'.($value?" checked":"").' value="'.($value&&strcmp($value,"0")?htmlspecialchars($value):1).'">';
							} else {
...
#						debug($this->messages);
	...
	function writeToLocalconf_control($lines="",$dontOutputContent=0)	{
		...
		if (!@is_writeable($this->writeToLocalconf_dat["file"]))	{
			die($this->writeToLocalconf_dat["file"]." is not writeable!");
		}
		if (!is_array($lines))	{
			...
			$updatedLine=$this->writeToLocalconf_dat["updatedText"].date("d-m-Y H:i:s");
			array_push($lines,$updatedLine);
			array_push($lines,$this->writeToLocalconf_dat["endLine"]);
			//debug($lines);
			if ($this->setLocalconf)	{
				...
				if (!$dontOutputContent)	{
					$content = $this->fw('<strong>Writing to \'localconf.php\':</strong><BR><BR>No values were changed, so nothing is updated!<BR><BR><a href="'.$this->action.'">Click to continue...</a>');
...
//		if (ereg("[".chr(10).chr(13)."\$\"\\]|//",$string)){
	...
	function setValueInLocalconfFile(&$line_array, $variable, $value)	{
		...
		if (!$this->localconf_addLinesOnly && !$tokenSet)	{
			...
			reset($line_array);
			while(list($k,$v)=each($line_array))	{
				$v2=trim($v);
...
//			$line_array[] = $variable.' = "'.addslashes($value).'";	// '.$comment;
	...
	function getFormElement($labels,$values,$fieldName,$default,$msg="")	{
		...
		if (count($labels)>1)		{
			...
			while(list($k,$v)=each($labels))	{
				...
				$options.='<option value="'.htmlspecialchars($values[$k]).'"'.(!strcmp($default,$cleanV)?" selected":"").'>'.htmlspecialchars($v).'</option>';
			}
...
#			t3lib_div::getFilesInDir(PATH_t3lib."stddb/","sql",1,1),
		...
		$out='';
			...
			$out.='<tr>
				<td nowrap><strong>'.$this->fw('Update required tables').'</strong></td>
				<td'.($action_type=="cmpFile|CURRENT_TABLES"?' bgColor="#D9D5C9"':'').'>'.$this->fw('<a href="'.$this->action.'&TYPO3_INSTALL[database_type]=cmpFile|CURRENT_TABLES#bottom"><strong>COMPARE</strong></a>').'</td>
				<td>'.$this->fw('&nbsp;').'</td>
				<td>'.$this->fw('&nbsp;').'</td>
			</tr>';

			$out.='<tr>
				<td nowrap><strong>'.$this->fw('Dump static data').'</strong></td>
				<td>'.$this->fw('&nbsp;').'</td>
				<td nowrap'.($action_type=="import|CURRENT_STATIC"?' bgColor="#D9D5C9"':'').'>'.$this->fw('<a href="'.$this->action.'&TYPO3_INSTALL[database_type]=import|CURRENT_STATIC#bottom"><strong>IMPORT</strong></a>').'</td>
				<td>'.$this->fw('&nbsp;').'</td>
			</tr>';
...
/*			$spec1 = (!count($whichTables) && $fShortName==TYPO3_mainDir."t3lib/install/tables_".str_replace(".","",$GLOBALS["TYPO_VERSION"]).".sql" ? " <font color=#000066>(1)</font>" : "");
			$spec2 = (count($whichTables) && !$static_template_count && $fShortName==TYPO3_mainDir."t3lib/install/static_template_".str_replace(".","",$GLOBALS["TYPO_VERSION"])."+adt.sql" ? " <font color=#000066>(2)</font>" : "");
			*/
			$out.='<tr>
				<td nowrap>'.$this->fw($fShortName." (".t3lib_div::formatSize(filesize($file)).")").'</td>
				<td'.($action_type=="cmpFile|".$file?' bgColor="#D9D5C9"':'').'>'.$this->fw('<a href="'.$this->action.'&TYPO3_INSTALL[database_type]=cmpFile|'.rawurlencode($file).'#bottom"><strong>COMPARE</strong></a>').'</td>
				<td nowrap'.($action_type=="import|".$file?' bgColor="#D9D5C9"':'').'>'.$this->fw('<a href="'.$this->action.'&TYPO3_INSTALL[database_type]=import|'.rawurlencode($file).'#bottom"><strong>IMPORT'.$spec1.$spec2.'</strong></a>').'</td>
				<td nowrap'.($action_type=="view|".$file?' bgColor="#D9D5C9"':'').'>'.$this->fw('<a href="'.$this->action.'&TYPO3_INSTALL[database_type]=view|'.rawurlencode($file).'#bottom"><strong>VIEW'.$spec1.$spec2.'</strong></a>').'</td>
			</tr>';
		}
			// TCA
		$out.='<tr>
			<td></td>
			<td colspan=3'.($action_type=="cmpTCA|"?' bgColor="#D9D5C9"':'').'><strong>'.$this->fw('<a href="'.$this->action.'&TYPO3_INSTALL[database_type]=cmpTCA|#bottom">Compare with $TCA</a>').'</strong></td>
		</tr>';
		$out.='<tr>
			<td></td>
			<td colspan=3'.($action_type=="adminUser|"?' bgColor="#D9D5C9"':'').'><strong>'.$this->fw('<a href="'.$this->action.'&TYPO3_INSTALL[database_type]=adminUser|#bottom">Create "admin" user</a>').'</strong></td>
		</tr>';
		$out.='<tr>
			<td></td>
			<td colspan=3'.($action_type=="UC|"?' bgColor="#D9D5C9"':'').'><strong>'.$this->fw('<a href="'.$this->action.'&TYPO3_INSTALL[database_type]=UC|#bottom">Reset user preferences</a>').'</strong></td>
		</tr>';
		$out.='<tr>
			<td></td>
			<td colspan=3'.($action_type=="cache|"?' bgColor="#D9D5C9"':'').'><strong>'.$this->fw('<a href="'.$this->action.'&TYPO3_INSTALL[database_type]=cache|#bottom">Clear tables</a>').'</strong></td>
		</tr>';
...
//debug(array($tblFileContent));
					...
					if ($tblFileContent)	{
						...
						if (is_array($this->INSTALL["database_update"]))	{
							...
							$this->preformUpdateQueries($update_statements["add"],$this->INSTALL["database_update"]);
							$this->preformUpdateQueries($update_statements["change"],$this->INSTALL["database_update"]);
							$this->preformUpdateQueries($remove_statements["change"],$this->INSTALL["database_update"]);
							$this->preformUpdateQueries($remove_statements["drop"],$this->INSTALL["database_update"]);

							$this->preformUpdateQueries($update_statements["create_table"],$this->INSTALL["database_update"]);
							...
							if (count($statements_table))	{
								...
								while(list($table,$definition)=each($statements_table))	{
									...
									$out.='<tr>
										...
										<td nowrap>'.$this->fw($insertCount[$table]?"Rows: ".$insertCount[$table]:"").'</td>
										<td><img src=clear.gif width=10 height=1></td>
			...
			while(list($key,$string)=each($arr))	{
				...
				if ($iconDis)	{
					if (stristr($string," user_"))	{
						$ico.= '<img src="'.$this->backPath.'t3lib/gfx/icon_warning.gif" width=18 height=16 align=top><strong>(USER) </strong>';
					}
					if (stristr($string," app_"))	{
						$ico.= '<img src="'.$this->backPath.'t3lib/gfx/icon_warning.gif" width=18 height=16 align=top><strong>(APP) </strong>';
					}
					if (stristr($string," ttx_") || stristr($string," tx_"))	{
						$ico.= '<img src="'.$this->backPath.'t3lib/gfx/icon_warning.gif" width=18 height=16 align=top><strong>(EXT) </strong>';
					}
				}
				$out.='<tr><td valign=top><input type="checkbox" name="TYPO3_INSTALL[database_update]['.$key.']" value="1"'.($checked?' checked':'').'></td><td nowrap>'.$this->fw(nl2br($ico.$string)).'</td></tr>';
				if (isset($currentValue[$key]))	{
					$out.='<tr><td valign=top></td><td nowrap>'.$this->fw(nl2br("<font color=#666666>".(!$cVfullMsg?"Current value: ":"")."<em>".$currentValue[$key]."</em></font>")).'</td></tr>';
				}
			}
			$content.='<BR><div align="center"><strong>'.$this->fw($label,3).'</strong></div><BR><table border=0 cellpadding=2 cellspacing=2>'.$out.'</table>';
		}
		return $content;
	}
...
/*			if (strtoupper(substr(trim($linecontent),0,strlen($str)))==$str)	{
				$outStatements[]=$linecontent;
			}*/
		}
	...
	function getStatementArray($sqlcode,$removeNonSQL=0,$query_regex="")	{
		...
		reset($sqlcodeArr);
		while(list($line,$linecontent)=each($sqlcodeArr))	{
	...
	function getFieldDefinitions_database($database)	{
		...
		if (is_array($tempKeys))	{
			...
			while(list($table,$keyInf)=each($tempKeys))	{
				reset($keyInf);
				while(list($kName,$index)=each($keyInf))	{
					ksort($index);
	...
	function getFieldDefinitions_sqlContent($sqlContent)	{
		...
		reset($lines);
		$isTable="";
		while(list(,$value)=each($lines))	{
			if ($value[0]!="#")	{
				if (!$isTable)	{
					$parts = explode(" ",$value);
					if ($parts[0]=="CREATE" && $parts[1]=="TABLE")	{
						$isTable=$parts[2];
						if (TYPO3_OS=="WIN")	{$isTable=strtolower($isTable);}	// tablenames are always lowercase on windows!
					}
				} else {
	...
	function getDatabaseExtra($FDsrc, $FDcomp)	{
		...
		if (is_array($FDsrc))	{
			...
			while(list($table,$info)=each($FDsrc))	{
				if (!isset($FDcomp[$table]))	{
					...
					while(list(,$theKey)=each($keyTypes))	{
		...
		return array("extra"=>$extraArr, "diff"=>$diffArr, "diff_currentValues"=>$diffArr_cur);
	}
...
//					$extraArr[$table]=$info;		// If the table was not in the FDcomp-array, the result array is loaded with that table.
	...
	function getUpdateSuggestions($diffArr,$keyList="extra,diff")	{
		...
		while(list(,$theKey)=each($keyList))	{
			if (is_array($diffArr[$theKey]))	{
				reset($diffArr[$theKey]);
				while(list($table,$info)=each($diffArr[$theKey]))	{
					$whole_table=array();
					if (is_array($info["fields"]))	{
						reset($info["fields"]);
						while(list($fN,$fV)=each($info["fields"]))	{
							if ($info["whole_table"])	{
								$whole_table[]=$fN." ".$fV;
							} else {
								if ($theKey=="extra")	{
					...
					if (is_array($info["keys"]))	{
						reset($info["keys"]);
						while(list($fN,$fV)=each($info["keys"]))	{
							if ($info["whole_table"])	{
								if ($fN=="PRIMARY")	{
//									$whole_table[]="PRIMARY KEY ".$fV;
									$whole_table[]=$fV;
								} else {
//									$whole_table[]="KEY ".$fN." ".$fV;
									$whole_table[]=$fV;
								}
							} else {
								if ($theKey=="extra")	{
...
//				$bgCol =' bgcolor=yellow';
	...
	function fw($str,$size=1)	{
		if ($GLOBALS["CLIENT"]["SYSTEM"]=="unix" && $GLOBALS["CLIENT"]["BROWSER"]=="konqu")	{
			return '<font face="sans-serif" size='.($size+2).' color=black>'.$str.'</font>';
		}
		return '<font face="verdana,sans-serif" size='.$size.' color=black>'.$str.'</font>';
	}
	...
	function wrapInCells($label,$content)	{
		return '<tr><td valign=top nowrap><strong>'.$this->fw($label).'</strong></td><td>&nbsp;</td><td valign=top>'.$this->fw($content).'<BR></td></tr>';
	}
	...
	function printAll()	{
		...
		while(list($header,$valArray)=each($this->sections))	{
			$out.='
			<tr><td>&nbsp;</td></tr>
			<tr><td><strong><div align="center">'.$this->fw($header.":",2).'</div></strong></td></tr>
			';
...
'.($this->step?$this->stepHeader():$this->menu()).$content.'<HR>'.$this->note123().$this->endNotes().'						
	...
	function stepHeader()	{
		...
		$msg4='You\'re done!';
		$out='<img src="../t3lib/gfx/123_'.$this->step.'.png" width="402" height="73" border="0" alt="" usemap="#id123_print_Map">
<MAP NAME="id123_print_Map">
<AREA title="'.$msg4.'" SHAPE="poly" ALT="" COORDS="299,35, 303,19, 313,9, 335,5, 366,6, 379,11, 388,21, 392,35, 390,47, 381,58, 376,64, 359,67, 320,66, 307,60, 302,51, 300,44" HREF="'.$this->scriptSelf.'?mode='.$this->mode.'&step=go">
<AREA title="'.$msg3.'" SHAPE="circle" ALT="" COORDS="234,36,32" HREF="'.$this->scriptSelf.'?mode='.$this->mode.'&step=3">
