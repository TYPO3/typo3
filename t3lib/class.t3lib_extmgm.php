...
' . $content;
	...
	protected static function createTypo3LoadedExtensionInformationArray() {
		$loadedExtensions = self::getLoadedExtensionListArray();
	...
	public static function isLocalconfWritable() {
		...
		if (
			!@is_writable(PATH_typo3conf . t3lib_Configuration::LOCALCONF_FILE)
			&& !@is_writable(PATH_typo3conf . t3lib_Configuration::LOCAL_CONFIGURATION_FILE)
		) {
			$result = FALSE;
		}
		return $result;
	...
	public static function loadExtension($extensionKey) {
		...
		$extList = t3lib_Configuration::getLocalConfigurationValueByPath('EXT/extList');
	...
	/**
	 * Write extension list and clear cache files.
	 *
	 * @param array Extension array to load, loader order is kept
	 * @return void
	 */
	protected static function writeNewExtensionList(array $newExtensionList) {
		$extensionList = array_unique($newExtensionList);
		t3lib_Configuration::setLocalConfigurationValueByPath('EXT/extList', $extensionList);
