 ...
 *
 * $Id: class.t3lib_tcemain.php 3947 2008-08-07 09:56:03Z flyguide $
				...
				if (t3lib_extMgm::isLoaded('cms'))	{
					...
					if ($table === 'pages' || $table === 'pages_language_overlay')	{
						...
						if ($TSConfig['clearCache_pageGrandParent'])	{
							...
							/* Add pages that have content elements linking to this page.
							 * We use all soft references but could eg. restrict to typolinks.
							 * Reason: a changed visibility of the page should be applied instantly
							 */
						if ($TSConfig['clearCache_pageLinkReference'])	{
							$res_tmp = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
											'pid',
											'sys_refindex,tt_content',
											'tablename=\'tt_content\' and ref_table=\'pages\' and recuid=uid and ref_uid='.intval($pageUid) .
												t3lib_BEfunc::deleteClause('tt_content') . t3lib_BEfunc::BEenableFields('tt_content')
										);
							if ($row_tmp = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res_tmp))	{
								$list_cache[] = $row_tmp['uid'];
							}
						...
						$list_cache[] = $pageUid = intval($this->getPID($table,$uid));
							...
							// Add pages that show the content from this page
						if ($table == 'tt_content' && $TSConfig['clearCache_pageContentReference'])	{
							$res_tmp = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
											'uid',
											'pages',
											'content_from_pid='.intval($pageUid).' AND deleted=0'
										);
							if ($row_tmp = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res_tmp))	{
								$list_cache[] = $row_tmp['uid'];
							}
					...
					if (is_array($list_cache))	{
						$list_cache_ids = implode(',',$GLOBALS['TYPO3_DB']->cleanIntArray($list_cache));
						$GLOBALS['TYPO3_DB']->exec_DELETEquery('cache_pages','page_id IN ('.$list_cache_ids.')');
						$GLOBALS['TYPO3_DB']->exec_DELETEquery('cache_pagesection', 'page_id IN ('.$list_cache_ids.')');
					}
				}
			}
