	...
	topbarButtonCallback: function() {
		...
		if (topPanel.currentlyClickedButton === this) {
			...
			if (!topPanel.filteringIndicator) {
				Ext.getCmp('typo3-pagetree-topPanelItems').setHeight(topPanel.topPanelItemsHeight);
			} else {
				Ext.getCmp('typo3-pagetree-topPanelItems').setHeight(topPanel.topPanelItemsHeight + topPanel.indicatorBarHeight);
			}
		} else {
			this.toggle(true);
			...
			if (this.connectedWidget.id === topPanel.id + '-languageWrap') {
				Ext.getCmp('typo3-pagetree-topPanelItems').setHeight(topPanel.topPanelItemsHeight - topPanel.languagePanelItemHeight + topPanel.languagePanelAllItemsHeight);
			} else {
				if (!topPanel.filteringIndicator) {
					Ext.getCmp('typo3-pagetree-topPanelItems').setHeight(topPanel.topPanelItemsHeight);
				} else {
