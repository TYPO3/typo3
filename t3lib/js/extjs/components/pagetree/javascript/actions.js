...
TYPO3.Components.PageTree.Actions = {
	 ...
	 * @return {void}
	 */
	showNodeInMenu: function(node) {
		TYPO3.Components.PageTree.Commands.showNodeInMenu(
			node.attributes.nodeData,
			function(response) {
				if (this.evaluateResponse(response)) {
					this.updateNode(node, node.isExpanded(), response);
	 ...
	 * @return {void}
	 */
	hideNodeInMenu: function(node) {
		TYPO3.Components.PageTree.Commands.hideNodeInMenu(
			node.attributes.nodeData,
			function(response) {
				if (this.evaluateResponse(response)) {
					this.updateNode(node, node.isExpanded(), response);
