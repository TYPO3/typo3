...
TYPO3.Components.TcaValueSlider = Ext.extend(Ext.slider.SingleSlider, {
		...
		Ext.apply(this, {
			...
			listeners: {
				beforerender: function(slider) {
					...
					Ext.get(this.itemElement).on('change', function(a) {
						switch (this.type) {
							case 'array':
								newValue = this.itemElement.selectedIndex;
							break;
							case 'time':
								newValue = this.parseTime(this.itemElement.value);
							break;
							case 'float':
								newValue = parseFloat(this.itemElement.value) * this.floatValue;
							break;
							case 'int':
							default:
								newValue = parseInt(this.itemElement.value);
						}
						this.setValue(newValue);
						if (this.elementType == 'input') {
							this.itemElement.value = this.renderValue(this.getValue());
						}
						if (this.elementType == 'select') {
							this.itemElement.options[this.getValue()].selected = '1';
						}
					}, this);
				},
	...
	*/
	...
	/**
	* Parse integer value from time
	*
	* @param {string} value
	* @return int
	*/
	parseTime: function(value) {
		var dateObj = new Date('01/01/1970 ' + value);
		var seconds = (dateObj.getTime() / 1000) - (dateObj.getTimezoneOffset() * 60);
		return seconds;
	}
