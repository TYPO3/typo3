...
abstract class t3lib_userAuth {
	...
	function processLoginData($loginData, $passwordTransmissionStrategy = '') {
		$loginSecurityLevel = $passwordTransmissionStrategy ? $passwordTransmissionStrategy : ($GLOBALS['TYPO3_CONF_VARS'][$this->loginType]['loginSecurityLevel'] ? trim($GLOBALS['TYPO3_CONF_VARS'][$this->loginType]['loginSecurityLevel']) : $this->security_level);
		...
		while (is_object($serviceObject = t3lib_div::makeInstanceService('auth', $subType, $serviceChain))) {
			...
			$serviceResult = $serviceObject->processLoginData($processedLoginData, $loginSecurityLevel);
