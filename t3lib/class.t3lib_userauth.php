...
abstract class t3lib_userAuth {
	...
	public $security_level = 'normal';
		// Server session lifetime. If > 0: session-timeout in seconds. If FALSE or <0: no timeout. If string: The string is a fieldname from the usertable where the timeout can be found.
	var $auth_timeout_field = 0;
		// Client session lifetime. 0 = Session-cookies. If session-cookies, the browser will stop the session when the browser is closed. Otherwise this specifies the lifetime of a cookie that keeps the session.
	var $lifetime = 0;
		// GarbageCollection. Purge all server session data older than $gc_time seconds. 0 = default to $this->timeout or use 86400 seconds (1 day) if $this->lifetime is 0
	var $gc_time = 0;
		// Possibility (in percent) for GarbageCollection to be run.
	var $gc_probability = 1;
		// Decides if the writelog() function is called at login and logout
	var $writeStdLog = FALSE;
		// If the writelog() functions is called if a login-attempt has be tried without success
	var $writeAttemptLog = FALSE;
		// If this is set, headers is sent to assure, caching is NOT done
	var $sendNoCacheHeaders = TRUE;
		// If this is set, authentication is also accepted by the $_GET. Notice that the identification is NOT 128bit MD5 hash but reduced. This is done in order to minimize the size for mobile-devices, such as WAP-phones
	var $getFallBack = FALSE;
		// The ident-hash is normally 32 characters and should be! But if you are making sites for WAP-devices og other lowbandwidth stuff, you may shorten the length. Never let this value drop below 6. A length of 6 would give you more than 16 mio possibilities.
	var $hash_length = 32;
		// Setting this flag TRUE lets user-authetication happen from GET_VARS if POST_VARS are not set. Thus you may supply username/password from the URL.
	var $getMethodEnabled = FALSE;
		// If set, will lock the session to the users IP address (all four numbers. Reducing to 1-3 means that only first, second or third part of the IP address is used).
	var $lockIP = 4;
		// Keyword list (commalist with no spaces!): "useragent". Each keyword indicates some information that can be included in a integer hash made to lock down usersessions. Configurable through $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['lockHashKeyWords']
	var $lockHashKeyWords = 'useragent';
		// warning -emailaddress:
	var $warningEmail = '';
		// Period back in time (in seconds) in which number of failed logins are collected
	var $warningPeriod = 3600;
	...
	var $checkPid_value = 0;

		// Internals
		// Internal: Will contain session_id (MD5-hash)
	var $id;
		// Internal: Will contain the session_id gotten from cookie or GET method. This is used in statistics as a reliable cookie (one which is known to come from $_COOKIE).
	var $cookieId;
	...
	var $loginSessionStarted = FALSE;
		// Internal: Will contain user- AND session-data from database (joined tables)
	var $user;
		// Internal: Will will be set to the url--ready (eg. '&login=ab7ef8d...') GET-auth-var if getFallBack is TRUE. Should be inserted in links!
	var $get_URL_ID = '';
	...
	var $dontSetCookie = FALSE;
		// If set, the challenge value will be stored in a session as well so the server can check that is was not forged.
	var $challengeStoredInCookie = FALSE;
