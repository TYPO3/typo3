...
class t3lib_cacheHash implements t3lib_Singleton {
	...
	/**
	 * @var string comma seperated list of parameters that need a value to be relevant for cacheHash calculation
	 */
	...
	public function __construct() {
		...
		$this->excludedParametersIfEmpty = trim($GLOBALS['TYPO3_CONF_VARS']['FE']['cHashExcludedParametersIfEmpty']);
	...
	protected function isAllowedWithEmptyValue($key) {
		return !($this->excludedParametersIfEmpty === '*' || t3lib_div::inList($this->excludedParametersIfEmpty, $key));
	}
}
