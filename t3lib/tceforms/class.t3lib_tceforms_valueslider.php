...
class t3lib_TCEforms_ValueSlider {
	...
	public function renderWizard(array $parameters, t3lib_TCEforms $pObj) {
		...
		$step =  floatval($parameters['wConf']['step'] ? $parameters['wConf']['step'] : 1);
		...
		if (isset($parameters['fieldConfig']['items'])) {
			$type = 'array';
			// @todo String values are not supported currently
			$value = intval($value);
		}
		...
		$js = LF . 'new TYPO3.Components.TcaValueSlider(' . json_encode($options) . ');' . LF;
