...
class t3lib_TCEmain {
					...
					if (is_array($incomingFieldArray)) {
						...
						if (!t3lib_div::testInt($id)) { // Is it a new record? (Then Id is a string)
							...
							if (isset($incomingFieldArray['pid'])) { // A pid must be set for new records.
								...
								} elseif ($pid_value >= 0 && $this->BE_USER->workspace !== 0 && $GLOBALS['TCA'][$table]['ctrl']['versioning_followPages']) { // PID points to page, the workspace is an offline space and the table follows page during versioning: This means we must check if the PID page has a version in the workspace with swapmode set to 0 (zero = page+content) and if so, change the pid to the uid of that version.
							...
							if (is_array($fieldArray)) {
								if ($status == 'new') {
									if ($createNewVersion) { // This creates a new version of the record with online placeholder and offline version
										$versioningType = $table === 'pages' ? $this->BE_USER->workspaceVersioningTypeGetClosest(t3lib_div::intInRange($GLOBALS['TYPO3_CONF_VARS']['BE']['newPagesVersioningType'], -1, 1)) : -1;
	...
	function fillInFieldArray($table, $id, $fieldArray, $incomingFieldArray, $realPid, $status, $tscPID) {
				...
				if (!$languageDeny) {
					...
					switch ($field) {
						...
						default:
							...
							} elseif ($GLOBALS['TCA'][$table]['ctrl']['origUid'] === $field) { // Allow value for original UID to pass by...
		...
		if ($diffStorageFlag && !isset($fieldArray[$GLOBALS['TCA'][$table]['ctrl']['transOrigDiffSourceField']])) { // If the field is set it would probably be because of an undo-operation - in which case we should not update the field of course...
   ...
   */
								...
								if ($row[$GLOBALS['TCA'][$table]['ctrl']['transOrigPointerField']] == 0 || $table === 'pages') {
									...
									if ($pass) {
										...
										foreach ($GLOBALS['TCA'][$Ttable]['columns'] as $fN => $fCfg) {
											...
											} elseif (t3lib_div::inList('exclude,noCopy,mergeIfNotBlank', $fCfg['l10n_mode']) && $fN != $GLOBALS['TCA'][$Ttable]['ctrl']['languageField'] && $fN != $GLOBALS['TCA'][$Ttable]['ctrl']['transOrigPointerField']) { // Otherwise, do not copy field (unless it is the language field or pointer to the original language)
												$excludeFields[] = $fN;
											}
										}
	...
	function addDefaultPermittedLanguageIfNotSet($table, &$incomingFieldArray) {
		...
		if ($GLOBALS['TCA'][$table]['ctrl']['languageField']) {
			if (!isset($incomingFieldArray[$GLOBALS['TCA'][$table]['ctrl']['languageField']])) { // Language field must be found in input row - otherwise it does not make sense.
	...
	function resolvePid($table, $pid) {
		...
		if ($pid < 0) {
			...
			$pid = intval($row['pid']);
		} elseif ($this->BE_USER->workspace !== 0 && $GLOBALS['TCA'][$table]['ctrl']['versioning_followPages']) { // PID points to page, the workspace is an offline space and the table follows page during versioning: This means we must check if the PID page has a version in the workspace with swapmode set to 0 (zero = page+content) and if so, change the pid to the uid of that version.
