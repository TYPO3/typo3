...
class t3lib_TCEmain {
					...
					if (is_array($incomingFieldArray)) {
						...
						foreach ($GLOBALS['TCA'][$table]['columns'] as $column => $config) {
							if (isset($incomingFieldArray[$column])) {
								if (isset($config['config']['xtype']) && t3lib_div::inList('date,datetime', $config['config']['xtype'])) {
									$emptyValue = $dateTimeFormats[$config['config']['xtype']]['empty'];
									$format = $dateTimeFormats[$config['config']['xtype']]['format'];
	...
	function checkValue_input($res, $value, $tcaFieldConf, $PP, $field = '') {
		list($table, $id, $curValue, $status, $realPid, $recFID) = $PP;
			...
			// Handle native date/time fields
		$dateTimeFormats = $GLOBALS['TYPO3_DB']->getDateTimeFormats($table);
		if (isset($tcaFieldConf['xtype']) && t3lib_div::inList('date,datetime', $tcaFieldConf['xtype'])) {
				// Converts the date/time into a timestamp for the sake of the checks
			$emptyValue = $dateTimeFormats[$tcaFieldConf['xtype']]['empty'];
			$timeOffset = $tcaFieldConf['xtype'] === 'date' ? 3600 * $GLOBALS['TYPO3_CONF_VARS']['SYS']['serverTimeZone'] : 0;
		...
		if ($field && $realPid >= 0) {
			...
			// Handle native date/time fields
		if (isset($tcaFieldConf['xtype']) && t3lib_div::inList('date,datetime', $tcaFieldConf['xtype'])) {
				// Converts the timestamp back to a date/time
			$emptyValue = $dateTimeFormats[$tcaFieldConf['xtype']]['empty'];
			$format = $dateTimeFormats[$tcaFieldConf['xtype']]['format'];
