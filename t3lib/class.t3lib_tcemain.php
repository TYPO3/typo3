...
class t3lib_TCEmain {
   ...
   */
	...
	/**
	 * Determines whether the this object is the outer most one.
	 * Since t3lib_TCEmain can create nested objects of itself,
	 * this method helps to determine the first (= outer most) one.
	 *
	 * @return boolean
	 */
	protected function isOuterMost() {
		if (!isset($this->isOuterMost)) {
			...
			foreach ($stack as $stackItem) {
				if (isset($stackItem['object']) && $stackItem['object'] instanceof t3lib_TCEmain) {
					$this->isOuterMost = ($stackItem['object'] === $this);
					break;
				}
			}
		}
