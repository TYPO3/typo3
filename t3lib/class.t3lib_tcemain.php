   ...
   */
	...
	public function clear_cacheCmd($cacheCmd) {
		...
		if (t3lib_div::isFirstPartOfStr(strtolower($cacheCmd), 'cachetag:')) {
			$cacheTag = substr($cacheCmd, strlen('cachetag:'));
			$pageCache->flushByTag($cacheTag);
			$pageSectionCache->flushByTag($cacheTag);
		}

			// remove cache entry by identifier
		if (t3lib_div::isFirstPartOfStr(strtolower($cacheCmd), 'cacheid:')) {
			$cacheId = substr($cacheCmd, strlen('cacheId:'));
			$pageCache->remove($cacheId);
			$pageSectionCache->remove($cacheId);
		}
