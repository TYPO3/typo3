			...
			for ($a = -1; $a < count($str1Lines); $a++) {
				if (is_array($diffResArray[$a + 1])) {
					...
					if (is_array($diffResArray[$a + 1]['old'])) {
						$outString .= '<' . $wrapTag . ' class="diff-r">' . htmlspecialchars(html_entity_decode(implode(' ', $diffResArray[$a + 1]['old']))) . '</' . $wrapTag . '> ';
					}
					if (is_array($diffResArray[$a + 1]['new'])) {
						$outString .= '<' . $wrapTag . ' class="diff-g">' . htmlspecialchars(html_entity_decode(implode(' ', $diffResArray[$a + 1]['new']))) . '</' . $wrapTag . '> ';
					}
					...
					$clearBuffer .= htmlspecialchars(html_entity_decode($str1Lines[$a])) . ' ';
				}
			}
