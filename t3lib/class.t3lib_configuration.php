...
class t3lib_Configuration {
	 ...
	 * Merges current configuration and new configuration.
	 *
	...
	/**
	 * Update a given path in local conf to a new value.
	 *
	...
	public static function setLocalConfigurationValueByPath($path, $value) {
		$result = FALSE;
		if (static::isValidLocalConfigurationPath($path) === TRUE) {
	...
	/**
	 * Update / set a list of path and values in local configuration file
	 *
	...
	protected static function isValidLocalConfigurationPath($path) {
		...
		foreach (static::$whiteListedLocalConfigurationPaths as $whiteListedPath) {
			if (mb_strpos($path, $whiteListedPath) === 0) {
				return TRUE;
			}
		}
