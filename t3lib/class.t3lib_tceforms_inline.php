 ...
 * @coauthor Kai Vogel <kai.vogel@speedprogs.de>
 */
class t3lib_TCEforms_inline {
	...
	function getSingleField_typeInline($table, $field, $row, &$PA) {
		...
		$this->inlineData['config'][$nameObject . self::Structure_Separator . $foreign_table] = array(
			...
			'fieldConfiguration' => json_encode($config),
		);
	 ...
	 * @param mixed $forceDeleted Mark record as deleted
	 * @return string The HTML code for this "foreign record"
	 */
	function renderForeignRecord($parentUid, $rec, $config = array(), $forceDeleted = FALSE) {
		$foreign_table = $config['foreign_table'];
		...
		if (!$isVirtualRecord) {
			...
			if ($isNewRecord) {
				...
				$disabled = (!$forceDeleted ? ' disabled="disabled"' : '');
	...
	protected function processAjaxRequestConstruct(&$ajaxArguments) {
		...
		$GLOBALS['SOBE']->tceforms->enableTabMenu = TRUE;
			...
			// Start clipboard
		$GLOBALS['SOBE']->tceforms->clipObj = t3lib_div::makeInstance('t3lib_clipboard');
	 ...
	 *
	 * @param string $domObjectId The calling object in hierarchy, that requested a new record.
	 * @param string $returnURL Unused parameter for AJAX call
	 * @param mixed $forceDeleted Mark record as deleted
	 * @return array An array to be used for JSON
	 */
	function getRecordDetails($domObjectId, $returnURL = NULL, $forceDeleted = NULL) {
			// The current table - for this table we should add/import records
		$current = $this->inlineStructure['unstable'];
			// The parent table - this table embeds the current table
		$parent = $this->getStructureLevel(-1);
		...
		$expandSingle = (isset($config['appearance']['expandSingle']) && $config['appearance']['expandSingle']);
			...
			// Get deleted state of the field from AJAX call
		$forceDeleted = ($forceDeleted === 'true' || $forceDeleted === TRUE);
		...
		$item = $this->renderForeignRecord($parent['uid'], $record, $config, $forceDeleted);
		if ($item === FALSE) {
			return $this->getErrorMessageForAJAX('Access denied');
		}
	 ...
	 * @param array $forceConfiguration Forece this field configuration if not empty
	 * @return void
	 */
	function parseStructureString($string, $loadConfig = TRUE, array $forceConfiguration = array()) {
		...
		while (count($parts) > 2) {
			...
			if (!empty($loadConfig)) {
				...
				if (!empty($forceConfiguration)) {
					$stable['config'] = t3lib_div::array_merge_recursive_overrule($stable['config'], $forceConfiguration);
				}
