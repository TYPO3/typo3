			...
			case 'file': // If the element is of the internal type "file":
				...
				if (!$disabled && !(isset($config['disable_controls']) && t3lib_div::inList($config['disable_controls'], 'upload'))) {
					...
					if ($this->edit_docModuleUpload && $config['uploadfolder']) {
						...
						$multipleAttribute = '';
						$multipleFilenamePostix = '';
						if (isset($config['maxitems']) && $config['maxitems'] > 1) {
							$multipleAttribute = ' multiple="multiple"';
							$multipleFilenamePostix = '[]';
						}

						$item .= '<div id="' . $PA['itemFormElID_file'] . '"><input type="file"'. $multipleAttribute .' name="' . $PA['itemFormElName_file'] . $multipleFilenamePostix . '" size="35" onchange="' . implode('', $PA['fieldChangeFunc']) . '" /></div>';
