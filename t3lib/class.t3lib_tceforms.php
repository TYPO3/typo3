...
class t3lib_TCEforms {
	...
	function getSingleField_typeGroup($table, $field, $row, &$PA) {
		...
		$noList = isset($config['disable_controls']) && t3lib_div::inList($config['disable_controls'], 'list');
			...
			// If maxitems==1  automatically replace present item (in  list and file selector)
		if ($maxitems === 1) {
			...
			// If the list was removed clear first entry if list is full to avoid "write once" field
		else if ($noList) {
			...
			function setFormValueFromBrowseWin(fName,value,label,exclusiveValues) {
				...
				if (formObj && value !== "--div--") {
					...
					var len = fObj.length;
						...
						// clear field before setting value if configured (maxitems==1)
