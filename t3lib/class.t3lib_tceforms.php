...
class t3lib_TCEforms {
	...
	function getIcon($icon) {
		...
		if ($selIconInfo === FALSE) $selIconFile = ''; // unset to empty string if icon is not available
	...
	function optionTagStyle($iconString) {
		if ($iconString) {
			...
			if (empty($selIconFile)) return ''; // skip background style if image is unavailable
