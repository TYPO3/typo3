			...
			switch ($mode) {
				case 'db':
					foreach ($itemArray as $pp) {
						...
						if (is_array($pRec)) {
							...
							$uidList[] = $pUid;
							$opt[] = '<option value="' . htmlspecialchars($pUid) . '" title="' . htmlspecialchars($pTitle) . '">' . htmlspecialchars($pTitle) . '</option>';
						}
					}
				break;
				case 'file_reference':
				case 'file':
					foreach ($itemArray as $item) {
						...
						$opt[] = '<option value="' . htmlspecialchars(rawurldecode($itemParts[0])) . '" title="' . htmlspecialchars(basename(rawurldecode($itemParts[0]))) . '">' . htmlspecialchars(basename(rawurldecode($itemParts[0]))) . '</option>';
					}
				break;
				case 'folder':
					foreach ($itemArray as $pp) {
						...
						$opt[] = '<option value="' . htmlspecialchars(rawurldecode($pParts[0])) . '" title="' . htmlspecialchars(rawurldecode($pParts[0])) . '">' . htmlspecialchars(rawurldecode($pParts[0])) . '</option>';
					}
				break;
				default:
					foreach ($itemArray as $pp) {
						...
						$pTitle = $pParts[1];
						$opt[] = '<option value="' . htmlspecialchars(rawurldecode($pUid)) . '" title="' . htmlspecialchars(rawurldecode($pTitle)) . '">' . htmlspecialchars(rawurldecode($pTitle)) . '</option>';
					}
				break;
			}
		}
