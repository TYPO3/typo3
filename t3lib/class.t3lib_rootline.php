<?php
/**
 * Class for retrieving the rootline
 */
class t3lib_rootline {
	...
	protected function getRecordArray($uid) {
		if (!isset(self::$pageRecordCache[$uid])) {
			$array = $GLOBALS['TYPO3_DB']->exec_SELECTgetSingleRow(
				...
				'pages',
				'uid = ' . intval($uid) . ' AND pages.deleted = 0 AND pages.doktype <> 255'
			);
	...
	protected function generateRootlineCache() {
		...
		if ($page == NULL) {
			throw new RuntimeException('Broken rootline. Could not resolve page.', 1343464101);
		}
		...
		if ($this->isMountedPage()) {
			...
			$parentUid = $mountPoint['pid'];	// anyhow after reaching the mount-point, we have to go up that rootline
		...
		if ($parentUid > 0) {
			...
			foreach ($rootline AS $entry) {
				...
				if ($entry['uid'] == $this->pageUid) {
					throw new RuntimeException('Circular connection in rootline found. Check your mountpoint configuration.', 1343464103);
				}
			}
		} else {
			$rootline = array();
	...
	protected function processMountedPage(array $mountedPageData, array $mountPointPageData) {
		if ($mountPointPageData['doktype'] == t3lib_pageSelect::DOKTYPE_MOUNTPOINT && $mountPointPageData['mount_pid'] == $mountedPageData['uid']) {
			...
			throw new RuntimeException('Broken rootline. Mountpoint-Parameter does not match the actual rootline.', 1343464100);
		}
	}
