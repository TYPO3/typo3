...
class t3lib_softrefproc {
	...
	function getTypoLinkParts($typolinkValue) {
		...
		if (strstr($link_param, '@') && !$pU['scheme']) { // If it's a mail address:
			...
			} else { // integer or alias (alias is without slashes or periods or commas, that is 'nospace,alphanum_x,lower,unique' according to definition in $GLOBALS['TCA']!)
...
if (defined('TYPO3_MODE') && isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['t3lib/class.t3lib_softrefproc.php'])) {
