...
class t3lib_cache_backend_DbBackend extends t3lib_cache_backend_AbstractBackend {
	...
	public function set($entryIdentifier, $data, array $tags = array(), $lifetime = NULL) {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'id, tag',
			$this->tagsTable,
			'tag IN(' . implode(', ', $GLOBALS['TYPO3_DB']->fullQuoteArray($tags, $this->tagsTable)) . ')'
		);

		$existingTags = array();
		$cacheMMEntries = array();

		while(($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
			$existingTags[] = $row['tag'];
			$cacheMMEntries[] = array($cacheEntryId, $row['id']);
		}
		...
		foreach($newTags as $newTag) {
			...
			$cacheMMEntries[] = array($cacheEntryId, $GLOBALS['TYPO3_DB']->sql_insert_id());
		}

		$GLOBALS['TYPO3_DB']->exec_INSERTmultipleRows(
			$this->cacheMMTable,
			array('id_cache','id_tags'),
			$cacheMMEntries
		);
	}
