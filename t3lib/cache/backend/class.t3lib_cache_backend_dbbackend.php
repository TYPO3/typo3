...
class t3lib_cache_backend_DbBackend extends t3lib_cache_backend_AbstractBackend {
	...
	public function set($entryIdentifier, $data, array $tags = array(), $lifetime = NULL) {
		...
		$this->performInsert($entryIdentifier, $data,  $lifetime );
	 ...
	 * @return void
	 */
	private function performInsert($entryIdentifier, $data, $lifetime = NULL ) {
	 ...
	 *
	 * Works by recreating the tables to allow scheme updates
	 * on-the-fly.
	 *
	 * @return void
	 * @author Ingo Renner <ingo@typo3.org>
	...
	public function createTables($drop=TRUE) {
		...
		$query=str_replace('###CACHE_TABLE###',$templateCacheTable,$this->cacheTable);
		$GLOBALS['TYPO3_DB']->admin_query($query);
		$query=str_replace('###CACHE_TAGS_TABLE###',$templateTagsTable,$this->tagsTable);
		$GLOBALS['TYPO3_DB']->admin_query($query);
