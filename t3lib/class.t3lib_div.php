		...
		if (!isset($bytes{$count - 1})) {
			...
			if (TYPO3_OS != 'WIN' && function_exists('openssl_random_pseudo_bytes')) {
				...
				$bytes = openssl_random_pseudo_bytes(max(4096, $count), $isStrong);
					...
					// Windows php versions below 5.3.4 have a bug when using urandom source (see #24410)
				if (TYPO3_OS != 'WIN' || version_compare(PHP_VERSION, '5.3.4', '>=')) {
			...
			if (!isset($bytes{$count - 1}) && TYPO3_OS != 'WIN' && $fh = @fopen('/dev/urandom', 'rb')) {
