		...
		switch ((string) $getEnvName) {
			...
			case 'TYPO3_SSL':
				...
				if (self::cmpIP(self::getIndpEnv('REMOTE_ADDR'), $proxySSL)) {
					...
					$retVal = $_SERVER['SSL_SESSION_ID'] || ($GLOBALS['TYPO3_CONF_VARS']['SYS']['AcceptXForwardedProtoSSL'] === '1' && !strcasecmp($_SERVER['HTTP_X_FORWARDED_PROTO'], 'https')) || !strcasecmp($_SERVER['HTTPS'], 'on') || !strcmp($_SERVER['HTTPS'], '1') ? TRUE : FALSE; // see http://bugs.typo3.org/view.php?id=3909
				}
				break;
			case '_ARRAY':
				$out = array();
