...
final class t3lib_div {
	 ...
	 *
	 * @param	string		A hostname or an IPv4/IPv6-address (will by reverse-resolved; typ. REMOTE_ADDR)
	...
	public static function cmpFQDN($baseHost, $list) {
		...
		if (self::validIPv4($baseHost) || self::validIPv6($baseHost)) {
			...
			if(strcmp($baseHostName, $baseHost) == 0) {
		...
		foreach ($values as $test) {
			...
			foreach ($hostNameParts as $index => $val) {
				...
				} elseif (strcmp($baseHostNameParts[$index], $val) != 0) {
					// in case of no match
					$yes = 0;
				}
			}
