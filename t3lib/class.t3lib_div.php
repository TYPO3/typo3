	...
	public static function generateRandomBytes($bytesToReturn) {
		...
		if (!isset($bytes{$bytesToReturn - 1})) {
			...
			if (TYPO3_OS !== 'WIN' && function_exists('openssl_random_pseudo_bytes')) {
				$isStrong = NULL;
				$bytes = openssl_random_pseudo_bytes($bytesToGenerate, $isStrong);
			} else if (function_exists('mcrypt_create_iv')) {
					// Windows PHP versions have a bug when using urandom source (see #24410)
				$randomSource = (TYPO3_OS !== 'WIN' ? MCRYPT_DEV_URANDOM : MCRYPT_RAND);
				$bytes = @mcrypt_create_iv($bytesToGenerate, $randomSource);
			}

				// If openssl or mcrypt failed, try /dev/urandom on *nix systems
			if (!isset($bytes{$bytesToReturn - 1}) && TYPO3_OS !== 'WIN' && $fh = @fopen('/dev/urandom', 'rb')) {
