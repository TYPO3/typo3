...
class SC_show_item {
	...
	function renderDBInfo() {
		...
		$extraFields = array(
			'crdate' => $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_general.xml:LGL.timestamp', 1),
			'cruser_id' => $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_general.xml:LGL.creationDate', 1),
			'tstamp' => $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_general.xml:LGL.creationUserId', 1)
		);
	...
	function renderFileInfo($returnLinkTag) {
		...
		$code = '<div class="fileInfoContainer">'
			. t3lib_iconWorks::getSpriteIconForFile($fileExtension) . '<strong>' .
