...
class SC_show_item {
	...
	function makeRef($table,$ref)	{
		...
		foreach($rows as $row)	{
			$infoData[] = '<tr class="bgColor4"">' .
					'<td>'.$GLOBALS['LANG']->sL($GLOBALS['TCA'][$row['tablename']]['ctrl']['title']).'</td>' .
					'<td>'. $this->getRecordTitle($row['tablename'], $row['recuid']) . '</td>' .
					'<td>'. $this->getFieldName($row['tablename'], $row['field']) . '</td>'.
	...
	function makeRefFrom($table,$ref)	{
		...
		foreach($rows as $row)	{
			$infoData[] = '<tr class="bgColor4"">' .
					'<td>' . $this->getFieldName($table, $row['field']) . '</td>'.
					'<td>'.$row['flexpointer'].'</td>'.
					'<td>'.$row['softref_key'].'</td>'.
					'<td>'.$row['sorting'].'</td>'.
					'<td>' .$GLOBALS['LANG']->sL($GLOBALS['TCA'][$row['ref_table']]['ctrl']['title']) . '</td>'.
					'<td>'.$row['ref_uid'].'</td>' .
