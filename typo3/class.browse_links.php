...
class TBE_browser_recordList extends localRecordList {
	...
	function linkWrapItems($table,$uid,$code,$row)	{
		...
		return $ATag.
				'<img'.t3lib_iconWorks::skinImg($GLOBALS['BACK_PATH'],'gfx/plusbullet2.gif','width="18" height="16"').' title="'.$GLOBALS['LANG']->getLL('addToList',1).'" alt="" />'.
				$ATag_e.
				$ATag_alt.
				$code.
				$ATag_e;
...
';
	...
	function main_file()	{
		...
		if ($GLOBALS['BE_USER']->getTSConfigVal('options.uploadFieldsInTopOfEB'))	$content.=$uploadForm;
		...
		if (!$GLOBALS['BE_USER']->getTSConfigVal('options.uploadFieldsInTopOfEB'))	$content.=$uploadForm;
		if ($GLOBALS['BE_USER']->isAdmin() || $GLOBALS['BE_USER']->getTSConfigVal('options.createFoldersInEB'))	$content.=$createFolder;
