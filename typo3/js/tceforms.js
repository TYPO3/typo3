...
TYPO3.Tceforms.Polling = {
	...
	tables: new Ext.util.MixedCollection(),
		// contains records edited by someelse, where we had a warning
					...
					function(result) {
						...
						if(this.tables.key(key) == 0) {
							...
							if(this.tables.key(key) != result.tstamp) {
								...
								Ext.Msg.show(
									{
										title: TYPO3.l10n.localize('tceform.recordChanged', {1: result.tableTranslated, 2: result.title, 3:result.uid}),
										msg: TYPO3.l10n.localize('tceform.recordChangedMsg') + '<a href="' + window.location.href +'" target="_blank">' + TYPO3.l10n.localize('tceform.recordChangedMsg2') + '</a>',
