...
class bootstrap {
	...
	/**
	 * Calculate all required base pathes and set as constants.
	 ...
	 */
	public static function defineAndCheckPathes($relativePathPart = '') {
			// Relative path from document root to typo3/ directory
			// Hardcoded to "typo3/"
		define('TYPO3_mainDir', 'typo3/');

			// Absolute path of the entry script that was called
			// All pathes are unified between Windows and Unix, so the \ of Windows is subsituted to a /
		...
		self::checkMainPathesExist();
	}

	/**
	 * Determine the operating system TYPO3 is running on.
	 *
	 * @return string Either 'WIN' if running on Windows, else emty string
	 */
	 ...
	 * find out the script name that was called in the first place and to substract the local
	 ...
	 * If TYPO3_MOD_PATH is defined, substract this 'local' part from the entry point directory, else use
	 * $relativePathPart to substract this from the the script entry point to find out the document root.
	 *
	 ...
	 *
	 * The method unifies the above and substracts the calculated path part from PATH_thisScript
	 *
	 * @return string Absolute path to document root of installation
	 */
	protected static function getPathSiteByTypo3ModulePath() {
		if (
			...
			$pathPartRelativeToDocumentRoot = 'typo3/' . TYPO3_MOD_PATH;
	...
	/**
	 * Find out document root by substracting $relativePathPart from PATH_thisScript
	 *
	...
	/**
	 * Split file name off absolute script path and unify for Windows and Unix
	 *
	 ...
	 * @return void
	 */
	protected static function checkMainPathesExist() {
		if (!is_file(PATH_thisScript)) {
			die('Unable to determite path to entry script.');
		}
