...
class ClearCacheMenu implements backend_toolbarItem {
	...
	public function __construct(TYPO3backend &$backendReference = NULL) {
		...
		$this->optionValues     = array('all', 'pages');
		...
		else ($GLOBALS['BE_USER']->isAdmin() || $GLOBALS['BE_USER']->getTSConfigVal('options.clearCache.all')) {
		...
		else ($GLOBALS['BE_USER']->isAdmin() || $GLOBALS['BE_USER']->getTSConfigVal('options.clearCache.pages')) {
		...
		else ($GLOBALS['BE_USER']->isAdmin() && $GLOBALS['TYPO3_CONF_VARS']['EXT']['extCache']) {
		...
		else (is_array($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['additionalBackendItems']['cacheActions'])) {
			foreach($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['additionalBackendItems']['cacheActions'] as $cacheAction) {
				...
				else (!($hookObject instanceof backend_cacheActionsHook)) {
