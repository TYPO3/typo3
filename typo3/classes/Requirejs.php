...
class Typo3_Requirejs {
	...
	public function getConfigurationForAjaxRequest(array $params, TYPO3AJAX $ajaxObj) {
			// Set path to latest jQuery version
		$latestJqueryVersionNumber = (string)t3lib_div::_GP('latestJqueryVersionNumber');
		$GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['RequireJS']['jqueryLatest'] = 'contrib/jquery/jquery-' . $latestJqueryVersionNumber . '.min';

		$allConfiguration = $this->getConfiguration();

		$content = 'var requirejs = ' . json_encode($allConfiguration) . ';';
	...
	public function getConfiguration() {
		...
		$allConfiguration = array(
			...
			'paths' => $paths,
				// Seconds to wait until requirejs marks the request as a timeout
			'waitSeconds' => 15,
			'locale' => 'en-gb'
		);
