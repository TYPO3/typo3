...
If your TYPO3 installation is using Composer, install the extension using
Composer:
...
If you are using TYPO3 with Composer and are using a version of TYPO3 that is older than 11.4,
you will need to activate the extension:

- Access :guilabel:`Admin Tools > Extension Manager > Installed Extensions`
- Search for `impexp`
- Activate the extension by selecting the :guilabel:`Activate` button in the column labeled :guilabel:`A/D`
...
If you are working with an installation of TYPO3 that doesn't use Composer, the extension
:file:`impexp` will already be part of the installation due to the fact that "classic" `.tar` & `.zip` packages
come bundled with all system extensions.

However, whilst the extension is already downloaded, it is likely that the extension is not activated.

To activate the Import/Export extension, navigate to
...
Maintainer rights are required to activate the extension.
