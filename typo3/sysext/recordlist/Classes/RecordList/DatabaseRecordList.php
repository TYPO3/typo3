			...
			if(isset($GLOBALS['TCA'][$table]['columns'])) {
				foreach($GLOBALS['TCA'][$table]['columns'] as $field => $fieldSetup) {
					if(isset($fieldSetup['config']['itemsProcFunc']) && $fieldSetup['config']['itemsProcFunc']) {
						$GLOBALS['TCA'][$table]['columns'][$field]['config']['items'] = $tceForms->procItems($fieldSetup['config']['items'], $fieldSetup['config']['itemsProcFunc'], $fieldSetup['config'], $table, $row, $field);
					}
				}
			}
...
';
			...
			$rangeIndicator = '<span class="pageIndicator">' . sprintf($GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_mod_web_list.xml:rangeIndicator'), ($this->firstElementNumber + 1), $lastElementNumber) . '</span>';
