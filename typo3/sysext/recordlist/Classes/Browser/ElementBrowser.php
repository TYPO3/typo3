...
class ElementBrowser {
	...
	protected function file_generateSearchForm() {
		...
		$target = $this->thisScript . '?&mode=' . $this->mode . '&bparams=' . rawurlencode($this->bparams);
		$content .= '<form action="' . htmlspecialchars($target) . '" name="fileSearchForm" method="post">
				<label for="search">' . $GLOBALS['LANG']->getLL('file_filter_searchfor') . '</label><input type="search" id="search" name="search" value="' . htmlspecialchars($searchValue) . '">
				<div style="margin-top:5px;margin-bottom:5px"><input type="checkbox" class="checkbox" id="search_recursive" name="search_recursive" value="1" '. ($recursive ? 'checked="checked"' : '')
