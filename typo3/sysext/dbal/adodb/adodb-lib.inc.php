...
function _adodb_getinsertsql(&$zthis,&$rs,$arrFields,$magicq=false)
{
	...
	foreach( $columns as $field ) { 
		...
		if (adodb_key_exists($upperfname,$arrFields)) {
			...
			$fields .= $zthis->nameQuote.$field->name.$zthis->nameQuote . ", ";
	...
	switch($type) {
		...
		case 'B':
			if ($action == 'I') {
				...
				$sql .= $zthis->nameQuote.$fname.$zthis->nameQuote . "=" . $zthis->qstr($arrFields[$fname],$magicq) . ", ";
			}
		  break;
		...
		case "D":
			if ($action == 'I') {
				...
				$sql .= $zthis->nameQuote.$fname.$zthis->nameQuote . "=" . $zthis->DBDate($arrFields[$fname]) . ", ";
			}
			break;
		...
		case "T":
			if ($action == 'I') {
				...
				$sql .= $zthis->nameQuote.$fname.$zthis->nameQuote . "=" . $zthis->DBTimeStamp($arrFields[$fname]) . ", ";
			}
			break;
		...
		default:
			...
			if ($action == 'I') {
				...
				$sql .= $zthis->nameQuote.$fname.$zthis->nameQuote . "=" . $val  . ", ";
			}
			break;
	}
