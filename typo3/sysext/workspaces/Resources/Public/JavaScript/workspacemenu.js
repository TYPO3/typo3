...
var WorkspaceMenu = Class.create({
	...
	initialize: function() {
		...
		//		top.TYPO3.Backend.Topbar.insertHTML('<div id="typo3-topbar-workspaces-title">' + top.TYPO3.configuration.workspaceTitle + '</div>');
			} else {
	...
	switchWorkspace: function(event) {
		...
		TYPO3.Ajax.ExtDirect.ToolbarMenu.setWorkspace({'workSpaceId': workspaceId }, function(response) {
			...
			if(response.title) {
				top.TYPO3.configuration.workspaceTitle = response.title;
			}
