...
class Tx_Workspaces_Service_Stages {
	...
	/** raise every stage uid with this number to prevent any conflict with default stages */
	private $raiseStageIdAmount = 20;
	...
	public function getStagesForWSUser() {
		...
		$stagesForWSUser = array(
			'total' => count($stagesForWSUserData),
			'data' => $stagesForWSUserData,
		);
		return $stagesForWSUser;
	}
	...
	public function getNextStage($stageid) {
		...
		if (!t3lib_div::testInt($stageid)) {
			throw new InvalidArgumentException('StageId is supposed to be an integer');
		}
	...
	public function getPositionOfCurrentStage($stageId) {
		$stagesOfWS = $this->getStagesForWS();
			// calculate amount of stages, add 2 because of edit and ready to publish stage
		$countOfStages = count($stagesOfWS) + 2;
