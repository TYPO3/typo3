	...
	public function sendToNextStageWindow($table, $uid, $t3ver_oid) {
		...
		if(is_array($elementRecord)) {
			...
			$result = array(
				'error' => array(
					'code' => 1287264776,
					'message' => 'The record element could not be found.',
				),
				'success' => FALSE,
			);
		}
		...
		if(is_array($elementRecord)) {
			...
			if ($stageId !== Tx_Workspaces_Service_Stages::STAGE_EDIT_ID) {
				...
				$result = array(
					'error' => array(
						'code' => 1287264746,
						'message' => 'The record element is already in editing stage, there is no previous stage.',
					),
					'success' => FALSE,
				);
			}
		...
		$result = array(
			...
			'items' => array(
				array(
					...
					'html' => 'The selected element(s) will be sent to ' . $stageTitle,
				),
				array(
				...
				array(
					'fieldLabel' => 'Additional receipients',
					'name' => 'additional',
					'xtype' => 'textarea',
					'width' => 250,
				),
