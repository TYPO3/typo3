...
class logFunctions_ext extends t3lib_BEDisplayLog {
	...
	function initArray()	{
		global $LANG;
		$codeArr=Array();
		$codeArr[$i][]=$GLOBALS['LANG']->getLL('chLog_l_time');
		$codeArr[$i][]=$GLOBALS['LANG']->getLL('chLog_l_user');
		$codeArr[$i][]=$GLOBALS['LANG']->getLL('chLog_l_error');
		$codeArr[$i][]=$GLOBALS['LANG']->getLL('chLog_l_action');
		$codeArr[$i][]=$GLOBALS['LANG']->getLL('chLog_l_table');
		$codeArr[$i][]=$GLOBALS['LANG']->getLL('chLog_l_details');
		return $codeArr;
	}
}
...
class tx_belog_webinfo extends t3lib_extobjbase {
	...
	function main()	{
		...
		$theOutput='';
		$menu='';
		$menu.=  '&nbsp;'.$GLOBALS['LANG']->getLL('chLog_menuUsers').': '.t3lib_BEfunc::getFuncMenu($this->pObj->id,'SET[log_users]',$this->pObj->MOD_SETTINGS['log_users'],$this->pObj->MOD_MENU['log_users']);
		$menu.=  '&nbsp;'.$GLOBALS['LANG']->getLL('chLog_menuDepth').': '.t3lib_BEfunc::getFuncMenu($this->pObj->id,'SET[depth]',$this->pObj->MOD_SETTINGS['depth'],$this->pObj->MOD_MENU['depth']);
		$menu.=  '&nbsp;'.$GLOBALS['LANG']->getLL('chLog_menuTime').': '.t3lib_BEfunc::getFuncMenu($this->pObj->id,'SET[log_time]',$this->pObj->MOD_SETTINGS['log_time'],$this->pObj->MOD_MENU['log_time']);
		$theOutput.=$this->pObj->doc->section($GLOBALS['LANG']->getLL('chLog_title'),'<span class="nobr">'.$menu.'</span>',0,1);
