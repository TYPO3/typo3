...
class Tx_Belog_ViewHelpers_WorkspaceTitleViewHelper extends Tx_Fluid_Core_ViewHelper_AbstractViewHelper {
	...
	public function render($uid) {
		...
		} else if (t3lib_extMgm::isLoaded('workspaces')) {
			/** @var $workspaceObject Tx_Belog_Domain_Model_Workspace */
			$workspaceObject = $this->workspaceRepository->findByUid($uid);

			if ($workspaceObject instanceof Tx_Belog_Domain_Model_Workspace) {
				$title =  $workspaceObject->getTitle();
			}
		}
		return $title;
	}
