...
class Tx_Fluid_Core_AbstractViewHelperTest_testcase extends Tx_Extbase_Base_testcase {
	/**
	 * @test
	 * @author Sebastian Kurfürst <sebastian@typo3.org>
	 */
	public function test_argumentsCanBeRegistered() {
		$viewHelper = $this->getMock($this->buildAccessibleProxy('Tx_Fluid_Core_AbstractViewHelper'), array('render'), array(), '', FALSE);
		...
		$type = "string";
		$isRequired = TRUE;
		$expected = new Tx_Fluid_Core_ArgumentDefinition($name, $type, $description, $isRequired);
	...
	public function test_prepareArgumentsRegistersAnnotationBasedArguments() {
		...
		$expected = array(
			'param1' => new Tx_Fluid_Core_ArgumentDefinition('param1', 'integer', 'P1 Stuff', TRUE, null, TRUE),
			'param2' => new Tx_Fluid_Core_ArgumentDefinition('param2', 'array', 'P2 Stuff', TRUE, null, TRUE),
			'param3' => new Tx_Fluid_Core_ArgumentDefinition('param3', 'string', 'P3 Stuff', FALSE, 'default', TRUE),
		);
	...
	public function test_validateArgumentsCallsTheRightValidators() {
		...
		$viewHelper->expects($this->once())->method('prepareArguments')->will($this->returnValue(array(
			'test' => new Tx_Fluid_Core_ArgumentDefinition("test", "string", FALSE, "documentation")
		)));
