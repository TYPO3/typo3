...
class SC_mod_web_perm_index {
	...
	public function doEdit() {
		...
		$code='
			...
			'<input type="submit" value="' . $GLOBALS['LANG']->getLL('Abort', 1) . '" onclick="' . htmlspecialchars('jumpToUrl(\'' . t3lib_BEfunc::getModuleUrl('web_list') . '&id=' . $this->id . '\'); return false;') . '" />
			<input type="hidden" name="redirect" value="' . t3lib_BEfunc::getModuleUrl('web_list') . 'mode=' . $this->MOD_SETTINGS['mode'] .
	...
	public function notEdit() {
		...
		foreach ($tree->tree as $data) {
			...
			if ($editPermsAllowed && $pageId) {
				$aHref = t3lib_BEfunc::getModuleUrl('web_list') . '&mode='.$this->MOD_SETTINGS['mode'].'&depth='.$this->MOD_SETTINGS['depth'].'&id='.($data['row']['_ORIG_uid'] ? $data['row']['_ORIG_uid'] : $pageId).'&return_id='.$this->id.'&edit=1';
				$cells[]='
