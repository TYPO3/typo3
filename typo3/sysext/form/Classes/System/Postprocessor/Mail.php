		...
		foreach ($elements as $element) {
			...
			if (is_a($element, 'tx_form_Domain_Model_Element_Fileupload')) {
				...
				if (is_array($submittedValues[$elementName]) && isset($submittedValues[$elementName]['tempFilename'])) {
					...
					if (is_file($filename) && t3lib_div::isAllowedAbsPath($filename) && t3lib_div::verifyFilenameAgainstDenyPattern($filename)) {
						$this->mailMessage->attach(
							Swift_Attachment::fromPath($submittedValues[$elementName]['tempFilename'])->setFilename($submittedValues[$elementName]['originalFilename'])
						);
					}
				}
			}
