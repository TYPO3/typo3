...
final class UploadedResourceViewHelperTest extends FunctionalTestCase
{
    ...
    public function resourcePointersAreRenderedForStorageWithoutInternalPositions(): void
    {
        ...
        /** @var ObjectStorage<FileReference> $storage */
        $storage = new ObjectStorage();
        // Mimics a restored payload in the format of TYPO3 v14: its spl_object_hash() based keys do
        // not belong to the objects of the current process, so no internal positions are recovered.
        (new \ReflectionProperty(ObjectStorage::class, 'storage'))->setValue($storage, [
