 ...
 * remove API
 *  - $conf['line']
 *  - previewBorder > thickness
 *  - previewBorder komplett
 *  - forceNoPopup
 *
 * remove
 *  - edit_editFormsOnPage
 *  - edit_editNoPopup
 *
 *
 * @see AdminPanelView -> ext_makeToolBar
 *
 */
...
class FrontendEditPanel {
	...
	public function editPanel($content, array $conf, $currentRecord = '', array $dataArr = array(), $table = '', $allow = '', $newUID = 0, array $hiddenFields = array()) {
		...
		foreach ((array)$hiddenFields as $name => $value) {
			$hiddenFieldString .= '<input type="hidden" name="TSFE_EDIT[' . $name . ']" value="' . htmlspecialchars ($value) . '"/>' . LF;
		}
...
//		if ($this->isDisabled($table, $dataArr)) {
//			$hiddenConfiguration = (is_array($conf['hidden.']) ? $conf['hidden.'] : array('wrap' => '<div class="typo3-editPanel-preview">|</div>'));
//			$finalOut = $this->cObj->stdWrap($finalOut, $hiddenConfiguration);
//		}

		$hidden = $this->isDisabled ($table, $dataArr) ? ' ce-hidden' : '';
		$outerWrapConfig = (is_array ($conf['wrap']) ? $conf['wrap'] : array('wrap' => '<div class="typo3-feedit-ce' . $hidden . '">|</div>'));
	 ...
	 * @param array|string $currentRecord The "table:uid" of the record being processed by the panel.
	...
	/**
	 * Returns TRUE if the input table/row would be hidden in the frontend, according the current time and simulate user group
	 *
