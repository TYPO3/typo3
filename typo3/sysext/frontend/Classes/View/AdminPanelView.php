...
class AdminPanelView {
	...
	public function isAdminModuleEnabled($key) {
		...
		if ($key === 'preview' && $this->ext_forcePreview) {
			$result = TRUE;
		} elseif ($GLOBALS['BE_USER']->extAdminConfig['enable.']['all']) {
			$result = TRUE;
		} elseif ($GLOBALS['BE_USER']->extAdminConfig['enable.'][$key]) {
			$result = TRUE;
		}
		return $result;
	}
	...
	/**
	 * Returns the value for a Admin Panel setting.
	 *
...
';
	...
	protected function getPreviewModule() {
		...
		if ($GLOBALS['BE_USER']->uc['TSFE_adminConfig']['display_preview']) {
			...
			if ($options) {
				$options = $options = '<option value="0">&nbsp;</option>' . $options;
	...
	protected function getInfoModule() {
		...
		if ($GLOBALS['BE_USER']->uc['TSFE_adminConfig']['display_info']) {
			...
			$tableArr[] = array('Total parsetime:', $GLOBALS['TSFE']->scriptParseTime . ' ms', TRUE);
			$table = '';
			foreach ($tableArr as $key => $arr) {
				$label = (isset($arr[2]) ? '<strong>' . htmlspecialchars ($arr[0]) . '</strong>' : htmlspecialchars ($arr[0]));
				$value = strlen ($arr[1]) ? $arr[1] : '';
				$table .=
					'<tr class="typo3-adminPanel-itemRow ' . ($key % 2 == 0 ? 'line-odd' : 'line-even') . '">
	...
	public function ext_makeToolBar() {
		...
		if ($GLOBALS['BE_USER']->check ('modules', 'web_list')) {
			...
			$toolBar .= '<a href="' . htmlspecialchars ((TYPO3_mainDir . \TYPO3\CMS\Backend\Utility\BackendUtility::getModuleUrl ('web_list', $urlParams))) . '">' . $icon . '</a>';
		}
