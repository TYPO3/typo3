...
class PageRepository {
			...
			if (count($fieldArr)) {
				...
				if ($noSpecialLanguageRequested) {
					$languageList = array_unique(array_merge(
						array($lUid, $GLOBALS['TSFE']->sys_language_content),
						$GLOBALS['TSFE']->sys_language_fallbackList
					));
				...
				if ($noSpecialLanguageRequested) {
					...
					foreach ($languageList as $languageId) {
						if ($firstRow['sys_lang_uid'] == $languageId) {
							$row = $firstRow;
							break;

						} elseif ($secondRow['sys_lang_uid'] == $languageId) {
							$row = $secondRow;
							break;
						}
					}
	 ...
	 */
	public function getRecordOverlayWithFallback($table, $row, $sys_language_content = NULL, $OLmode = NULL) {
		$sys_language_content = $sys_language_content !== NULL ? $sys_language_content : $GLOBALS['TSFE']->sys_language_content;
		$OLmode = $OLmode !== NULL ? $OLmode: $GLOBALS['TSFE']->sys_language_contentOL;
		$fallback = count($GLOBALS['TSFE']->sys_language_fallbackList);
		...
		if (!is_array($record) || !isset($record['_LOCALIZED_UID'])) {
			if ($fallback && $sys_language_content && $OLmode !== 'hideNonTranslated') {
				foreach ($GLOBALS['TSFE']->sys_language_fallbackList as $fallbackId) {
	 ...
	 * @return mixed Returns the input record, possibly overlaid with a translation. But if $OLmode is "hideNonTranslated" then it will return FALSE if no translation is found.
	 */
	protected function getRecordOverlayWithoutFallback($table, $row, $sys_language_content, $OLmode) {
