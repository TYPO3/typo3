				...
				switch ($confData['type']) {
					...
					case 'submit':
						...
						if ($image) {
							...
							$fieldCode = sprintf('<input type="submit" name="%s"%s value="%s"%s />', $confData['fieldname'], $elementIdAttribute, htmlspecialchars($value, ENT_COMPAT | ENT_HTML401, 'UTF-8', FALSE), $addParams);
						}
						break;
					case 'reset':
						$value = trim($parts[2]);
						$fieldCode = sprintf('<input type="reset" name="%s"%s value="%s"%s />', $confData['fieldname'], $elementIdAttribute, htmlspecialchars($value, ENT_COMPAT | ENT_HTML401, 'UTF-8', FALSE), $addParams);
						break;
					case 'label':
