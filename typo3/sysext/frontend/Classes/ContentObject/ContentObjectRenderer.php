...
/', $conf['doubleBrTag'], $content);
	...
	protected function resolveMixedLinkParameter($linkText, $mixedLinkParameter, &$configuration = array()) {
		...
		$linkParameterParts = \TYPO3\CMS\Core\Utility\GeneralUtility::unQuoteFilenames($mixedLinkParameter, TRUE);
		...
		if (isset($linkParameterParts[4]) && strlen(trim($linkParameterParts[4])) > 0) {
	...
	public function typoLink($linktxt, $conf) {
		...
		if (!is_array($resolvedLinkParameters)) {
			...
			$linkParameter = $resolvedLinkParameters[0];
			$linkTarget = $forceTarget = $resolvedLinkParameters[1];
			$linkClass = $resolvedLinkParameters[2];
			$linkTitle = $forceTitle = $resolvedLinkParameters[3];
		}
		...
		if ($linkParameter) {
			...
			switch ($linkType) {
					...
					if (file_exists(rawurldecode($splitLinkParam[0])) || strpos($linkParameter, '/') !== FALSE) {
						...
						if ($GLOBALS['TSFE']->config['config']['jumpurl_enable'] || $conf['jumpurl']) {
							...
							if ($conf['jumpurl.']['secure']) {
								...
								$url .= '&juHash=' . \TYPO3\CMS\Core\Utility\GeneralUtility::hmac($linkParameter, 'jumpurl');
							}
				...
				case 'page':
					...
					$pairParts = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $linkParameter, TRUE);
