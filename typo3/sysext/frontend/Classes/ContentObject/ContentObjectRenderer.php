...
class ContentObjectRenderer {
	...
	public function cImage($file, $conf) {
		...
		if (is_array($info)) {
			...
			$imageTagTemplate = $this->getImageTagTemplate($layoutKey, $conf['layout.']);
	 ...
	 *
	 * @param arrac $conf TypoScript configuration properties
	 * @return string
	 *
	 */
	public function getImageTagTemplate($layoutKey, $layoutConfigurations) {
		if ($layoutKey && $layoutConfigurations[$layoutKey . '.']) {
			$imageTagLayout = $this->stdWrap($layoutConfigurations[$layoutKey . '.']['element'], $layoutConfigurations[$layoutKey . '.']['element.']);
		} else {
	...
	public function getImageSourceCollection($layoutKey, $conf, $file) {
		...
		if ($layoutKey && $conf['sourceCollection.'] && ($conf['layout.'][$layoutKey . '.']['source'] || $conf['layout.'][$layoutKey . '.']['source.'])) {
			...
			foreach ($activeSourceCollections as $key => $sourceConfiguration) {
				...
				$alternateSources .= $this->substituteMarkerArray($sourceLayout, $sourceConfiguration, '###|###', TRUE, TRUE);
			}
		}
