...
';
	...
	public function getTreeList($id, $depth, $begin = 0, $dontCheckEnableFields = FALSE, $addSelectFields = '', $moreWhereClauses = '', array $prevId_array = array(), $recursionLevel = 0) {
		...
		if ($id) {
			...
			// Select sublevel:
			if ($depth > 0) {
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows($allFields, 'pages', 'pid = ' . intval($id) . ' AND deleted = 0 ' . $moreWhereClauses, '', 'sorting');
								...
								if (!in_array($next_id, $prevId_array)) {
									$theList = array_merge(
										\TYPO3\CMS\Core\Utility\GeneralUtility::intExplode(
											',',
											self::getTreeList($next_id, $depth - 1, $begin - 1, $dontCheckEnableFields, $addSelectFields, $moreWhereClauses, $prevId_array, $recursionLevel + 1)
										),
										$theList
									);
								}
	...
	public function getQuery($table, $conf, $returnQueryArray = FALSE) {
		...
		if (isset($conf['recursive'])) {
			...
			if ($conf['recursive'] > 0) {
				$pidList = \TYPO3\CMS\Core\Utility\GeneralUtility::intExplode(',', $conf['pidInList']);
				...
				foreach ($pidList as $value) {
					// implementation of getTreeList allows to pass the id negative to include it into the result
					// otherwise only childpages are returned
					$pidList = array_merge(
						\TYPO3\CMS\Core\Utility\GeneralUtility::intExplode(',',$this->getTreeList($value, $conf['recursive'])),
						$pidList
					);
				}
				$conf['pidInList'] = \TYPO3\CMS\Core\Utility\GeneralUtility::intExplode(',',$pidList);
			}
		}
