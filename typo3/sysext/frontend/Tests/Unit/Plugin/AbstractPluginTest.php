...
class AbstractPluginTest extends \TYPO3\CMS\Core\Tests\UnitTestCase {
	...
	public function piSetPiVarDefaultsStdWrap($input, $expected) {
		...
		$abstractPlugin->cObj = new \TYPO3\CMS\Frontend\ContentObject\ContentObjectRenderer();
		$abstractPlugin->conf['_DEFAULT_PI_VARS.'] = $input;
		$abstractPlugin->pi_setPiVarDefaults();
		$this->assertEquals($expected, $abstractPlugin->piVars);
	}
