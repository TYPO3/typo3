...
class ContentObjectRendererTest extends \TYPO3\CMS\Core\Tests\UnitTestCase {
	...
	public function setUp() {
		...
		$this->tsfe = $this->getAccessibleMock('TYPO3\\CMS\\Frontend\\Controller\\TypoScriptFrontendController', array('dummy'));
	...
	/**
	 * Checks if stdWrap.cropHTML works with a complex content with many expectedTags. Currently cropHTML
...
';
	...
	public function getQueryDataProvider() {
		$data = array(
			'testing empty configuration' => array(
				'tt_content',
				array(),
				array(
					'SELECT' => '*'
	...
	public function stdWrap_addPageCacheTagsAddsPageTags(array $expectedTags, array $configuration){
		...
		$this->assertAttributeEquals($expectedTags, 'pageCacheTags', $this->tsfe);
	}
