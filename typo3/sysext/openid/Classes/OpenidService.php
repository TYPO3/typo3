...
class OpenidService extends \TYPO3\CMS\Core\Service\AbstractService {
				...
				if ($openIDIdentifier) {
					...
					if ($userRecord != NULL) {
						...
						if (GeneralUtility::_GP('tx_openid_prefill')) {
							...
							setcookie('typo3-login-openid-' . strtolower(TYPO3_MODE), 'deleted', 1, '/');
						}
					} else {
	...
	protected function getReturnURL() {
		if ($this->authenticationInformation['loginType'] === 'FE') {
			...
			$returnURL = 'index.php?eID=tx_openid&' . 'pid=' . $this->authenticationInformation['db_user']['checkPidList'] . '&' . 'logintype=login&tx_openid_prefill=' . ((bool)GeneralUtility::_GP('tx_openid_prefill')) . '&';
		} else {
			...
			$returnURL = $absoluteSiteURL . TYPO3_mainDir . 'sysext/' . $this->extKey . '/class.tx_openid_return.php?login_status=login&tx_openid_prefill=' . ((bool)GeneralUtility::_GP('tx_openid_prefill')) . '&';
		}
