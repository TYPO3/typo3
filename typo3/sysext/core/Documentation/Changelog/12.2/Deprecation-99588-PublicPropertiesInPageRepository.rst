...
One of TYPO3's main classes for fetching page records and content in TYPO3 Frontend is
...
For this reason, the following public properties are marked as deprecated:
...
Setting or reading these properties via PHP code in custom extensions will trigger a PHP deprecation notice, however they still continue to work in TYPO3 v12.


Affected installations
======================

TYPO3 installations with custom extensions making use of these properties, which is highly
unlikely.
...
Contexts (e.g. to show hidden records, or to use other Workspace Constraints), as this is already
done in TYPO3 Core since various versions.

When needing to build queries with the common restrictions, it is recommended to use
the API methods of this class, where most of the methods already have a `$disableGroupAccessCheck` argument, or `enableFields()` which allows to return
