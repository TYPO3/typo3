...
Instances with extensions that add Backend modules using the bootstrap
related tooltips plugin may be affected. A typical sign for this is
using the :html:`data-bs-toggle="tooltip"` attribute on elements, loading
...
Some parts of the core will fallback to the :html:`title` attribute for now. However,
