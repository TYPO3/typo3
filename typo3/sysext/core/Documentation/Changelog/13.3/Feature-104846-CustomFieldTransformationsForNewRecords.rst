...
With :issue:`103783` the new :php:`Record` object has been introduced, which
is an object representing a raw database record based on TCA and is usually
used in the frontend (via Fluid Templates).

Since :ref:`feature-103581-1723209131` the properties of those :php:`Record`
objects are transformed / expanded from their raw database value into
"rich-flavored" values. Those values might be relations to e.g. :php:`Record`,
:php:`FileReference`, :php:`Folder` or :php:`\DateTimeImmutable` objects.
...
introduced. It's dispatched right before a :php:`Record` is created and
therefore allows to fully manipulate any property, even the ones already
transformed by TYPO3.

The new event is stoppable (implementing :php:`StoppableEventInterface`), which
allows listeners to actually create a :php:`Record` object completely on their
own.

.. important::

    The event operates on the :php:`RecordInterface` instead of an actual
...
* :php:`setRecord()` - Manually adds a :php:`RecordInterface` object (stops the event propagation)
* :php:`hasProperty()` - Whether a property exists
* :php:`setProperty()` - Add or overwrite a property
* :php:`setProperties()` - Set properties for the :php:`RecordInterface`
...
Using the new PSR-14 :php:`RecordCreationEvent`, extension authors are able to
apply any field transformation to any property before a :php:`Record` is created.
It's even possible to completely create a new :php:`RecordInterface` object on
their own.
