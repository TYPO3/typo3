...
However, with TYPO3 v9, Site Handling was introduced, which produces absolute
URLs or absolute paths directly. In addition, with TYPO3 v12.1 the option
:typoscript:`config.forceAbsoluteUrls = 1` allows to generate absolute URLs
completely for all links, images or assets, making the baseURL option obsolete,
as it isn't as powerful as the mentioned alternatives:
...
TYPO3 installations using the `config.baseURL` option, which is common for
projects which were started before TYPO3 v9.


Migration
=========

Use Site Configuration with fully qualified domain names to achieve the same
result, as rendering a :html:`<base>` tag in HTML will not be supported
out-of-the-box anymore by TYPO3 v13.

If you are already using Site Configuration, but need to build fully
qualified URLs, you can safely remove the TypoScript option
...
In general it is recommended to not use the :html:`<base>` tag as
