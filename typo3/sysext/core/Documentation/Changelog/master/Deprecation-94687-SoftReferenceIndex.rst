...
had its own method residing in one class. It was already possible to define
a dedicated class for each parser. This has been done now. As a result
:php:`SoftReferenceIndex` is not needed anymore and has been therefore marked
as deprecated.

The related method :php:`BackendUtility::softRefParserObj` was used to get
the according soft reference parser object and was basically a factory method.
This logic has been moved into
:php:`TYPO3\CMS\Core\DataHandling\SoftReference\SoftReferenceParserFactory`.
:php:`BackendUtility::softRefParserObj` has been marked as internal in v11
already. To ease migration, the old static method is still in place and logs
a deprecation error, when called.

Another tightly coupled method :php:`BackendUtility::explodeSoftRefParserList`,
which was used to parse the comma separated list of soft refeference parsers
and return them as an array, has now also been deprecated. It was mostly used
...
Not doing so will log a deprecation error. In v12 this will throw an exception.

Impact
======

The following classes are marked as deprecated. Instantiating this classes will
log a deprecation error.

*  :php:`TYPO3\CMS\Core\Database\SoftReferenceIndex`

The following methods are marked as deprecated. Calling these methods will log
a deprecation error.
...
:php:`TYPO3\CMS\Core\DataHandling\SoftReference\SoftReferenceParserInterface`.
Otherwise a deprecation error will be logged and an exception will be thrown
in v12.
...
Among other methods
:php:`TYPO3\CMS\Core\DataHandling\SoftReference\SoftReferenceParserInterface`
ensures the method :php:`parse` is implemented. The previous used method name
...
:php:`$parserList` as first argument, same as in the `BackendUtility`
