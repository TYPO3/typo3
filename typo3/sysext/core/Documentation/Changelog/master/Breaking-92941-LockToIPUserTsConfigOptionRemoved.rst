...
The UserTsConfig setting `options.lockToIP` which allowed Backend
...
could be disabled as system-wide setting where the UserTsConfig setting wa
never evaluated anymore.

The global toggle was also removed, as it did not serve any other purposes.

Sidenote: From a TYPO3-internal request workflow this feature was never part of
...
When using the UserTsConfig setting `options.lockToIP` is set, it will not be
evaluated anymore.
