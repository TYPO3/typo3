...
* However, when a record is moved to another page in a workspace, the PID changes.
  ...
  Therefore :php:`fixVersioningPid()` is now deprecated.
...
the workspace, where as the "_ORIG_pid" contains the live records' pid value.

If a moved record has been overlaid, the "_ORIG_pid" and "pid" field values
are now switched.
