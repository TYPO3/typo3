...
Therefore, the main difference is that the user authentication objects such as
...
The array `AbstractUserAuthentication->user` previously contained the logged-in
user record (from be_users / fe_users database table) AND the session record
prefixed via `ses_*` array properties. This has been removed, to separate
the functionality. Instead, all session properties are placed inside the
`UserSession` object, accessible via e.g. `$GLOBALS[BE_USER]->getSession()`.
...
The property `newSessionID` is now available in :php:`UserSession->isNew()`.

Use the :php:`UserSessionManager->elevateToFixatedUserSession()` as a
replacement for :php:`getNewSessionRecord()` to migrate an anoymous session
...
database queries using this field, change it to use use `ses_userid` instead.
