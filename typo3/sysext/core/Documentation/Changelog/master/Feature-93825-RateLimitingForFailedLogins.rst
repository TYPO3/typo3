...
.. important::
   ...
   (e.g. login or form submission) of an application.
...
The rate limiter `LoginRateLimiter` for the TYPO3 backend login is enabled by
...
When the maximum amount of login attempts has exceeded, a `RequestRateLimitedException`
exception is thrown. The exception implements `AbstractClientErrorException`
...
When the maximum amount of login attempts is exceeded, a `RequestRateLimitedException`
exception is thrown. The exception implements `AbstractClientErrorException`
resulting in a user readable error message together with a 403 HTTP status code.
...
.. code-block:: typoscript
   ...
   [FE][loginRateLimit] = 10;
   [FE][loginRateLimitInterval] = '15 minutes';
   [FE][loginRateLimitIpExcludeList] = '';
