...
The signature of method :php:`TYPO3\\CMS\\Extbase\\Mvc\\Controller\\ControllerInterface::processRequest` changed in the regard that no longer :php:`$request` and :php:`$response` are passed into it. Instead, only a :php:`$request` argument is needed. Additionally, that method now needs you to return a response.
