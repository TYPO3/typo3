...
2. TYPO3 allows to use record in non-default language as a translation source. In this case the information about the translation source was lost.
  Now, the `translationSource` field always contains an uid of the record used as a translation source.

3. In some places `origUid` (e.g. `t3_origuid`) field was misused as a translation source. Now  these places can be refactored to use the `translationSource` field.
