...
:php:`JavaScriptModuleInstruction` declarations. In :php:`FormEngine`,
loading RequireJS module via arrays is deprecated and has to be migrated
as well.
...
Using :php:`$resultArray['requireJsModules']` with scalar `string` values will
...
RequireJS modules via :php:`$resultArray['requireJsModules']`.
...
custom module implementations, e.g. triggered by corresponding on-ready handlers.
