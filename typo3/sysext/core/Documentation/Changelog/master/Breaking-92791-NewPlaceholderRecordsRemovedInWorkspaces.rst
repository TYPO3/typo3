...
`shadowColumnsForNewPlaceholders` TCA option, which wasn't kept in sync when
...
database via the TYPO3 Database via API classes such as `PageRepository` or
`BackendUtility`.
...
`shadowColumnsForNewPlaceholders` obsolete.

Using methods like `getWorkspaceVersionOfRecord` on a new versioned record
...
newly created in a workspace do not carry the `ORIG_uid` information anymore
...
or writing based on `t3ver_state` database fields, which is very uncommon.
...
It is highly recommend to use the TYPO3 API methods within Extbase, PageRepository
and BackendUtility to ensure records are resolved properly.

At any times, it is recommended to use the `WorkspaceRestriction` of TYPO3's
