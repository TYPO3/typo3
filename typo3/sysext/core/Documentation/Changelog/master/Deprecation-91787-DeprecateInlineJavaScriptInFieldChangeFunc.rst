...
As a result, `fieldChangeFunc` declarations are not using plain inline
JavaScript (as scalar `string`) anymore, but make use of corresponding objects
...
Using `fieldChangeFunc` with scalar `string` values is deprecated and has to
...
Using `fieldChangeFunc` with scalar `string` values will trigger a
