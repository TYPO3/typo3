...
Therefore, the module displays all relevant global variables such as
...
`lowlevel.configuration.module.provider` tag. Moreover arbitrary attributes
...
Therefore a full implementation would look like this:
...
which already implements the required methods expect :php:`getConfiguration`.
...
To disable an already registered configuration simply add the `disabled: true`
