...
To ease the use of this, the :php:`ShortcutButton` is extended for
...
module. Therefore, the buttons' icon is also changed. On click, a dropdown
opens, including the additional possibility to copy the current backend URL
directly to the operating systems' clipboard, next to the already existing
bookmark option.

For the dropdown button, a new icon `share-alt` is registered, which can
...
.. note::
   ...
   Since both view helpers, :html:`<be:moduleLayout.button.shortcutButton>`
   ...
   view helpers, but still want to profit from the new option in your custom
...
for most of the "buildUri" methods, e.g. :php:`UriBuilder->buildUriFromRoute()`.

.. code-block:: php

   $uri = $uriBuilder->buildUriFromRoute($routeName, $arguments, UriBuilder::SHAREABLE_URL)

Above exmaple will return an absolute URL without the automatically
...
* Option to copy the URL of the current page to the operating systems' clipboard
