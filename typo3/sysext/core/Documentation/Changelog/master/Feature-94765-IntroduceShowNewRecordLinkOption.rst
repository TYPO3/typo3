...
TCA `inline` elements, without simultaneously also disabling either the
...
Furthermore does the :php:`['appearance']['levelLinksPosition']` option
not longer support `none` as value. This option should only be used to
position the level links, not to hide them. This can therefore be
achieved, setting the corresponding link specific options
...
It's now possible to disable the "New record" link of TCA `inline` elements
