...
the internals is a requirement to build plugins. The best way to debug plugin
...
The Inspector can be enabled in two different ways:
* By having :php:`$GLOBALS['TYPO3_CONF_VARS']['BE']['debug']` enabled and being in the `Development` context
* By setting the option :yaml:`editor.config.debug` to :yaml:`true` in your CKEditor configuration
...
By being in the right context or enabling the given option, it's now possible to easier debug CKEditor instances for plugin development.
