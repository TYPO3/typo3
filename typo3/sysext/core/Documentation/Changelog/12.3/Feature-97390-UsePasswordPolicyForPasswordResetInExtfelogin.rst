...
The password reset feature for TYPO3 frontend users does now consider the
...
if the feature toggle `security.usePasswordPolicyForFrontendUsers` is set
to `false`.

TYPO3 websites, which have set the feature toggle
`security.usePasswordPolicyForFrontendUsers` to `true`, will use the globally
configured password policy when a TYPO3 frontend user resets the password.
