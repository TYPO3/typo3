...
is added, providing the page type number as additional value. The main use-case
...
implementing an event listener for :ref:`\TYPO3\CMS\Redirects\Event\SlugRedirectChangeItemCreatedEvent <feature-99746-1675059434>`.
...
- :php:`getTargetLinkParameters()`: Returns the link parameters which should be used to create the target based on t3-syntax
...
The new :php:`PageTypeSource` can be used to provide additional source variants.
For example, based on custom page types using full URI building, which would take
configured PageTypeSuffix decorators into account. For page type 0 (default), Core
