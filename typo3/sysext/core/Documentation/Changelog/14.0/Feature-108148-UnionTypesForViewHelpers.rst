...
than one type should be possible. Now it is possible to specify multiple types
...
..  code-block:: php
    ...
    class MyViewHelper extends AbstractViewHelper
    {
        ...
        {
            $this->registerArgument('file', File::class . '|' . FileReference::class, 'a file object');
            $this->registerArgument('items', 'iterable', 'a list of items');
        }
    }
...
Note that union types disable automatic type conversion by Fluid, so it might
be necessary to specify more types to keep ViewHelpers flexible. Example:

..  code-block:: php

    $this->registerArgument('ids', 'array|string|int', 'a list of ids, either comma-separated or as array');
