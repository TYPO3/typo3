...
future internal use by Fluid itself, similarly to the already existing `{_all}`.
...
`<f:for> <https://docs.typo3.org/permalink/t3viewhelper:typo3fluid-fluid-for>`_ or
`<f:render> <https://docs.typo3.org/permalink/t3viewhelper:typo3-fluid-render>`_.
It also affects Fluid's PHP APIs, namely :php:`$view->assign()` and :php:`$view->assignMultiple()`.
...
Note that this only affects variable names, but not property names in objects
or array keys that are accessed within a Fluid template. The following examples
