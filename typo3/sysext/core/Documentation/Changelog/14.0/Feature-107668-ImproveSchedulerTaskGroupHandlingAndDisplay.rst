...
A new :sql:`color` field has been added to the
:sql:`tx_scheduler_task_group` table, allowing administrators to assign hex
...
The color field is implemented as a TCA field of type :php:`'color'` with the
following characteristics:

*   Stores standard hex color values (for example :php:`#FF8700`).
...
:php:`$GLOBALS['TCA']['tx_scheduler_task_group']['columns']['color']['config']['valuePicker']['items']`
array in your TCA overrides file.

..  code-block:: php
    :caption: Add a custom color option in a TCA override file
