...
However, there are many usage cases where :php:`datetime` TCA fields should have
a much broader time span, or at least past 2038.

Now, all these fields are changed to use the :sql:`bigint unsigned` data type.
...
remove the column definition from :file:`ext_tables.sql` and utilize with the
type :sql:`bigint unsigned`. This will allow to store timestamps after
2038.

In the rare case that your extension needed an even broader, non-negative
range (:sql:`bigint signed`), you need to keep a custom column definition.
...
and are now defined just like :sql:`starttime`, :sql:`endtime`, :sql:`tstamp`,
:sql:`crdate`:
