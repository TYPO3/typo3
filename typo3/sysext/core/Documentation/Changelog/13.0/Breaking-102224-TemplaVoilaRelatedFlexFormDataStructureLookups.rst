...
The following :php:`TCA` config options for :php:`'type' = 'flex'` column fields are
not handled anymore:
...
instances based on "TemplaVoila". This detail never found broader acceptance in
...
lookup logic in class :php:`FlexFormTools`. Those can be used to re-implement
the logic that has been removed from TYPO3 core if needed.
