...
The legacy entrypoint `/typo3/index.php` is no longer needed and deprecated in
favor of handling all backend and frontend requests with `/index.php`. The
entrypoint is still in place in case webserver configuration has not been adapted
yet and the maintenance and emergency tool is still available via
...
legacy entrypoint for that can be defined in composer.json:
