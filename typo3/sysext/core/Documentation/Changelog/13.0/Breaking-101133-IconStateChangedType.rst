...
:php:`TYPO3\CMS\Core\Imaging\Icon->state` is now a native enum
:php:`\TYPO3\CMS\Core\Imaging\IconState` instead of an instance of
...
.. code-block:: php
    ...
    $icon->setState(\TYPO3\CMS\Core\Type\Icon\IconState::cast(\TYPO3\CMS\Core\Type\Icon\IconState::STATE_DEFAULT));
    $state = $icon->getState();
    $stateValue = (string)$state;

    // After
    $icon = GeneralUtility::makeInstance(Icon::class);
    $icon->setState(\TYPO3\CMS\Core\Imaging\IconState::STATE_DEFAULT);
