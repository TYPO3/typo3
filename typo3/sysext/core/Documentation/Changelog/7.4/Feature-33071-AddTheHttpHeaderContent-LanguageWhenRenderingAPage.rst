...
By default a header "Content-language: XX" is sent where "XX" is the iso code of the sys_language_content (in the sys_language record, it is represented by the sys_language_isocode field) if that is properly defined by the sys_language record representing the sys_language_uid.
