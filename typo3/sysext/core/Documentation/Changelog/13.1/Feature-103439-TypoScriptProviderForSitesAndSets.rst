...
(root-flag) and does not inherit from parent sites (e.g. sites up in the
rootline). That means it behaves as if the clear-flag is set in a `sys_template`
record. This behavior is not configurable by design, as TypoScript code sharing
is intended to be implemented via sharable sets (introduced in :issue:`#103437`).

Note that `sys_template` records will still be loaded, but they are optional now,
and applied after TypoScript provided by the site.
...
Note that `@import` statements are still fine to be used for local includes, but
...
Sites and sets can ship TypoScript without the need for sys_template records in
database and dependencies can expressed via sets, allowing for automatic
