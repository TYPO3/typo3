...
class DatabaseConnection {
	 ...
	 * @var array<string>
	 *
	 * NO_BACKSLASH_ESCAPES This mode disables the use of the backslash character as an escape character, but we depend
	 *                      on this for escaping.
	 * STRICT_ALL_TABLES    If a value is not valid for column mysql will throw an error in this mode, but we depend on
	 *                      convert to a valid value.
	 */
	protected $incompatibleSqlModes = array('NO_BACKSLASH_ESCAPES','STRICT_ALL_TABLES');
	...
	protected function setSqlModes() {
		...
		if (count($detectedIncompatibleSqlModes) > 0) {
			...
			/**
			 * "SET sql_mode='';" can issued by any user for his own session.
			 * @see http://dev.mysql.com/doc/refman/5.6/en/server-sql-mode.html
			 **/
			$this->sql_query($query);
			\TYPO3\CMS\Core\Utility\GeneralUtility::sysLog(
				sprintf(
					'There is an incompatible SQL mode (%s) that has been removed for the MySQL Session. '
					. 'You should remove that setting in your MySQL environment or in $TYPO3_CONF_VARS["SYS"]["setDBinit"].',
