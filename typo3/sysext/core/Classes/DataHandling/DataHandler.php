...
class DataHandler {
				...
				foreach ($this->cmdmap[$table] as $id => $incomingCmdArray) {
					if (is_array($incomingCmdArray)) {
						...
						if (!$commandIsProcessed) {
							...
							if (is_array($cmdData) && $cmdData['update']) {
	 ...
	 *
	 * @param boolean $stripslashes_values If TRUE, incoming values in the data-array have their slashes stripped.
	 * @param boolean $dontProcessTransformations If set, then transformations are NOT performed on the input.
	 * @return \TYPO3\CMS\Core\DataHandling\DataHandler
	 * @todo Define visibility
	 */
	public function getLocalTCE($stripslashes_values = 0, $dontProcessTransformations = 1) {
		$copyTCE = GeneralUtility::makeInstance('TYPO3\\CMS\\Core\\DataHandling\\DataHandler');
		$copyTCE->stripslashes_values = $stripslashes_values;
