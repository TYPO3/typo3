...
class DataHandler {
	...
	protected function processClearCacheQueue() {
		...
		foreach (static::$recordsToClearCacheFor as $table => $uids) {
			foreach (array_unique($uids) as $uid) {
				...
				// For move commands we may get more than 1 parent.
	...
	public function printLogErrorMessages($redirect) {
		$res_log = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'sys_log', 'type=1 AND action<256 AND userid=' . (int)$this->BE_USER->user['uid'] . ' AND tstamp=' . (int)$GLOBALS['EXEC_TIME'] . '	AND error<>0');
