...
class BackendUserAuthentication extends \TYPO3\CMS\Core\Authentication\AbstractUserAuthentication {
	...
	/**
	 * array holds lists of eg. tables, fields and other values related to the permission-system. See fetchGroupData
	 *
	 * @var array
	 * @internal
	 */
	 ...
	 * @param mixed $recData Integer (record uid) or array where fields are at least: pid, t3ver_wsid, t3ver_stage (if versioningWS is set)
	 ...
	 * This method is called by ->backendCheckLogin() (from extending class \TYPO3\CMS\Core\Authentication\BackendUserAuthentication)
	 ...
	 */
	public function writelog($type, $action, $error, $details_nr, $details, $data, $tablename = '', $recuid = 0, $recpid = 0, $event_pid = -1, $NEWid = '', $userId = 0) {
		$fields_values = array(
...
';
	 ...
	 *
	 * @return boolean Returns TRUE if a CLI user was loaded, otherwise NULL!
	 */
	...
	public function backendSetUC() {
		...
		// Saving if updated.
		if ($updated) {
			// Method from the \TYPO3\CMS\Core\Authentication\AbstractUserAuthentication class.
			$this->writeUC();
		}
	}
