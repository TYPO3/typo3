	...
	protected function checkUtf8DatabaseSettingsOrDie() {
		...
		if (preg_match($regExprSetNames, $setDBInit) === 0) {
			...
			if ($matchCount === 0 && TYPO3_enterInstallScript !== '1') {
				...
				die('This TYPO3 installation is using the $GLOBALS[\'TYPO3_CONF_VARS\'][\'SYS\'][\'setDBinit\'] property with the following value:' . LF .
					$setDBInit . LF . LF .
					'SET NAMES defaults to UTF-8 since TYPO3 4.7 and changing its value is not supported anymore.' . LF . LF .
