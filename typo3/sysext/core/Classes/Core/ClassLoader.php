...
class ClassLoader {
	 ...
	 * @param bool $withoutRequire Prevents the requirement of the cache entry and triggers addition to cache only
	 * @return bool
	 */
	static public function autoload($className, $withoutRequire = FALSE) {
	...
	static protected function createCoreAndExtensionRegistry() {
		...
		foreach (ExtensionManagementUtility::getLoadedExtensionListArray() as $extensionKey) {
			try {
				$extensionAutoloadFile = ExtensionManagementUtility::extPath($extensionKey, 'ext_autoload.php');
