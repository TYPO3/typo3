...
class ClassLoader {
	...
	static protected function loadClassLoaderCache() {
		...
		if (!is_array($aliasToClassNameMapping)) {
			...
			$aliasToClassNameMapping = static::createCoreAndExtensionClassAliasMap() ?: array();
		}
		...
		if (!is_array($classRegistry)) {
			static::$cacheUpdateRequired = TRUE;
			$classRegistry = static::lowerCaseClassRegistry(static::createCoreAndExtensionRegistry()) ?: array();
		}
