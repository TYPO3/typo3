...
class CompatbilityClassLoaderPhpBelow50307 extends \TYPO3\CMS\Core\Core\ClassLoader {
	...
	static public function requireClassFileOnce($classPath, $className) {
		if (GeneralUtility::isFirstPartOfStr($classPath, PATH_typo3 . 'sysext/') || !in_array(substr($className, 0, 3), array('tx_', 'Tx_', 'user_'))) {
				// Do nothing for system extensions or external libraries They are already using the proper type hints.
