...
class FileProcessingService {
	...
	protected function process(Resource\ProcessedFile $processedFile, Resource\ResourceStorage $targetStorage) {
		...
		if ($processedFile->isNew() || (!$processedFile->usesOriginalFile() && !$processedFile->exists()) ||
			...
			$task = $processedFile->getTaskObject();
	...
	/**
	 * Factory method for task objects.
	 *
	 * @param string $task
	...
	protected function createTaskObject($task, Resource\ProcessedFile $processedFile, $configuration) {
		/** @var $taskRegistry \TYPO3\CMS\Core\Resource\Processing\TaskTypeRegistry */
		$taskRegistry = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Core\\Resource\\Processing\\TaskTypeRegistry');
		$taskClass = $taskRegistry->getClassForTaskType($task);

		return Utility\GeneralUtility::makeInstance($taskClass, $processedFile, $configuration);
	}
