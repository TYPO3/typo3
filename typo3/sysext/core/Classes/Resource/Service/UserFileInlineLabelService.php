...
class UserFileInlineLabelService {
	...
	public function getInlineLabel(array &$params) {
		# configuratino
		$title = array();
		$sysFileFields = $params['options']['sys_file'] ? : array();
		$sysFileReferenceFields = $params['options']['sys_file_reference'] ? : array();
		...
		foreach ($sysFileFields as $field) {
			...
			$title[] = LocalizationUtility::translate('LLL:EXT:lang/locallang_tca.xlf:sys_file.' . $field, 'lang') . ' ' . $fileLabel . ': <b>' . $value . '</b>';
		}
		...
		foreach ($sysFileReferenceFields as $field) {
			...
			$title[] = LocalizationUtility::translate('LLL:EXT:lang/locallang_tca.xlf:sys_file_reference.' . $field, 'lang') . ': <b>' . $value . '</b>';
		}

		$params['title'] = implode('<br />', $title);
	}
