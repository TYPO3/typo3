...
class ProcessedFile extends AbstractFile {
	...
	/**
	 * @var \TYPO3\CMS\Core\Resource\Processing\Task
	 */
	 ...
	 */
	public static function reconstituteFromDatabaseRecord(File $originalFile, array $record) {
		$task = $record['task'];
		$processingConfiguration = unserialize($record['configuration']);

		$file = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Core\\Resource\\ProcessedFile', $originalFile, $task, $processingConfiguration);
		$file->originalFileSha1 = $record['originalfilesha1'];
		$file->identifier = $record['identifier'];
		$file->name = $record['name'];
		$file->task = $record['task'];
	...
	public function calculateChecksum() {
		return $this->getTaskObject()->getConfigurationChecksum();
	}
	...
	public function getTaskObject() {
		if ($this->taskObject == NULL) {
			...
			$taskClass = $taskRegistry->getClassForTaskType($this->task);

			if ($taskClass === NULL) {
				throw new \RuntimeException('Unknown processing task "' . $this->task . '"');
			}

			$this->taskObject = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance($taskClass,
