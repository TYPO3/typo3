...
class ProcessedFile extends AbstractFile {
	 ...
	 */
	// TODO this needs a massive rewrite -> most of it should be moved to a reconstituteSavedObject() method (that recreates an object from a DB row)
	...
	public function isOutdated() {
		// TODO fully implement; throw away needsReprocessing() afterwards
	...
	public function needsReprocessing() {
		...
		if (array_key_exists('checksum', $this->properties) && $this->calculateChecksum() !== $this->properties['checksum'])  {
			...
			// TODO change this check to use the checksum
	...
	public function generateProcessedFileNameWithoutExtension() {
		$name = '';
		if ($this->task == self::CONTEXT_IMAGEPREVIEW) {
			$name .= 'thumbnail_';
		}
