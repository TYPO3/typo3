 ...
 */
class IndexRecordRepository implements SingletonInterface {
	 ...
	 *
	 * @return IndexRecordRepository
	 */
	public static function getInstance() {
		if (static::$instance === NULL) {
			static::$instance = GeneralUtility::makeInstance('TYPO3\CMS\Core\Resource\Index\IndexRecordRepository');
		}
	...
	/**
	 * Returns all indexed files which
	 *
	 * @param string $hash
	 * @return mixed
	 */
	...
	public function hasIndexRecord(File $file) {
		if (intval($file->getUid()) > 0) {
			return $this->getDatabase()->exec_SELECTcountRows('uid', $this->table, 'uid=' . intval($file->getUid())) === 1;
		} else {
			return $this->getDatabase()->exec_SELECTcountRows(
				'uid',
				$this->table,
				sprintf('storage=%u AND identifier=%s', intval($file->getStorage()->getUid()), $this->getDatabase()->fullQuoteStr($file->getIdentifier(), $this->table))
			) === 1;
		}
	}
	...
	public function update(File $file) {
		$updatedProperties = array_diff($this->fields, $file->getUpdatedProperties());
		$updateRow = array();
