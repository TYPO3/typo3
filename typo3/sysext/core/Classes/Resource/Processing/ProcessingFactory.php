...
class ProcessingFactory {
	 ...
	 * @param string $taskType The task to process on the file
	 ...
	 * @param Resource\File $originalFile
	 * @param string $taskType
	 * @param array $configuration
	 * @return \TYPO3\CMS\Core\Resource\ProcessedFile
	 */
	public function createProcessedFileObjectFromDatabase(array $databaseRow, Resource\File $originalFile = NULL, $taskType = '', array $configuration = array()) {
		...
		return $this->createDomainObject($originalFile, $taskType, $configuration, $databaseRow);
	}

	/**
	 * Creates a ProcessedFile object from a file object and a processing configuration
	 *
	 * @param Resource\FileInterface $originalFile
	 * @param string $taskType
	 * @param array $configuration
	 * @return Resource\ProcessedFile
	 */
	public function createNewProcessedFileObject(Resource\FileInterface $originalFile, $taskType, array $configuration) {
		return $this->createDomainObject($originalFile, $taskType, $configuration);
	}
	 ...
	 * @param string $taskType
	 * @param array $configuration
	 * @param null|array $databaseRow
	 * @return Resource\ProcessedFile
	 */
	protected function createDomainObject(Resource\FileInterface $originalFile, $taskType, $configuration, array $databaseRow = NULL) {
