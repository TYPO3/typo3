...
class LocalDriver extends \TYPO3\CMS\Core\Resource\Driver\AbstractDriver {
	...
	protected function resolveHumanReadableName($fileOrFolderName) {
		...
		if (preg_match('/^_(.*)_$/i', $fileOrFolderName, $match)) {
			$newFileOrFolderName = $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_mod_file_list.xml:' . $match[1], TRUE);
		}

		// Append original name
		if ($newFileOrFolderName && $newFileOrFolderName !== $fileOrFolderName) {
			$fileOrFolderName = $newFileOrFolderName . ' (' . $fileOrFolderName . ')';
		}
