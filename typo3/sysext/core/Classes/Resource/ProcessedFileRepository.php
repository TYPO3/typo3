...
class ProcessedFileRepository extends AbstractRepository {
	...
	public function remove($processedFile) {
		if (!$processedFile instanceof ProcessedFile) {
			throw new \InvalidArgumentException('Parameter is no ProcessedFile object but got type "' . (is_object($processedFile) ? get_class($processedFile) : gettype($processedFile)) . '"', 1378653061);
		}
