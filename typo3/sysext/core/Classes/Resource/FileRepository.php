...
class FileRepository extends AbstractRepository {
	 ...
	 * @param integer $max Maximum number of relations to return
	 * @param integer $offset Offset of the first relation to return
	 * @return array An array of objects, empty if no objects found
	 * @throws \InvalidArgumentException
	 * @api
	 */
	public function findByRelation($tableName, $fieldName, $uid, $max = NULL, $offset = NULL) {
		$limit = '';
		$itemList = array();
		...
		if ($offset !== NULL && $max !== NULL) {
			$limit .= intval($offset) . ',' . intval($max);
		} elseif ($max !== NULL) {
			$limit .= intval($max);
		} elseif($offset !== NULL) {
			$limit .= intval($offset) . ',99999999999';
		}
		debug($limit);
