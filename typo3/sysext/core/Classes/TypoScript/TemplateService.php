	...
	public function updateRootlineData($fullRootLine) {
		...
		foreach ($this->rootLine as $level => $dataArray) {
			...
			if (!array_key_exists($currentUid, $fullRootLineByUid)) {
				throw new \RuntimeException(sprintf('The full rootLine does not contain data for the page with the uid %d that is contained in the template rootline.', $currentUid));
			}
