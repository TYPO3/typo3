...
//		$this->createDatabaseStructure();
	...
	private function setUpLocalConfiguration() {
		...
		//TODO: add additional extensions to local configuration
		$localConfigurationArray = $this->array_merge_recursive_overrule(
		...
		$result = $this->writeFile(
			...
			'return ' .
			$this->arrayExport(
				$this->renumberKeysToAvoidLeapsIfKeysAreAllNumeric($localConfigurationArray)
			) .
			';' . LF .
			'?>'
		);
...
//			->initializeTypo3DbGlobal()
	 ...
	 */
	private function array_merge_recursive_overrule(array $arr0, array $arr1, $notAddKeys = FALSE, $includeEmptyValues = TRUE, $enableUnsetFeature = TRUE) {
		foreach ($arr1 as $key => $val) {
			...
			if (is_array($arr0[$key])) {
				if (is_array($arr1[$key])) {
					$arr0[$key] = $this->array_merge_recursive_overrule($arr0[$key], $arr1[$key], $notAddKeys, $includeEmptyValues, $enableUnsetFeature);
				}
			} elseif (
...
//				throw new \RuntimeException('Objects are not supported', 1342294986);
	...
	private function renumberKeysToAvoidLeapsIfKeysAreAllNumeric(array $array = array(), $level = 0) {
		...
		foreach ($renumberedArray as $key => $value) {
			if (is_array($value)) {
				$renumberedArray[$key] = $this->renumberKeysToAvoidLeapsIfKeysAreAllNumeric($value, $level);
			}
		}
