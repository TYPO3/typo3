...
class ResourceStorageTest extends \TYPO3\CMS\Core\Tests\Unit\Resource\BaseTestCase {
	 ...
	 * @test
	 */
	public function getFolderRoleReturnsDefaultForRegularFolders() {
		...
		$role = $this->fixture->getFolderRole($this->getSimpleFolderMock('/' . $folderIdentifier . '/'));
	 ...
	 * @test
	 */
	public function getFolderRoleReturnsCorrectValueForDefaultProcessingFolder() {
		$this->prepareFixture(array());

		$role = $this->fixture->getFolderRole($this->getSimpleFolderMock('/' . ResourceStorage::DEFAULT_ProcessingFolder . '/'));
	 ...
	 * @test
	 */
	public function getFolderRoleReturnsCorrectValueForConfiguredProcessingFolder() {
		$folderIdentifier = uniqid();
		$this->addToMount(array(
			$folderIdentifier => array()
		));
