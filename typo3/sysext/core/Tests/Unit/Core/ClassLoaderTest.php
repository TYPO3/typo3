...
class ClassLoaderTest extends \TYPO3\CMS\Core\Tests\UnitTestCase {
	 ...
	 * @test
	 */
	public function unregisterAutoloaderWritesNotExistingXclassCLassLookupFromGetClassPathByRegistryLookupToCache() {
		$xclassClassName = 'Tx_Foo' . uniqid() . 'Xclass';
		...
		$this->assertSame(NULL, \TYPO3\CMS\Core\Core\ClassLoader::getClassPathByRegistryLookup($xclassClassName));

		// Expect NULL lookup is cached
		$expectedCacheString = '\'' . strtolower($xclassClassName) . '\' => NULL,';
		$mockCache
			->expects($this->once())
			->method('set')
			->with($this->anything(), $this->stringContains($expectedCacheString));
