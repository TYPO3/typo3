...
/**
 * Testcase for the Yac cache backend.
 * Based on testcase for the APC cache backend.
 *
 * @author Ingo Renner <ingo@typo3.org>
 * @author Stefan Neufeind <ingo (at) speedpartner.de>
 */
class YacBackendTest extends \TYPO3\CMS\Core\Tests\UnitTestCase {
	...
	public function setUp() {
		if (!extension_loaded('yac')) {
			$this->markTestSkipped('Yac extension was not available');
		}
	}
	...
	public function setThrowsExceptionIfNoFrontEndHasBeenSet() {
		$backend = new \TYPO3\CMS\Core\Cache\Backend\YacBackend('Testing');
		$data = 'Some data';
	...
	public function itIsPossibleToSetAndCheckExistenceInCache() {
		...
		$this->assertTrue($inCache, 'Yac backend failed to set and check entry');
	}
	...
	public function itIsPossibleToSetAndGetEntry() {
		...
		$this->assertEquals($data, $fetchedData, 'Yac backend failed to set and retrieve data');
	}
	...
	public function itIsPossibleToRemoveEntryFromCache() {
		...
		$inCache = $backend->has($identifier);
		$this->assertFalse($inCache, 'Failed to set and remove data from Yac backend');
	}
	...
	public function itIsPossibleToOverwriteAnEntryInTheCache() {
		...
		$fetchedData = $backend->get($identifier);
		$this->assertEquals($otherData, $fetchedData, 'Yac backend failed to overwrite and retrieve data');
	}
	...
	public function setCacheIsSettingIdentifierPrefixWithCacheIdentifier() {
		...
		/** @var $backendMock \TYPO3\CMS\Core\Cache\Backend\YacBackend */
		$backendMock = $this->getMock(
			'TYPO3\\CMS\\Core\\Cache\\Backend\\YacBackend',
	...
	public function flushByTagRemovesCacheEntriesWithSpecifiedTag() {
		...
		$backend->set('BackendYacTest1', $data, array('UnitTestTag%test', 'UnitTestTag%boring'));
		$backend->set('BackendYacTest2', $data, array('UnitTestTag%test', 'UnitTestTag%special'));
		$backend->set('BackendYacTest3', $data, array('UnitTestTag%test'));
		$backend->flushByTag('UnitTestTag%special');
		$this->assertTrue($backend->has('BackendYacTest1'), 'BackendYacTest1');
		$this->assertFalse($backend->has('BackendYacTest2'), 'BackendYacTest2');
		$this->assertTrue($backend->has('BackendYacTest3'), 'BackendYacTest3');
	}
	...
	public function flushRemovesAllCacheEntries() {
		$backend = $this->setUpBackend();
		$data = 'some data' . microtime();
		$backend->set('BackendYacTest1', $data);
		$backend->set('BackendYacTest2', $data);
		$backend->set('BackendYacTest3', $data);
		$backend->flush();
		$this->assertFalse($backend->has('BackendYacTest1'), 'BackendYacTest1');
		$this->assertFalse($backend->has('BackendYacTest2'), 'BackendYacTest2');
		$this->assertFalse($backend->has('BackendYacTest3'), 'BackendYacTest3');
	}
	...
	public function flushRemovesOnlyOwnEntries() {
		...
		$thisBackend = new \TYPO3\CMS\Core\Cache\Backend\YacBackend('Testing');
		$thisBackend->setCache($thisCache);
		$thatCache = $this->getMock('TYPO3\\CMS\\Core\\Cache\\Frontend\\FrontendInterface', array(), array(), '', FALSE);
		$thatCache->expects($this->any())->method('getIdentifier')->will($this->returnValue('thatCache'));
		$thatBackend = new \TYPO3\CMS\Core\Cache\Backend\YacBackend('Testing');
	...
	/**
	 * Sets up the Yac backend used for testing
	 *
	 * @return \TYPO3\CMS\Core\Cache\Backend\YacBackend
	 */
