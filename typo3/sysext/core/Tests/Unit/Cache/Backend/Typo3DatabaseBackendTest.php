...
class Typo3DatabaseBackendTest extends \TYPO3\CMS\Core\Tests\UnitTestCase {
	...
	public function flushByTagDoesNotLeaveOrphanedEntriesInTagsTable() {
		...
		$this->setUpMockFrontendOfBackend($backend);
		$data = 'some data' . microtime();
		$backend->set('BackendDbTest', $data, array('UnitTestTag%boring', 'UnitTestTag%special'));
		$backend->flushByTag('UnitTestTag%special');

		$tagEntriesFound = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('*', $this->testingTagsTable, 'identifier = ' . $GLOBALS['TYPO3_DB']->fullQuoteStr('BackendDbTest', $this->testingTagsTable));
