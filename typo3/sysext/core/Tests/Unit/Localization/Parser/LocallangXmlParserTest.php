...
class LocallangXmlParserTest extends \TYPO3\CMS\Core\Tests\UnitTestCase {
	...
	public function canTranslateNumericKeys() {
		...
		foreach ($LOCAL_LANG['default'] as $key => $labelData) {
			$expectedLabels[$key] = $labelData[0]['source'] . ' [FR]';
		}

		$LOCAL_LANG = \TYPO3\CMS\Core\Utility\GeneralUtility::readLLfile($this->llxmlFileNames['locallangNumericKeys'], 'fr');
		foreach ($LOCAL_LANG['fr'] as $key => $labelData) {
			$this->assertEquals($expectedLabels[$key], $labelData[0]['target']);
		}
	}
