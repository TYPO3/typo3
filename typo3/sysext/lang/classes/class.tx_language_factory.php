...
/**
 * class.tx_language_factory.php
 *
 * Provide a language parser factory
 *
 ...
 * @author	Dominique Feyer <dfeyer@reelpeek.net>
 */
class tx_language_Factory implements t3lib_Singleton {

	/**
	 * @var tx_Language_CacheAbstract
	 */
	protected $cache;
	...
	public function __construct() {
		...
		$this->store = t3lib_div::makeInstance('tx_language_Store');
	...
	public function initialize() {
		$cacheClass = (TYPO3_UseCachingFramework) ? 'tx_Language_CacheCachingFramework' : 'tx_Language_CacheFile';
		$this->cache = t3lib_div::makeInstance($cacheClass);
	}

	/**
	 * Return an existing instance of the current class
	 *
	 * This singleton return always the same class instance, only one instance per file reference can be created
	 *
	 * @static
	...
	public function getParsedData($fileReference, $languageKey, $charset, $errorMode) {
		...
		try {
			...
			/** @var $parser tx_Language_ParserInterface */
			...
			} catch (tx_language_FileNotFoundException $exception) {
