 ...
 */
class tx_lang_LlxmlParser extends tx_lang_XmlParserAbstract {
	...
	public function getParsedData($sourcePath, $languageKey, $charset) {
		...
		} catch (tx_lang_InvalidXMLFileException $e) {
	 ...
	 *
	 * @throws tx_lang_InvalidXMLFileException
	...
	protected function parseXmlTargetFile($targetPath) {
		...
		if (!isset($rootXmlNode) || $rootXmlNode === FALSE) {
			throw new tx_lang_InvalidXMLFileException('The path provided does not point to existing and accessible well-formed XML file (' . $targetPath . ').', 1278155987);
		}
