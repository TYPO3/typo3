var languageModule = {
	...
	processElements: function(elements, type) {
		...
		jQuery.ajax({
			...
			success: function(response) {
				...
				$element.addClass('languageState' + state).html($icon.html());
	...
	updateTranslations: function() {
		var $elements = jQuery('.languageState' + languageModule.states.available);
		if ($elements.length < 1) {
			$elements = jQuery('.languageState');
		}
		languageModule.processElements($elements, 'update');
	},
