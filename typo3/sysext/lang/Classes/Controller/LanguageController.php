...
class Tx_Lang_Controller_LanguageController extends Tx_Extbase_MVC_Controller_ActionController {
	 ...
	 * @return array
	 */
	protected function mergeUpdateResult($extensions, $updateResult) {
			...
			if (!isset($fetch[$lang])) {
				...
				if ($md5OfTranslationFile !== $fetch[$lang]['md5']) {
					...
					$result[$lang] = $update ?
						...
						array(
							'icon' => '<span class="t3-icon t3-icon-status t3-icon-status-status t3-icon-status-permission-denied">&nbsp;</span>',
