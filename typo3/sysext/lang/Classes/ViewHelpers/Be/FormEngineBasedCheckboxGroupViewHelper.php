...
class FormEngineBasedCheckboxGroupViewHelper extends \TYPO3\CMS\Fluid\ViewHelpers\Form\SelectViewHelper {
	...
	public function initializeArguments() {
		...
		$this->registerArgument('icon', 'string', 'If specified, will show the given icon in front of each option label.');
		$this->registerArgument('defaultIcon', 'string', 'If specified, will show the given icon in front of each option label if "icon" was not found.');
	}
	...
	protected function renderCheckboxGroupField($formName, $fieldName) {
		...
		foreach($options as $value => $label) {
			$items[] = array($label, $value, $this->getIconFileName($value));
	...
	/**
	 * Returns the filename of given icon
	 *
	 * @param string $value Option value
	 * @return string The path
	 */
	protected function getIconFileName($value = '') {
		if (empty($this->arguments['icon'])) {
			return (!empty($this->arguments['defaultIcon']) ? $this->arguments['defaultIcon'] : '');
		}
		if (strpos($this->arguments['icon'], '###VALUE###') === FALSE || empty($value)) {
			return $this->arguments['icon'];
		}
		$fileName = str_replace('###VALUE###', $value, $this->arguments['icon']);
		if (!file_exists(\TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName($fileName))) {
			$fileName = (!empty($this->arguments['defaultIcon']) ? $this->arguments['defaultIcon'] : '');
		}
		return $fileName;
	}
