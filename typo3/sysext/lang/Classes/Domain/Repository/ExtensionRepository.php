	...
	public function findAll() {
		if (!count($this->extensions)) {
			...
			foreach ($availableAndInstalledExtensions as $entry) {
				$extensionIcon = $GLOBALS['TYPO3_LOADED_EXT'][$entry['key']]['ext_icon'];
				if (empty($extensionIcon)) {
					$extensionIcon = \TYPO3\CMS\Core\Extension\ExtensionManager::getExtensionIcon(PATH_site . $entry['siteRelPath'] . '/');
				}
				if (empty($extensionIcon)) {
					$extensionIcon = '/typo3/clear.gif';
				} else {
					$extensionIcon = '../' . $entry['siteRelPath'] . '/' . $extensionIcon;
				}
