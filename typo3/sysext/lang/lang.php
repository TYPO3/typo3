...
class language {
	...
	public function getLL($index, $hsc = FALSE) {
		$output = '';
			...
			// Get Local Language
		if (isset($GLOBALS['LOCAL_LANG'][$this->lang][$index][0]['target']) && trim((string)$GLOBALS['LOCAL_LANG'][$this->lang][$index][0]['target']) !== '') {
			$output = $this->hscAndCharConv($GLOBALS['LOCAL_LANG'][$this->lang][$index][0]['target'], $hsc);
		} elseif (isset($GLOBALS['LOCAL_LANG']['default'][$index][0]['target']) && trim((string)$GLOBALS['LOCAL_LANG']['default'][$index][0]['target']) !== '') {
	...
	public function getLLL($index, $localLanguage, $hsc = FALSE) {
		$output = '';
			...
			// Get Local Language
		if (isset($localLanguage[$this->lang][$index][0]['target']) && trim((string)$localLanguage[$this->lang][$index][0]['target']) !== '') {
			$output = $this->hscAndCharConv($localLanguage[$this->lang][$index][0]['target'], $hsc);
		} elseif (isset($localLanguage['default'][$index][0]['target']) && trim((string)$localLanguage['default'][$index][0]['target']) !== '') {
