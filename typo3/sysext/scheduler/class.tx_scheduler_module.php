		...
		if ($numRows == 0) {
			...
			while (($schedulerRecord = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
				...
				if ($this->scheduler->isValidTaskObject($task)) {
					...
					$executionStatusOutput = '<img ' . t3lib_iconWorks::skinImg(t3lib_extMgm::extRelPath('scheduler'), 'res/gfx/status_' . $executionStatus . '.png') . ' alt="' . htmlspecialchars($GLOBALS['LANG']->getLL('status.' . $executionStatus)) . '" title="' . htmlspecialchars($GLOBALS['LANG']->getLL('status.' . $executionStatus)) . (($executionStatusDetail) ? ' (' . htmlspecialchars($executionStatusDetail) . ')': '') . '" />' . $failureOutput;
