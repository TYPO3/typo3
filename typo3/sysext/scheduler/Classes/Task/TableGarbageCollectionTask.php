...
class TableGarbageCollectionTask extends \TYPO3\CMS\Scheduler\Task {
	...
	public function execute() {
		$tableConfigurations = $GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['scheduler']['tasks']['TableGarbageCollectionTask']['options']['tables'];
		$tableHandled = FALSE;
		...
		if (!$tableHandled) {
			throw new \RuntimeException(('TableGarbageCollectionTask misconfiguration: ' . $this->table) . ' does not exist in configuration', 1308354399);
		}
		return TRUE;
	}
		...
		if (!empty($configuration['expireField'])) {
			...
			if (!$this->allTables) {
				...
				if (!isset($configuration['expirePeriod'])) {
					throw new \RuntimeException('TableGarbageCollectionTask misconfiguration: No expirePeriod defined for table ' . $table, 1308355095);
				}
				$deleteTimestamp = strtotime(('-' . $configuration['expirePeriod']) . 'days');
			}
			$where = ($configuration['dateField'] . ' < ') . $deleteTimestamp;
		} else {
			throw new \RuntimeException('TableGarbageCollectionTask misconfiguration: Either expireField or dateField must be defined for table ' . $table, 1308355268);
		}
		$GLOBALS['TYPO3_DB']->exec_DELETEquery($table, $where);
		$error = $GLOBALS['TYPO3_DB']->sql_error();
		if ($error) {
			throw new \RuntimeException((('TableGarbageCollectionTask failed for table ' . $this->table) . ' with error: ') . $error, 1308255491);
		}
		return TRUE;
	}
