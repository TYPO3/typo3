...
class CachingFrameworkGarbageCollectionAdditionalFieldProvider extends AbstractAdditionalFieldProvider
{
    ...
    public function getAdditionalFields(array &$taskInfo, $task, SchedulerModuleController $schedulerModule)
    {
        ...
        $fieldHtml = '<select class="form-select" name="' . $fieldName . '" id="' . $fieldId . '" size="10" multiple="multiple">'
            . $fieldOptions . '</select>';
        $additionalFields = [];
    ...
    public function validateAdditionalFields(array &$submittedData, SchedulerModuleController $schedulerModule)
    {
        ...
        if (is_array($submittedData['scheduler_cachingFrameworkGarbageCollection_selectedBackends'])) {
            ...
            $this->addMessage($this->getLanguageService()->sL('LLL:EXT:scheduler/Resources/Private/Language/locallang.xlf:msg.noCacheBackendSelected'), ContextualFeedbackSeverity::ERROR);
            $validData = false;
        }
        return $validData;
    }
