...
class TableGarbageCollectionAdditionalFieldProvider extends AbstractAdditionalFieldProvider
{
    ...
    public function validateNumberOfDaysAdditionalField(array &$submittedData)
    {
        ...
        if (!isset($submittedData['scheduler_tableGarbageCollection_numberOfDays'])) {
            ...
            // Issue error message
            $this->addMessage($this->getLanguageService()->sL('LLL:EXT:scheduler/Resources/Private/Language/locallang.xlf:msg.invalidNumberOfDays'), ContextualFeedbackSeverity::ERROR);
        }
        return $validData;
    }
    ...
    public function saveAdditionalFields(array $submittedData, AbstractTask $task)
    {
        $task->setTaskParameters(
            [
                'allTables' => ($submittedData['scheduler_tableGarbageCollection_allTables'] ?? '') === 'on',
                'numberOfDays' => (int)($submittedData['scheduler_tableGarbageCollection_numberOfDays'] ?? 0),
