...
class tx_scheduler_RecyclerCleanup_AdditionalFieldProvider implements tx_scheduler_AdditionalFieldProvider {
	...
	public function getAdditionalFields(array &$taskInfo, $task, tx_scheduler_Module $parentObject) {
		...
		if (empty($taskInfo['scheduler_recyclerCleanup_numberOfDays'])) {
			$taskInfo['scheduler_recyclerCleanup_numberOfDays'] = (int) $this->defaultNumberOfDays;
			if ($parentObject->CMD == 'edit') {
				$taskInfo['scheduler_recyclerCleanup_numberOfDays'] = (int) $task->numberOfDays;
			}
		}

		$fieldName    = 'tx_scheduler[scheduler_recyclerCleanup_numberOfDays]';
		$fieldId      = 'task_recyclerCleanup_numberOfDays';
		$fieldValue   = $taskInfo['scheduler_recyclerCleanup_numberOfDays'];
		$fieldHtml    = '<input type="text" name="' . $fieldName . '" id="' . $fieldId . '" value="' . $fieldValue . '" />';
