 ...
 */
class tx_scheduler_RecyclerCleanup extends tx_scheduler_Task {
	...
	public function execute() {
		$seconds   = (60 * 60 * 60 * (int) $this->numberOfDays);
	...
	/**
	 * Get a list of all files in a directory recursively and removes
	 * old ones
	 *
	...
	protected function cleanupRecycledFiles($directory, $timestamp) {
		...
		if (!(@is_dir($directory))) {
			throw new InvalidArgumentException('Given directory "' . $directory . '" doesn\'t exist', 1301614535);
		}

			// Unlink old files in directory
		$directoryContent = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($directory));
		foreach ($directoryContent as $fileName => $path) {
			$pathName = $path->getPath();

				// Skip other directories than recycler directory
			if (substr($pathName, strrpos($pathName, '/') + 1) !== $this->recyclerDirectory) {
				continue;
			}

				// Remove file, if last modified before given timestamp
			if ($path->isFile() && $timestamp > $path->getMTime()) {
				if (!(@unlink($fileName))) {
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/scheduler/tasks/class.tx_scheduler_recyclercleanup.php']) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/scheduler/tasks/class.tx_scheduler_recyclercleanup.php']);
}
