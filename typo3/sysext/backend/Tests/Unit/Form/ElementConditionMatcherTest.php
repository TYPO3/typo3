...
/**
 * Tests for the ElementConditionMatcher (the TCA column 'displayCond' feature)
 */
class ElementConditionMatcherTest extends \TYPO3\CMS\Core\Tests\UnitTestCase {
	...
	public function matchConditionStrings($condition, array $record, $flexformValueKey, $expectedResult) {
		$this->fixture->setFlexformValueKey($flexformValueKey);
		$this->fixture->setRecord($record);
		$this->assertEquals($expectedResult, $this->fixture->match($condition));
	}
	...
	public function conditionStringDataProvider() {
		return array(
			...
			'HIDE_FOR_NON_ADMINS' => array(
				'HIDE_FOR_NON_ADMINS', array(), NULL, $GLOBALS['BE_USER']->isAdmin()
			),
			...
			'HIDE_L10N_SIBLINGS (#4)' => array(
				'HIDE_L10N_SIBLINGS:except_admin', array(), 'vEN', $GLOBALS['BE_USER']->isAdmin()
			),
			'REC (#1)' => array(
