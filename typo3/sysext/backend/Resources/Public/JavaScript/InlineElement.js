define('TYPO3/CMS/Backend/InlineElement', [], function ($) {
	...
	inline = {
		...
		expandCollapseRecord: function(objectId, expandSingle, returnURL) {
			...
			if((TYPO3.jQuery('#' + escapedObjectId + '_fields') && TYPO3.jQuery('#irre-loading-indicator' + objectId).length > 0) || inline.isLoading) {
				return false;
			} else if (TYPO3.jQuery('#' + objectId + '_fields') && TYPO3.jQuery('#' + objectId + '_fields').html().substr(0,16) == '<!--notloaded-->') {
		...
		domAddRecordDetails: function(objectId, objectPrefix, expandSingle, htmlData) {
			...
			if (TYPO3.jQuery('#'+objectId + '_icon')) {
				TYPO3.jQuery('#'+objectId + '_iconcontainer').removeClass('loading-indicator');
				TYPO3.jQuery('#'+objectId + '_icon').show();
			}
		...
		getDomHeadChildren: function(head) {
			var headTags = [];
			TYPO3.jQuery('head script', 'head link').each(function(tag) {
				headTags.push(tag);
			});
			return headTags;
		},
		...
		changeSorting: function(objectId, direction) {
			...
			if (formObj.length) {
				...
				if (changed) {
					...
					TYPO3.jQuery('#'+objectPrefix + this.structureSeparator + records[current-cAdj] + '_div').insertBefore(TYPO3.jQuery('#'+objectPrefix + this.structureSeparator + records[current+1-cAdj] + '_div'));
		...
		redrawSortingButtons: function(objectPrefix, records) {
			...
			for (i=0; i<records.length; i++) {
				...
				headerObj = TYPO3.jQuery('#'+objectPrefix + this.structureSeparator + records[i] + '_header').parent();
				if (headerObj.find('.sortingUp').length > 0) {
					if (i == 0) {
						headerObj.find('.sortingUp').css('visibility', 'hidden');
					} else {
						headerObj.find('.sortingUp').css('visibility', 'visible');
					}
				}
				if (headerObj.find('.sortingDown').length > 0) {
					if (i == records.length-1) {
						headerObj.find('.sortingDown').css('visibility', 'hidden');
					} else {
						headerObj.find('.sortingDown').css('visibility', 'visible');
					}
				}
/*				sortingObj[0] = headerObj.find('.sortingUp');
				sortingObj[1] = headerObj.find('.sortingDown');

				if (sortingObj[0].length > 0) {
					sortingObj[0].first().css('visibility', (i == 0 ? 'hidden' : 'visible'));
				}
				if (sortingObj[1].length > 0) {
					sortingObj[1].last().css('visibility', (i == records.length-1 ? 'hidden' : 'visible'));
				}*/
			}
		},
		...
		enableDisableRecord: function(objectId) {
			...
			var icon = TYPO3.jQuery('#'+objectId + '_disabled');
		...
		handleChangedField: function(formField, objectId) {
			...
			if (formObj != undefined) {
				...
				TYPO3.jQuery('#'+objectId+'_label').innerHTML = value.length ? value : this.noTitleString;
			}
			return true;
		},
		...
		setVisibilityOfElementsWithClassName: function(action, selector, parentElement) {
			var domObjects = Selector.findChildElements(TYPO3.jQuery('#'+parentElement), [selector]);
			if (action == 'hide') {
	...
	var initialize = function(options) {
		...
		/*if (undefined !== TYPO3.settings.RequireJS.PostInitializationModules['TYPO3/CMS/Backend/InlineElement']) {
			$.each(TYPO3.settings.RequireJS.PostInitializationModules['TYPO3/CMS/Backend/InlineElement'], function(pos, moduleName) {
				require([moduleName]);
			});
		}*/
	...
	//$(document).on('click', 'div.t3-form-field-header-inline', inline.toggleEvent);
	TYPO3.jQuery(document).delegate('div.t3-form-field-header-inline', 'click', inline.toggleEvent);
