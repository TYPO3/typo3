...
class BackendUtility {
	...
	static public function getUserNames($fields = 'username,usergroup,usergroup_cached_list,uid', $where = '') {
		return self::getRecordsWithSortedTitle(GeneralUtility::trimExplode(',', $fields, TRUE), 'be_users', 'username', $where);
	}
	...
	static public function getGroupNames($fields = 'title,uid', $where = '') {
		return self::getRecordsWithSortedTitle(GeneralUtility::trimExplode(',', $fields, TRUE), 'be_groups', 'title', $where);
	}
	...
	static protected function getRecordsSortedByTitle(array $fields, $table, $titleField, $where = '') {
		$result = array();
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', $table, 'pid=0 ' . $where . self::deleteClause($table));
