...
';
	 ...
	 * @var boolean
	 */
	protected $generateHiDpiSprite = TRUE;
	...
	/**
	 * Enables/Disables HiDpiSprite Generation
	 *
	 * @param boolean $boolean
	 * @return \TYPO3\CMS\Backend\Sprite\SpriteGenerator An instance of $this, to enable chaining.
	 */
	public function setGenerateHiDpiSprite($boolean = TRUE) {
		$this->generateHiDpiSprite = $boolean;
		return $this;
	}
	/**
	...
	public function generateSpriteFromArray(array $files) {
		...
		if ($this->generateHiDpiSprite) {
			$this->generateHiDPiGraphic();
		}
		$this->generateCSS();
	...
	protected function generateCSS() {
		...
		foreach ($this->spriteBases as $base) {
			...
			if ($this->generateHiDpiSprite) {
				$hiDpiMarkerArray = array_merge($markerArray, array(
				...
				$cssData .= HtmlParser::substituteMarkerArray($this->templateSpriteHiDpi, $hiDpiMarkerArray);
			}
		}
	 ...
	 * @return void
	 */
	protected function generateHiDPiGraphic() {
		...
		foreach ($this->iconsData as $icon) {
			$function = 'imagecreatefrom' . strtolower($icon['fileExtension']);
			if (function_exists($function)) {
				if ($icon['fileNameHiDpi'] !== FALSE) {
					// copy HiDPI file
					$currentIcon = $function($icon['fileNameHiDpi']);
	...
	protected function buildFileInformationCache(array $files) {
		foreach ($files as $iconName => $iconFile) {
			...
			if ($this->generateHiDpiSprite) {
				$hiDpiFile = str_replace('.' . $fileInfo['extension'], '.x2.' . $fileInfo['extension'], $iconFile);
				if (@file_exists(PATH_site . $hiDpiFile)) {
					$this->iconsData[$iconName]['fileNameHiDpi'] = $hiDpiFile;
				}
			}
