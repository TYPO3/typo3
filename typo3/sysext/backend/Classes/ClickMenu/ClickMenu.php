...
class ClickMenu {
	...
	public function printFileClickMenu($combinedIdentifier) {
		...
		if ($fileObject) {
			...
			// Hide
			if (!in_array('hide', $this->disabledItems) && $isStorageRoot && $editStorage) {
				$record = \t3lib_BEfunc::getRecord('sys_file_storage', $fileObject->getStorage()->getUid());
			...
			if (!in_array('edit', $this->disabledItems)) {
				...
				} elseif ($isStorageRoot && $editStorage) {
			...
			if (!in_array('delete', $this->disabledItems)) {
				if ($isStorageRoot) {
					if ($editStorage) {
						$menuItems['delete'] = $this->DB_delete('sys_file_storage', $fileObject->getStorage()->getUid());
					}
				} else {
