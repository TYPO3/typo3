...
';
	...
	public function getSingleField_typeSelect_single($table, $field, $row, &$PA, $config, $selItems, $nMV_label) {
		...
		if (($minItems = \TYPO3\CMS\Core\Utility\MathUtility::forceIntegerInRange($config['minitems'], 0)) > 0) {
			$this->registerRequiredProperty('field', $table . '_' . $row['uid'] . '_' . $field, $PA['itemFormElName']);
		}
			...
			foreach ($dataStruct as $key => $value) {
				...
				if (is_array($value)) {
					...
					if (strlen($theTitle) > 0) {
						$theTitle = htmlspecialchars(\TYPO3\CMS\Core\Utility\GeneralUtility::fixed_lgd_cs($this->sL($theTitle), (int)$GLOBALS['BE_USER']->uc['titleLen']));
					}
