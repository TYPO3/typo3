				...
				if (in_array($field, $this->hiddenFieldListArr)) {
					...
					if ($PA['fieldConf']['config']['mode'] == 'useOrOverridePlaceholder') {
						...
						. '<span class="t3-tceforms-placeholder-override-checkbox">' .
							...
							'<input type="checkbox" name="' . $PA['itemFormElNameActive'] . '" value="1" onchange="' . $onChange . '"' . $checked . ' /> <label>' . $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.xlf:labels.placeholder.override') . '</label>' . // TODO add id to label?
						'</span>'
