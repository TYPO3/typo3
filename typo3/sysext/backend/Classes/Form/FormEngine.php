		...
		if (
			...
			if (!$PA['fieldTSConfig']['disabled']) {
				...
				if (in_array($field, $this->hiddenFieldListArr)) {
					...
					$label = htmlspecialchars($PA['label'], ENT_COMPAT | ENT_HTML401, 'UTF-8', FALSE);
...
';
			...
			if (!($p[1] != $PA['itemFormElValue'] && is_array($uniqueIds) && in_array($p[1], $uniqueIds))) {
				if (!strcmp($p[1], '--div--')) {
					...
					if (count($optGroupStart)) {
						...
						$opt[] = '<optgroup label="' . htmlspecialchars($optGroupStart[0], ENT_COMPAT | ENT_HTML401, 'UTF-8', FALSE) . '"' . ($optGroupStart[1] ? ' style="' . htmlspecialchars($optGroupStart[1]) . '"' : '') . ' class="c-divider">' . LF;
						$optGroupOpen = TRUE;
						$c--;
						$optGroupStart = array();
					}
		...
		if (!$disabled) {
			...
			foreach ($selItems as $p) {
				...
				if (!strcmp($p[1], '--div--')) {
					...
					$label = htmlspecialchars($p[0], ENT_COMPAT | ENT_HTML401, 'UTF-8', FALSE);
					if ($hasHelp) {
			...
			foreach ($itemArray as $theNoMatchValue => $temp) {
				...
				array_unshift($tRows, '
						<tr class="c-invalidItem">
							...
							<td class="c-labelCell">' . htmlspecialchars(@sprintf($nMV_label, $theNoMatchValue), ENT_COMPAT | ENT_HTML401, 'UTF-8', FALSE) . '</td><td>&nbsp;</td>
						</tr>');
				$c++;
			}
		}
	...
	public function getSingleField_typeSelect_singlebox($table, $field, $row, &$PA, $config, $selItems, $nMV_label) {
		...
		foreach ($selItems as $p) {
			...
			// Compile <option> tag:
			$opt[] = '<option value="' . htmlspecialchars($p[1]) . '"' . $sM . $nonSel . ($styleAttrValue ? ' style="' . htmlspecialchars($styleAttrValue) . '"' : '') . '>' . htmlspecialchars($p[0], ENT_COMPAT | ENT_HTML401, 'UTF-8', FALSE) . '</option>';
			$c++;
		}
		// Remaining values:
		if (count($itemArray) && !$PA['fieldTSConfig']['disableNoMatchingValueElement'] && !$config['disableNoMatchingValueElement']) {
			foreach ($itemArray as $theNoMatchValue => $temp) {
				// Compile <option> tag:
				array_unshift($opt, '<option value="' . htmlspecialchars($theNoMatchValue) . '" selected="selected">' . htmlspecialchars(@sprintf($nMV_label, $theNoMatchValue), ENT_COMPAT | ENT_HTML401, 'UTF-8', FALSE) . '</option>');
			}
		}
