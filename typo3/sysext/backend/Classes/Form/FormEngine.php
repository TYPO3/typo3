...
class FormEngine {
	...
	public function getSingleField($table, $field, $row, $altName = '', $palette = 0, $extra = '', $pal = 0) {
		...
		if ($PA['fieldConf']['displayCond']) {
			...
			$displayConditionResult = $elementConditionMatcher->match($PA['fieldConf']['displayCond']);
		}
		// Now, check if this field is configured and editable (according to excludefields + other configuration)
		if (is_array($PA['fieldConf']) && !$skipThisField && (!$PA['fieldConf']['exclude'] || $GLOBALS['BE_USER']->check('non_exclude_fields', $table . ':' . $field)) && $PA['fieldConf']['config']['form_type'] != 'passthrough' && ($this->RTEenabled || !$PA['fieldConf']['config']['showIfRTE']) && $displayConditionResult && (!$GLOBALS['TCA'][$table]['ctrl']['languageField'] || $PA['fieldConf']['l10n_display'] || strcmp($PA['fieldConf']['l10n_mode'], 'exclude') || $row[$GLOBALS['TCA'][$table]['ctrl']['languageField']] <= 0) && (!$GLOBALS['TCA'][$table]['ctrl']['languageField'] || !$this->localizationMode || $this->localizationMode === $PA['fieldConf']['l10n_cat'])) {
...
';
	...
	public function getSingleField_typeFlex($table, $field, $row, &$PA) {
		...
		if (is_array($dataStructArray)) {
			...
			/** @var $elementConditionMatcher \TYPO3\CMS\Backend\Form\ElementConditionMatcher */
			$elementConditionMatcher = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Backend\\Form\\ElementConditionMatcher');
			$elementConditionMatcher->setFlexformValueKey('vDef');
			foreach ($rotateLang as $lKey) {
				...
				foreach ($tabsToTraverse as $sheet) {
					...
					if ($dataStruct['ROOT']['TCEforms']['displayCond']) {
						...
						$displayConditionResult = TRUE;
						if ($dataStruct['ROOT']['TCEforms']['displayCond']) {
							$elementConditionMatcher->setRecord($fakeRow);
							$displayConditionResult = $elementConditionMatcher->match($dataStruct['ROOT']['TCEforms']['displayCond']);
						}
			...
			foreach ($dataStruct as $key => $value) {
				...
				if (is_array($value)) {
					...
					if ($value['type'] == 'array') {
						...
						/** @var $elementConditionMatcher \TYPO3\CMS\Backend\Form\ElementConditionMatcher */
						$elementConditionMatcher = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Backend\\Form\\ElementConditionMatcher');
						$elementConditionMatcher->setRecord($conditionData);
						foreach ($rotateLang as $vDEFkey) {
							$vDEFkey = 'v' . $vDEFkey;
							$displayConditionResult = TRUE;
							if ($value['TCEforms']['displayCond']) {
								$elementConditionMatcher->setFlexformValueKey($vDEFkey);
								$displayConditionResult = $elementConditionMatcher->match($value['TCEforms']['displayCond']);
							}
