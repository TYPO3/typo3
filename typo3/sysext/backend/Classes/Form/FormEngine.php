				...
				if (in_array($field, $this->hiddenFieldListArr)) {
					...
					if (isset($PA['fieldConf']['config']['mode']) && $PA['fieldConf']['config']['mode'] == 'useOrOverridePlaceholder') {
						...
						. '<span class="t3-tceforms-placeholder-override-checkbox">' .
							...
							'<input type="checkbox" name="' . $PA['itemFormElNameActive'] . '" value="1" id="tce-forms-textfield-use-override-' . $field . '-' . $row['uid'] . '" onchange="' . $onChange . '"' . $checked . ' /> <label for="tce-forms-textfield-use-override-' . $field . '-' . $row['uid'] . '">' . sprintf($GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.xlf:labels.placeholder.override'), $PA['label']) . '</label>' .
						'</span>'
