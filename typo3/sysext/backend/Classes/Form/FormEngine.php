...
';
	...
	public function dbFileIcons($fName, $mode, $allowed, $itemArray, $selector = '', $params = array(), $onFocus = '', $table = '', $field = '', $uid = '', $config = array()) {
		...
		if (!$params['readOnly'] && !$params['noList']) {
			...
			if (!$params['dontShowMoveIcons']) {
				if ($sSize >= 5) {
					$icons['L'][] = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-move-to-top', array(
						'data-fieldname' => $fName,
						'class' => 't3-btn t3-btn-moveoption-top',
						'title' => htmlspecialchars($this->getLL('l_move_to_top'))
					));
				}
				$icons['L'][] = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-move-up', array(
					'data-fieldname' => $fName,
					'class' => 't3-btn t3-btn-moveoption-up',
					'title' => htmlspecialchars($this->getLL('l_move_up'))
				));
				$icons['L'][] = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-move-down', array(
					'data-fieldname' => $fName,
					'class' => 't3-btn t3-btn-moveoption-down',
					'title' => htmlspecialchars($this->getLL('l_move_down'))
				));
				if ($sSize >= 5) {
					$icons['L'][] = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-move-to-bottom', array(
						'data-fieldname' => $fName,
		...
		if (!$params['readOnly'] && !$params['noDelete']) {
			$icons['L'][] = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-selection-delete', array(
	 ...
	 * @deprecated since TYPO3 6.1, remove two versions later. This is now done in an external file, see printNeededJSfunctions
	 */
