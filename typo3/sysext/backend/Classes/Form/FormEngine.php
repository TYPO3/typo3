...
class FormEngine {
	...
	public function getMainFields($table, array $row, $depth = 0, array $overruleTypesArray = array()) {
		...
		if ($out_sheet > 0) {
			...
			if (count($parts) > 1) {
				...
				$dividersToTabsBehaviour = $GLOBALS['TCA'][$table]['ctrl']['dividers2tabs'] ?: 1;
	...
	public function getSingleField_typeInput($table, $field, $row, &$PA) {
		...
		$size = \TYPO3\CMS\Core\Utility\MathUtility::forceIntegerInRange($config['size'] ?: 30, 5, $this->maxInputWidth);
