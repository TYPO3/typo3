...
class InlineElement {
	...
	public function renderForeignRecordHeader($parentUid, $foreign_table, $rec, $config, $isVirtualRecord = FALSE) {
		...
		// Try using a self-defined user function only for inline labels
		if (isset($GLOBALS['TCA'][$foreign_table]['ctrl']['inlineLabel_userFunc'])) {
			$params = array(
				'table' => $foreign_table,
				'row' => $rec,
				'title' => '',
				'isOnSymmetricSide' => $isOnSymmetricSide,
				'options' => isset($GLOBALS['TCA'][$foreign_table]['ctrl']['inlineLabel_userFunc_options']) ? $GLOBALS['TCA'][$foreign_table]['ctrl']['inlineLabel_userFunc_options'] : array(),
				'parent' => array(
					'uid' => $parentUid,
					'config' => $config
				)
			...
			$null = NULL;
			GeneralUtility::callUserFunction($GLOBALS['TCA'][$foreign_table]['ctrl']['inlineLabel_userFunc'], $params, $null);
			$recTitle = $params['title'];

		// Try using a normal self-defined user function
		} elseif ($GLOBALS['TCA'][$foreign_table]['ctrl']['label_userFunc']) {
			$params = array(
				'table' => $foreign_table,
				'row' => $rec,
				'title' => '',
