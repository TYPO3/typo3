...
class PageLayoutController {
		...
		foreach ($dblist->activeTables as $table => $value) {
			if (!isset($dblist->externalTables[$table])) {
				...
				if (is_array($GLOBALS['TCA']['tt_content']['columns']['colPos']['config']['items'])) {
					$colList = array();
					$tcaItems = \TYPO3\CMS\Core\Utility\GeneralUtility::callUserFunction('TYPO3\\CMS\\Backend\\View\\BackendLayoutView->getColPosListItemsParsed', $this->id, $this);
	...
	protected function getButtons($function = '') {
		...
		if (!$this->modTSconfig['properties']['disableIconToolbar']) {
			...
			if ($this->CALC_PERMS & 2 && $GLOBALS['BE_USER']->checkLa
nguageAccess(0)) {
