...
class LiveSearch
{
    ...
    protected function getEditLink($tableName, $row)
    {
        ...
        $editLink = '';
        if ($tableName === 'pages') {
            $permsEdit = $backendUser->calcPerms(BackendUtility::getRecord('pages', $row['uid']) ?? []) & Permission::PAGE_EDIT;
        } else {
            $permsEdit = $backendUser->calcPerms(BackendUtility::readPageAccess($row['pid'], $this->userPermissions) ?: []) & Permission::CONTENT_EDIT;
        }
