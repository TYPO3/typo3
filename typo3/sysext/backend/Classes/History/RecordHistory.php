...
class RecordHistory {
	...
	public function displayHistory() {
		...
		foreach ($this->changeLog as $sysLogUid => $entry) {
			...
			if (!$entry['action']) {
				if ($entry['snapshot']) {
                    $image =  IconUtility::getSpriteIcon('actions-unmarkstate', array('title' => $GLOBALS['LANG']->getLL('unmarkState',1)), array());
				} else {
                    $image =  IconUtility::getSpriteIcon('actions-markstate', array('title' => $GLOBALS['LANG']->getLL('markState',1)), array());
				}
