...
class ExtdirectTreeDataProvider extends \TYPO3\CMS\Backend\Tree\AbstractExtJsTree {
	...
	public function getLanguages() {
		$languages = \TYPO3\CMS\Backend\Utility\BackendUtility::getSystemLanguages(TRUE);
		if ($GLOBALS['BE_USER']->getTSConfigVal('options.pageTree.sortLanguageSelectionAlphabetically') == 1) {
		...
		foreach ($languages as $language) {
			if ($GLOBALS['BE_USER']->checkLanguageAccess($language[1])) {
				...
				$systemLanguages[] = array(
					...
					'icon' => $language[2],
					'iconCls' => \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIconClasses($language[2])
				);
			}
		}
