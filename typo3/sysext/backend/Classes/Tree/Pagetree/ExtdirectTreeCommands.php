...
class ExtdirectTreeCommands {
	...
	public function visiblyNode($nodeData) {
		...
		try {
			...
			$newNode = Commands::getNode($node->getId(), TRUE, $node->getLanguage());
	...
	public function visiblyNodeOverlay($nodeData) {
		...
		try {
			Commands::visiblyNodeOverlay($node);
			$newNode = Commands::getNode($node->getId(), TRUE, $node->getLanguage());
			$newNode->setLeaf($node->isLeafNode());
			$returnValue = $newNode->toArray();
		} catch (\Exception $exception) {
	...
	public function moveNodeToFirstChildOfDestination($nodeData, $destination) {
		...
		try {
			...
			$newNode = Commands::getNode($node->getId(), FALSE, $node->getLanguage());
			$newNode->setLeaf($node->isLeafNode());
			$returnValue = $newNode->toArray();
		} catch (\Exception $exception) {
			$returnValue = array(
	...
	public function copyNodeToFirstChildOfDestination($nodeData, $destination) {
		...
		try {
			...
			$newNode = Commands::getNode($newPageId, TRUE, $node->getLanguage());
			$newNode->setLeaf($node->isLeafNode());
			$returnValue = $newNode->toArray();
		} catch (\Exception $exception) {
			$returnValue = array(
	...
	static public function addRootlineOfNodeToStateHash($stateId, $nodeId) {
		...
		$userSettings = GeneralUtility::makeInstance('extDirect_DataProvider_BackenduserSettings');
