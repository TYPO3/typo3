...
class TsconfigWizard {
	...
	public function browseTSprop($mode, $show) {
		...
		if ($show) {
			...
			$out .= '<a href="' . htmlspecialchars(GeneralUtility::linkThisScript(array('show' => ''))) . '" class="typo3-goBack">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-view-go-back') . htmlspecialchars($obj_string) . '</a><br />';
			if ($rec['title']) {
