	...
	public function colPosListItemProcFunc(&$params) {
		...
		if (is_array($hooks)) {
			foreach ($hooks as $hookClass) {
				...
				if (!($hookObject instanceof tx_cms_BackendLayoutProcessor)) {
					throw new UnexpectedValueException(get_class($hookObject) . ' must implement interface tx_cms_BackendLayoutProcessor', 1326311344);
				}
				$hookObject->postProcessColumnPositionProcessingFunctionItems($params);
			}
		}
	}
	...
	public function getColPosListItemsParsed($id) {
		...
		if (is_array($hooks)) {
			foreach ($hooks as $hookClass) {
				...
				$hookObject->postProcessColumnPositionListItemsParsed($id, $tcaItems, $tceForms);
			}
		}
		return $tcaItems;
	}
	...
	public function getSelectedBackendLayout($id) {
		...
		if (is_array($hooks)) {
			foreach ($hooks as $hookClass) {
				...
				$hookObject->postProcessBackendLayout($id, $backendLayout);
			}
		}
