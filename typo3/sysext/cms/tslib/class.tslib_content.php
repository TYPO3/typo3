				...
				if ($pU['scheme'] || ($isLocalFile != 1 && $urlChar && (!$containsSlash || $urlChar < $fileChar))) { // url (external): If doubleSlash or if a '.' comes before a '/'.
					...
					if (count($page)) {
						...
						if ($conf['useCacheHash']) {
							...
							if (trim($params, '& ') != '') {
								/** @var $cachingChash tslib_caching_CacheHash */
								$tslibCacheHashInstance = t3lib_div::makeInstance('tslib_caching_CacheHash');
+								$addQueryParams .= '&cHash=' . $tslibCacheHashInstance->generateCacheHashForGetQuery($params);
							}
							unset($params);
						}
