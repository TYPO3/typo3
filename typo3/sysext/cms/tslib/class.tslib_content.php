		...
		if (@is_file($theFile)) {
			...
			if ($conf['icon']) {
				$conf['icon.']['path'] = isset($conf['icon.']['path.'])
					...
					: $conf['icon.']['path'];
				$iconP = isset($conf['icon.']['path']) ? $conf['icon.']['path'] : t3lib_extMgm::siteRelPath('cms') . 'tslib/media/fileicons/';
				$conf['icon.']['ext'] = isset($conf['icon.']['ext.'])
					? $this->stdWrap($conf['icon.']['ext'], $conf['icon.']['ext.'])
					: $conf['icon.']['ext'];
				$iconExt = isset($conf['icon.']['ext']) ? '.' . $conf['icon.']['ext'] : '.gif';
				...
				if ($fI['fileext'] && t3lib_div::inList($image_ext_list, $fI['fileext'])) {
					...
					$iconWidth = isset($conf['icon.']['widthAttribute']) ? $conf['icon.']['widthAttribute'] : 18;
					...
					$iconHeight = isset($conf['icon.']['heightAttribute']) ? $conf['icon.']['heightAttribute'] : 16;
					$icon = '<img src="' . htmlspecialchars($GLOBALS['TSFE']->absRefPrefix . $icon) .
