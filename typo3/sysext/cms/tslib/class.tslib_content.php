	...
	function filelink($theValue, $conf) {
		...
		if (@is_file($theFile)) {
			...
			if ($conf['icon']) {
				...
				$iconP = isset($conf['icon.']['path.'])
					? $this->stdWrap($iconP, $conf['icon.']['path.'])
					: $iconP;
				$iconExt = $conf['icon.']['ext'] ? '.' . $conf['icon.']['ext'] : '.gif';
				$iconExt = isset($conf['icon.']['ext.'])
					? $this->stdWrap($iconExt, $conf['icon.']['ext.'])
					: $iconExt;
				...
				if ($fI['fileext'] && t3lib_div::inList($image_ext_list, $fI['fileext'])) {
					...
					$iconWidth = isset($conf['icon.']['widthAttribute.'])
						? $this->stdWrap($iconWidth, $conf['icon.']['widthAttribute.'])
						: $iconWidth;
					$iconHeight = $conf['icon.']['heightAttribute'] ? $conf['icon.']['heightAttribute'] : 16;
					$iconHeight = isset($conf['icon.']['heightAttribute.'])
						? $this->stdWrap($iconHeight, $conf['icon.']['heightAttribute.'])
						: $iconHeight;
					$icon = '<img src="' . htmlspecialchars($GLOBALS['TSFE']->absRefPrefix . $icon) .
