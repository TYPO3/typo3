...
class tslib_menu {
	...
	protected function sectionIndex($altSortField, $pid = NULL) {
		...
		if ($GLOBALS['TSFE']->sys_language_contentOL) {
			$selectSetup['where'] .= ' AND sys_language_uid IN (-1, 0)';
		} else {
			$selectSetup['where'] .= ' AND sys_language_uid = ' . $GLOBALS['TSFE']->sys_language_content . ' AND l18n_parent = 0';
		}
...
class tslib_imgmenu extends tslib_menu {
	...
	function writeMenu() {
		if ($this->result) {
			$res = &$this->result;
				// shortMD5 260900
			$menuName = 'menu_'.t3lib_div::shortMD5($res['imgMap']);
