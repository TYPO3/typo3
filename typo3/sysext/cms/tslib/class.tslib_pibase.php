...
class tslib_pibase {
	 ...
	 */
	function pi_getLL($key,$alternativeLabel='',$hsc=FALSE)	{
		...
		if (!$this->LOCAL_LANG_loaded && $this->scriptRelPath)	{
			...
			if (is_array($confLL)) {
				foreach ($confLL as $languageKey => $languageArray) {
						// Dont process label if the langue is not loaded
					...
					if (is_array($languageArray) && is_array($this->LOCAL_LANG[$languageKey])) {
						...
						foreach($languageArray as $labelKey => $labelValue)	{
							if (!is_array($labelValue))	{
								...
								$this->LOCAL_LANG_charset[$languageKey][$labelKey] = $GLOBALS['TYPO3_CONF_VARS']['BE']['forceCharset'] ? $GLOBALS['TYPO3_CONF_VARS']['BE']['forceCharset'] : $GLOBALS['TSFE']->csConvObj->charSetArray[$languageKey];
							}
						}
					}
				}
