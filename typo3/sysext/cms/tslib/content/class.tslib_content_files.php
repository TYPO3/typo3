...
class tslib_content_Files extends tslib_content_Abstract {
	...
	public function render($conf = array()) {
		...
		if ($conf['references'] || $conf['references.']) {
			...
			*/
			...
			$referencesFieldName = $this->stdWrapValue('fieldName', $conf['references.']); // it's important that this always stays "fieldName" and not be renamed to "field" as it would otherwise collide with the stdWrap key of that name
		...
		if ($conf['files'] || $conf['files.']) {
			...
			*/
			...
			$fileUids = t3lib_div::trimExplode(',', $this->stdWrapValue('files', $conf));
		...
		if ($conf['collections'] || $conf['collections.']) {
			$collectionUids = t3lib_div::trimExplode(',', $this->stdWrapValue('collections', $conf));
	 ...
	 *
	 * @param $key					The config variable key (from TS array).
