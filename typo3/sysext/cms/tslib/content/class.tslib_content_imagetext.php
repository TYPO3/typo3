	...
	public function render($conf = array()) {
		...
		if ($imgList) {
			...
			if ($conf['captionSplit'] && $conf['captionSplit.']['cObject']) {
				...
				$captionArray = explode($capSplit, $this->cObj->cObjGetSingle(
					...
					if ($b && $colspacing) { // If this is NOT the first iteration AND if column space is required. In case of "noCols", the space is done without a separate cell.
						if (!$noCols) {
							...
							$colSpacer = '<img src="' . $GLOBALS['TSFE']->absRefPrefix . 'clear.gif" width="' .
								...
								$this->cObj->getBorderAttr(' border="0"') . ' align="' .
			...
			switch ($contentPosition) {
				case '0' : // above
					$output = '<div style="text-align:' . $align . ';">' . $tablecode . '</div>' .
						$this->cObj->wrapSpace($content, $spaceBelowAbove . '|0');
				break;
				case '8' : // below
					$output = $this->cObj->wrapSpace($content, '0|' . $spaceBelowAbove) .
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['tslib/content/class.tslib_content_imagetext.php']) {
	include_once ($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['tslib/content/class.tslib_content_imagetext.php']);
}
