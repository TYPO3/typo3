...
class tslib_content_Media extends tslib_content_Abstract {
	...
	public function render($conf = array()) {
		...
		$conf['predefined'] = array();
			...
			// Width and height
		$conf['width'] = $this->doFlexFormOverlay($conf, 'width');
		$conf['height'] = $this->doFlexFormOverlay($conf, 'height');
	 ...
	 * @return string|NULL
	 */
	protected function retrieveMediaUrl($fileName) {
		...
		$mediaWizard = tslib_mediaWizardManager::getValidMediaWizardProvider($fileName);
		if ($mediaWizard !== NULL) {
			$returnValue = $this->cObj->typoLink_URL(array('parameter' => $mediaWizard->rewriteUrl($fileName)));
		}
