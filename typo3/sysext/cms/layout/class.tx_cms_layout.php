...
class tx_cms_layout extends recordList {
					...
					foreach ($rowArr as $rKey => $row) {
						...
						if (is_array($row) && (int) $row['t3ver_state'] != 2) {
							...
							$singleElementHTML .= '<div class="t3-page-ce-body-inner">' .
														$innerContent .
													'</div>' .
													$this->tt_content_drawFooter($row, $this->tt_contentConfig['showInfo'] ? 15 : 5, $disableMoveAndNewButtons, TRUE);
							...
							if (!$disableMoveAndNewButtons) {
								...
								$singleElementHTML .= '<div style="margin-bottom:7px;margin-left:5px;"><a href="#" onclick="' . htmlspecialchars($onClick) . '" title="' . $GLOBALS['LANG']->getLL('newAfter', 1) . '">' .
	 ...
	 */
	function tt_content_drawFooter($row, $space = 0, $disableMoveAndNewButtons = FALSE, $langMode = FALSE) {
		$content = '';
...
//		GRI!! still open
//		$out .= 'xxx' . htmlspecialchars($this->CType_labels[$row['CType']]) .
