...
class tx_cms_layout extends recordList {
	...
	var $agePrefixes = ' min| hrs| days| yrs| min| hour| day| year';
	...
	/**
	 * Draw droptarget area
	 *
	 * @param integer $id
	 */
	public function drawDroptargetArea($identifier = NULL) {
		return '
			<div class="t3-page-ce-droptarget" style="margin-bottom:7px;margin-left:5px;" id="t3-page-ce-droptarget-' . $identifier .'">
				this is a droparea
			</div>
		';
	}
			...
			foreach ($langListArr as $lP) {
				...
				foreach ($cList as $key) {
					...
					';
						...
						// Droparea
					#$content[$key] .= $this->drawDroptargetArea($id);
					...
					foreach ($rowArr as $rKey => $row) {
						...
						if (is_array($row) && (int) $row['t3ver_state'] != 2) {
							...
							$content[$key] .= $this->drawDroptargetArea($row['uid']);
