...
class tx_cms_layout extends recordList {
	...
	function dataFields($fieldArr, $table, $row, $out = array()) {
		...
		if ($GLOBALS['TCA'][$table]) {
			...
			foreach ($fieldArr as $fieldName) {
				...
				if ($GLOBALS['TCA'][$table]['columns'][$fieldName]) { // Each field has its own cell (if configured in TCA)
	...
	function isDisabled($table, $row) {
		if (
			($GLOBALS['TCA'][$table]['ctrl']['enablecolumns']['disabled'] && $row[$GLOBALS['TCA'][$table]['ctrl']['enablecolumns']['disabled']]) ||
	...
	function getPageInfoBox($rec, $edit = 0) {
		...
		if ($this->pI_showStat && t3lib_extMgm::isLoaded('sys_stat')) {
			...
			if ($count) {
				...
				$lines[] = '';
				$lines[] = array($GLOBALS['LANG']->getLL('pI_hitsPeriod') . ':', t3lib_BEfunc::date($rrow2[0]) . ' - ' . t3lib_BEfunc::date($rrow2[1]) . ' (' . t3lib_BEfunc::calcAge($rrow2[1] - $rrow2[0], $this->agePrefixes) . ')');
		...
		foreach ($theTables as $tName) {
			...
			if ($GLOBALS['BE_USER']->check('tables_select', $tName) && (t3lib_extMgm::isLoaded($tName) || t3lib_div::inList('fe_users,tt_content', $tName) || isset($this->externalTables[$tName]))) {
				...
				if ($c || t3lib_div::inList('tt_content', $tName)) {
					...
					$out .= '
					<td><a href="#' . $tName . '"></a>' .
							t3lib_iconWorks::getSpriteIconForRecord($tName, Array(), array('title' => $GLOBALS['LANG']->sL($GLOBALS['TCA'][$tName]['ctrl']['title'], 1))) .
							'</td>';

					// ... and to the internal array, activeTables we also add table icon and title (for use elsewhere)
					$this->activeTables[$tName] =
							t3lib_iconWorks::getSpriteIconForRecord($tName, Array(), array('title' => $GLOBALS['LANG']->sL($GLOBALS['TCA'][$tName]['ctrl']['title'], 1) . ': ' . $c . ' ' . $GLOBALS['LANG']->getLL('records', 1))) .
									'&nbsp;' .
