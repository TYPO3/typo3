					...
					foreach ($rowArr as $rKey => $row) {
						...
						if (is_array($row) && (int) $row['t3ver_state'] != 2) {
							...
							$singleElementHTML .= '<div class="t3-page-ce-body-inner">' .
														$innerContent .
													'</div>' .
													$this->tt_content_drawFooter($row, $this->tt_contentConfig['showInfo'] ? 15 : 5, $this->defLangBinding && $lP > 0, TRUE);
...
//				' <strong>' . $GLOBALS['LANG']->sL(t3lib_BEfunc::getLabelFromItemlist('tt_content', 'list_type', $row['list_type']), 1) . '</strong><br />';
	 ...
	 * @return void
	 */
	function getProcessedValue($table, array $fieldList, array $row, array &$info) {
