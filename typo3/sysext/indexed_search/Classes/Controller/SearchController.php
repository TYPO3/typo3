...
class Tx_IndexedSearch_Controller_SearchController extends Tx_Extbase_MVC_Controller_ActionController {
	...
	function makeItemTypeIcon($imageType, $alt, $specRowConf) {
		...
		if ($imageType === '0' && $specRowConf['_pid'] && is_array($specRowConf['pageIcon.']) && !is_array($this->conf['iconRendering.'])) {
		...
		if (!isset($this->iconFileNameCache[$imageType])) {
			...
			if (is_array($this->conf['iconRendering.'])) {
				...
				$this->iconFileNameCache[$imageType] = $this->cObj->cObjGetSingle($this->conf['iconRendering'], $this->conf['iconRendering.']);
				// ... otherwise, get flag from sys_language record:
			} else {
...
// debug($parts,$regExString);
	 ...
	 *
	 * @param string $str String to convert (utf-8)
	 ...
	 * @param integer $freeIndexUid Pointing to free indexing configurations to search. -1 represents no filtering, 0 represents TYPO3 pages only, any number above zero is a uid of an indexing configuration!
	 * @return string HTML output
	 */
	function pi_list_browseresults($showResultCount = TRUE, $addString = '', $addPart = '', $freeIndexUid = -1) {
