...
class Tx_IndexedSearch_Domain_Repository_IndexSearchRepository {
	...
	public function initialize($settings, $searchData, $externalParsers, $searchRootPageIdList) {
		...
		$this->numberOfResults = $searchData['numberOfResults'];
	}
	 ...
	 * @param	integer		Pointer to which indexing configuration you want to 
	 * 						search in. -1 means no filtering. 0 means only regular indexed content.
	 * @return	pointer
	 */
			...
			switch ($theType) {
				...
				case '20':
					...
					$res = $this->searchSentence($sWord);
						...
						// sentence in the word-table). So therefore mtime is used instaed. 
	...
	protected function sectionTableWhere() {
		...
		if (substr($this->sections, 0, 4) == 'rl1_') {
			...
			$match = TRUE;
		} else if (substr($this->sections, 0, 4) == 'rl2_') {
			$list = implode(',', t3lib_div::intExplode(',', substr($this->sections, 4)));
			$whereClause .= ' AND ISEC.rl2 IN (' . $list . ')';
			$match = TRUE;
		} else if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['indexed_search']['addRootLineFields'])) {
