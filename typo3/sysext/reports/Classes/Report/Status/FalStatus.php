...
class FalStatus implements \TYPO3\CMS\Reports\StatusProviderInterface {
	...
	protected function getMissingFilesStatus() {
		...
		// todo: only count missing files in active storages
		$count = $GLOBALS['TYPO3_DB']->exec_SELECTcountRows('*', 'sys_file', 'missing=1');
		if ($count) {
			$value = sprintf($GLOBALS['LANG']->getLL('status_missingFilesCount'), $count);
			$severity = \TYPO3\CMS\Reports\Status::WARNING;

			// todo:
			// - display storage + file indentifier
			// - add notice on how to fix
			// - add link + functionality to delete the sys_file records etc
			$message = '';
		}
