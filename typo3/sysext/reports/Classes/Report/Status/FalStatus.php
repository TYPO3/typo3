 ...
 *  Copyright notice
 *
 *  (c) 2009-2013 Frans Saris <franssaris@gmail.com>
 *  All rights reserved
 *
...
class FalStatus implements \TYPO3\CMS\Reports\StatusProviderInterface {
	...
	protected function getMissingFilesStatus() {
		...
		// only need to check missing files when there are storages online
		if (count($storages)) {
			$count = $GLOBALS['TYPO3_DB']->exec_SELECTcountRows('*', 'sys_file', 'missing=1 AND storage IN (' . implode(',', array_keys($storages)) . ')');
		}
		...
		if ($count) {
			...
			$files = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('identifier,storage', 'sys_file', 'missing=1 AND storage IN (' . implode(',', array_keys($storages)) . ')', '', '', $maxFilesToShow);
