...
class SystemStatus implements \TYPO3\CMS\Reports\StatusProviderInterface {
	 ...
	 *
	 * @return \TYPO3\CMS\Reports\Report\Status\Status A status of whether a minimum PHP version requirement is met
	 */
	protected function getPhpStatus() {
		$message = '';
		$severity = \TYPO3\CMS\Reports\Report\Status\Status::OK;
		if (version_compare(phpversion(), TYPO3_REQUIREMENTS_MINIMUM_PHP) < 0) {
			$message = $GLOBALS['LANG']->getLL('status_phpTooOld');
			$severity = \TYPO3\CMS\Reports\Report\Status\Status::ERROR;
		}
		return \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Reports\\Report\\Status\\Status', $GLOBALS['LANG']->getLL('status_phpVersion'), phpversion(), $message, $severity);
	}

	/**
	 * Checks the current memory limit against a minimum required version.
	 *
	 * @return \TYPO3\CMS\Reports\Report\Status\Status A status of whether a minimum memory limit requirement is met
	 */
	protected function getPhpMemoryLimitStatus() {
		...
		if ($memoryLimitBytes > 0) {
			if ($memoryLimitBytes < \TYPO3\CMS\Core\Utility\GeneralUtility::getBytesFromSizeMeasurement(TYPO3_REQUIREMENTS_RECOMMENDED_PHP_MEMORY_LIMIT)) {
				...
				$severity = \TYPO3\CMS\Reports\Report\Status\Status::WARNING;
			}
			...
			if ($severity > \TYPO3\CMS\Reports\Report\Status\Status::OK) {
		...
		return \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Reports\\Report\\Status\\Status', $GLOBALS['LANG']->getLL('status_phpMemory'), $memoryLimitBytes > 0 ? $memoryLimit : $GLOBALS['LANG']->getLL('status_phpMemoryUnlimited'), $message, $severity);
	}
	...
	protected function getPhpPeakMemoryStatus() {
		...
		return \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Reports\\Report\\Status\\Status', $GLOBALS['LANG']->getLL('status_phpPeakMemory'), $value, $message, $severity);
	}

	/**
	 * Reports the webserver TYPO3 is running on.
	 *
	 * @return \TYPO3\CMS\Reports\Report\Status\Status The server software as a status
	 */
	protected function getWebserverStatus() {
		return \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Reports\\Report\\Status\\Status', $GLOBALS['LANG']->getLL('status_webServer'), $_SERVER['SERVER_SOFTWARE']);
	}

	/**
	 * Reports whether any of the required PHP modules are missing
	 *
	 * @return \TYPO3\CMS\Reports\Report\Status\Status A status of missing PHP modules
	 */
	protected function getMissingPhpModules() {
		...
		return \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Reports\\Report\\Status\\Status', $GLOBALS['LANG']->getLL('status_phpModules'), $value, $message, $severity);
	}
