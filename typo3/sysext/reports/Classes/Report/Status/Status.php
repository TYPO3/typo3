...
class Status implements \TYPO3\CMS\Reports\ReportInterface {
	...
	public function getHighestSeverity(array $statusCollection) {
		$highestSeverity = \TYPO3\CMS\Reports\Report\Status\Status::NOTICE;
		foreach ($statusCollection as $statusProvider => $providerStatuses) {
			foreach ($providerStatuses as $status) {
				...
				if ($highestSeverity == \TYPO3\CMS\Reports\Report\Status\Status::ERROR) {
					break;
				}
			}
		}
	...
	protected function renderStatus(array $statusCollection) {
		...
		foreach ($statuses as $provider => $providerStatus) {
			...
			$classes = array(
				\TYPO3\CMS\Reports\Report\Status\Status::NOTICE => 'notice',
				\TYPO3\CMS\Reports\Report\Status\Status::INFO => 'information',
				\TYPO3\CMS\Reports\Report\Status\Status::OK => 'ok',
				\TYPO3\CMS\Reports\Report\Status\Status::WARNING => 'warning',
				\TYPO3\CMS\Reports\Report\Status\Status::ERROR => 'error'
			);
			$icon[\TYPO3\CMS\Reports\Report\Status\Status::WARNING] = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('status-dialog-warning');
			$icon[\TYPO3\CMS\Reports\Report\Status\Status::ERROR] = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('status-dialog-error');
			$messages = '';
			$headerIcon = '';
			$sectionSeverity = 0;
