...
/**
 * Contains the update class for adding outsourced / new system extensions.
 *
...
class tx_coreupdates_installsysexts extends Tx_Install_Updates_Base {
	 ...
	 * @param boolean $downloadIfNotExist Download extension from repository if not exist locally
	 * @return void
	 */
	protected function installExtensions($extensionKeys) {
		$extensionManagerConnection = $this->getExtensionManagerConnection();
		$installedExtensions = $extensionManagerConnection->getFlatExtensionList();
		foreach ($extensionKeys as $extensionKey) {
			if (empty($installedExtensions['data'][$extensionKey]['installed'])) {
				...
				$result = $extensionManagerConnection->installExtension($t3xContent, 'L', $extensionDetails['versionString'], '', FALSE);
				if (empty($result)) {
					continue;
				}
			}
	...
	protected function fetchUrl($url) {
		...
		$fileContent = t3lib_div::getUrl($url, 0, array(TYPO3_user_agent));
		...
		if ($fileContent === FALSE) {
			throw new Exception('Can not fetch URL "' . $url . '". Possibile reasons are network problems or misconfiguration.', 1344685036);
		}
