...
class InstallBootstrap {
	...
	static public function dieWithExceptionMessage(\Exception $exception) {
		self::dieWithMessage(
			...
			'
				<p>
					The Install Tool have a problem, maybe you can fix it, so please read the error message carefully.
				</p>
				<ul>
					<li>
				...
				<p>
					Mostly the problem is a missing write permission for the webserver user to the typo3conf and/or
					typo3temp directory.
				</p>
			'
			. ($exception->getCode() > 0
				? '<p>More information regarding this error might be available <a href="'
					. TYPO3_URL_EXCEPTION . rawurlencode($exception->getCode()) . '" target="_blank">online</a>.</p>'
				: ''
			)
		);
	}
