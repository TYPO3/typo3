...
class Check {
	...
	protected function checkMaximumFileUploadSize() {
		...
		if ($maximumUploadFilesize < 1024 * 1024 * 10) {
			...
			$status->setMessage(
				...
				' It is recommended that the value of upload_max_filesize equals to the value' .
	...
	protected function checkMemorySettings() {
		...
		if ($memoryLimit <= 0) {
			...
			$status->setMessage(
				...
				' To be safe, set a limit in PHP, but with a minimum of ' . $recommendedMemoryLimit . 'MB:
			...
			$status->setMessage(
				...
				' We recommend a minimum PHP memory limit of ' . $recommendedMemoryLimit . 'MB:
				memory_limit=' . $recommendedMemoryLimit . 'M'
			);
			...
			$status->setMessage(
				...
				' To be on the safe side, we recommend a minimum PHP memory limit of ' . $recommendedMemoryLimit . 'MB:
				memory_limit=' . $recommendedMemoryLimit . 'M'
			);
		} else {
			$status = new Status\OkStatus();
	...
	protected function checkMaxExecutionTime() {
		...
		if ($currentMaximumExecutionTime == 0) {
			if (PHP_SAPI === 'cli') {
				...
				$status->setTitle('Infinite PHP script execution time');
				$status->setMessage(
					'max_execution_time=' . $currentMaximumExecutionTime . '
					...
					' if it is reduced to ' . $recommendedMaximumExecutionTime . ' seconds:
			...
			$status->setMessage(
				...
				' It is recommended to raise the limit to ' . $recommendedMaximumExecutionTime . ' seconds:
				max_execution_time=' . $recommendedMaximumExecutionTime
			);
			...
			$status->setMessage(
				...
				' and maybe raise the limit to ' . $recommendedMaximumExecutionTime . ' seconds:
				max_execution_time=' . $recommendedMaximumExecutionTime
			);
		} else {
			$status = new Status\OkStatus();
	...
	protected function checkDisableFunctions() {
		...
		if (strlen($disabledFunctions) > 0 && count($disabledFunctionsArray) > 0) {
			...
			$status->setMessage(
				'disable_functions=' . implode(' ', explode(',', $disabledFunctions)) . '
	...
	protected function checkSafeMode() {
		...
		if ($safeModeEnabled) {
			...
			$status->setMessage(
				'PHP safe_mode enabled. This is unsupported by TYPO3 CMS, it must be turned off:
				safe_mode=Off'
			);
		} else {
			$status = new Status\OkStatus();
	...
	protected function checkXdebugMaxNestingLevel() {
		if (extension_loaded('xdebug')) {
			...
			if ($currentMaxNestingLevel < $recommendedMaxNestingLevel) {
				...
				$status->setMessage(
					'xdebug.max_nesting_level=' . $currentMaxNestingLevel . '
					...
					' if the default value of 100 is not raised significantly to:
	...
	protected function checkSuhosinRequestMaxVars() {
		...
		if ($this->isSuhosinLoaded()) {
			...
			if ($currentRequestMaxVars < $recommendedRequestMaxVars) {
				...
				$status->setMessage(
					'suhosin.request.max_vars=' . $currentRequestMaxVars . '
	...
	protected function checkSuhosinPostMaxVars() {
		...
		if ($this->isSuhosinLoaded()) {
			...
			if ($currentPostMaxVars < $recommendedPostMaxVars) {
				...
				$status->setMessage(
					'suhosin.post.max_vars=' . $currentPostMaxVars . '
					This setting can lead to lost information if submitting big forms in TYPO3 CMS like' .
					' it is done in the install tool. It is heavily recommended to raise this' .
	...
	protected function checkSuhosinGetMaxValueLength() {
		...
		if ($this->isSuhosinLoaded()) {
			...
			if ($currentGetMaxValueLength < $recommendedGetMaxValueLength) {
				...
				$status->setMessage(
					'suhosin.get.max_value_length=' . $currentGetMaxValueLength . '
					This setting can lead to lost information if submitting big forms in TYPO3 CMS like' .
					' it is done in the install tool. It is heavily recommended to raise this' .
	...
	protected function checkSuhosinExecutorIncludeWhiteListContainsPhar() {
		if ($this->isSuhosinLoaded()) {
			...
			if (!in_array('phar', $currentWhiteListArray)) {
				...
				$status->setMessage(
					'suhosin.executor.include.whitelist= ' . implode(' ', $currentWhiteListArray) . '
					"phar" is currently not a hard requirement of TYPO3 CMS but is nice to have and a possible requirement' .
					' in future versions. A useful setting is:
	...
	protected function checkWindowsApacheThreadStackSize() {
		if (
			...
			$status->setMessage(
				...
				' to httpd.conf and restart Apache afterwards:
	...
	protected function checkPhpMagicQuotes() {
		...
		if ($magicQuotesGpc) {
			...
			$status->setMessage(
				'magic_quotes_gpc=' . $magicQuotesGpc . '
				Setting magic_quotes_gpc is deprecated since PHP 5.3.' .
				' You are advised to disable it until it gets completely removed:
