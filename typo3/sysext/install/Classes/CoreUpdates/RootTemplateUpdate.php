 ...
 */
class RootTemplateUpdate extends \TYPO3\CMS\Install\Updates\InitializeUpdate {
	...
	public function checkForUpdate(&$description, &$showUpdate = 0) {
		...
		if ($pages !== NULL && $this->findRootTemplates(array_keys($pages)) !== NULL) {
			$description = 'There is already at least on TypoScript root template available.';
			$showUpdate = 0;
		} else {
			$description = 'There is no TypoScript root template which is however required for a correct Extbase behaviour.';
			$showUpdate = 1;
		}
