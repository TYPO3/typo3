			...
			if ($error !== NULL) {
				...
				if ($errorType & (E_ERROR | E_PARSE | E_CORE_ERROR | E_COMPILE_ERROR | E_USER_ERROR | E_RECOVERABLE_ERROR)) {
					...
					if (!headers_sent()) {
						...
						echo '
<p><strong>
	The system determined a fatal error during the include of extension configurations.
	Please go to the <a href="' . $redirectLocation . '">extension check tool</a> which finds all incompatible extension configurations
	and offers the possibility to uninstall broken extensions.
</strong></p>';
					}
				}
			}
