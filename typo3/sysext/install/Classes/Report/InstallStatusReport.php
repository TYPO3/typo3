...
class InstallStatusReport implements \TYPO3\CMS\Reports\StatusProviderInterface {
	...
	protected function getFileSystemStatus() {
		...
		foreach ($checkWritable as $relPath => $requirementLevel) {
			...
			if (!@is_dir((PATH_site . $relPath))) {
				if ($requirementLevel > 0) {
					...
					$message .= '<br />';
					if ($severity < \TYPO3\CMS\Reports\Report\Status\Status::WARNING) {
		...
		return \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Reports\\Report\\Status\\Status', $GLOBALS['LANG']->sL('LLL:EXT:install/report/locallang.xml:status_fileSystem'), $value, $message, $severity);
	}
	...
	protected function getRemainingUpdatesStatus() {
		...
		return \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Reports\\Report\\Status\\Status', $GLOBALS['LANG']->sL('LLL:EXT:install/report/locallang.xml:status_remainingUpdates'), $value, $message, $severity);
	}
