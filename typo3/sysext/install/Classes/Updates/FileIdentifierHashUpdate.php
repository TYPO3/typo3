...
class FileIdentifierHashUpdate extends AbstractUpdate {
	 ...
	 * @var string
	 */
	protected $title = 'Add the file identifier hash to existing sys_file records.';
	...
	public function checkForUpdate(&$description) {
		...
		$unhashedFileCount = $this->db->exec_SELECTcountRows(
			'uid',
			'sys_file',
			'identifier_hash = "" ' . \TYPO3\CMS\Backend\Utility\BackendUtility::deleteClause('sys_file')
		);
		return $unhashedFileCount > 0 && !$this->isWizardDone();
	}
	...
	public function performUpdate(array &$dbQueries, &$customMessages) {
		...
		// TODO removed storages are not respected here
		...
		if (!ExtensionManagementUtility::isLoaded('dbal')) {
			...
			foreach ($files as $file) {
				...
				$queries[] = $query = $this->db->UPDATEquery(
					'sys_file', 'uid=' . $file['uid'], array('identifier_hash' => $hash)
				);
