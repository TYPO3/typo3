...
class TtContentUploadsUpdateWizard extends AbstractUpdate {
	...
	/**
	 * @var \TYPO3\CMS\Core\Resource\Index\IndexRecordRepository
	 */
	...
	protected function init() {
		...
		$this->indexRecordRepository = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Core\\Resource\\Index\\IndexRecordRepository');
...
', $record['titleText']);
		...
		foreach ($files as $file) {
			if (file_exists(PATH_site . 'uploads/media/' . $file)) {
				...
				$this->indexRecordRepository->addToIndex($fileObject);
				$dataArray = array(
