...
class RteMagicImagesUpdateWizard extends \TYPO3\CMS\Install\Updates\AbstractUpdate {
	...
	public function __construct() {
		...
		if ($GLOBALS['TYPO3_CONF_VARS']['BE']['RTE_imageStorageDir']) {
	...
	public function checkForUpdate(&$description) {
		$description = 'This update wizard goes through all magic images, located in ' . dirname($this->oldPrefix) . '., and moves the files to fileadmin/_migrated_/RTE/.<br />It also moves the files from uploads/ to the fileadmin/_migrated/ path.';
		// this wizard is only available
		if ($this->oldPrefix) {
	...
	public function performUpdate(array &$dbQueries, &$customMessages) {
		...
		foreach ($oldRecords as $refRecord) {
			...
			$sourceFileName     = $refRecord['ref_string'];	// is usually uploads/RTE_magicC_123423324.png.png
			$fullSourceFileName = PATH_site . $refRecord['ref_string'];	// absolute path/filename
			$targetFileName     = $targetDirectory . basename($refRecord['ref_string']);
			$fullTargetFileName = $fullTargetDirectory . basename($refRecord['ref_string']);	// full directory
