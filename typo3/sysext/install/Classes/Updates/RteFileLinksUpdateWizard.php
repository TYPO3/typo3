...
class RteFileLinksUpdateWizard extends \TYPO3\CMS\Install\Updates\AbstractUpdate {
	 ...
	 * @return void
	 */
	protected function convertFileLinks($reference, $record) {
	 ...
	 */
	protected function fetchReferencedFile($path, $reference) {
		$fileObject = NULL;
	 ...
	 * @return void
	 */
	protected function saveChanges($modifiedText, $reference, $file) {
		...
		$GLOBALS['TYPO3_DB']->exec_UPDATEquery(
			...
			'uid = ' . $reference['recuid'],
			array(
				$reference['field'] => $GLOBALS['TYPO3_DB']->fullQuoteStr($modifiedText, $reference['tablename'])
			)
		);
