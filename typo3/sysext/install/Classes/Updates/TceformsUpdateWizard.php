			...
			foreach ($this->tables as $table => $fields) {
				...
				foreach (array_keys($fields) as $field) {
					if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('dbal')) {
						// this won't tackle the rare case of a single file without extension, but with DBAL we have no other detection possibilities
						$conditionParts[] = ' (' . $field . ' LIKE \'%,%\' OR ' . $field  . ' LIKE \'%.%\') ';
					} else {
						$conditionParts[] = '( NOT (' . $field . ' > 0 AND ' . $field  . ' < 2147483647) AND image <> \'\' )';
					}
				}
