...
class Tx_Install_Updates_LocalConfiguration extends Tx_Install_Updates_Base {
	...
	public function performUpdate(array &$dbQueries, &$customMessages) {
		...
		try {
			...
			foreach ($localConfigurationContent as $line) {
				...
				if (preg_match('/^\$TYPO3_CONF_VARS\[\'EXT\'\]\[\'extList\'\] *={1} *\'(.+)\';{1}/', $line, $matches) === 1) {
					...
					if (isset($typo_db_host)) {
						$typoDatabaseVariables['host'] = $typo_db_host;
					} elseif (isset($typo_db)) {
						$typoDatabaseVariables['database'] = $typo_db;
					} elseif (isset($typo_db_username)) {
						$typoDatabaseVariables['username'] = $typo_db_username;
					} elseif (isset($typo_db_password)) {
						$typoDatabaseVariables['password'] = $typo_db_password;
					} elseif (isset($typo_db_extTableDef_script)) {
						$typoDatabaseVariables['extTablesDefinitionScript'] = $typo_db_extTableDef_script;
					}
					unset(
						$typo_db_host,
						$typo_db,
			...
			$TYPO3_CONF_VARS['DB'] = $typoDatabaseVariables;
