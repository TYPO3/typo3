...
class tx_indexedsearch_mysql {
	...
	public function getSearchString($searchWordArray) {
		...
		foreach ($searchWordArray as $searchWordData) {
			...
			switch ($searchType) {
				...
				case self::SENTENCE:
					$searchBoolean = TRUE;
						// FIXME The regexp does not work with '"xxx"yyy"', makes 'xxx"yyy', which will make problems later in the code
					$searchWord = preg_replace('/^"(.*)"$/', '$1', $searchWord);	// Remove existing quotes as they will be added later anyway...
				break;
			}
			...
			switch ($searchWordData['oper']) {
				case 'AND NOT':
					$booleanSearchString .= ' -' . $searchWord . $trailBoolean;
					$searchBoolean = TRUE;
				break;
				case 'OR':
					$booleanSearchString .= ' ' . $searchWord . $trailBoolean;
				...
				default:
					$booleanSearchString .= ' +' . $searchWord . $trailBoolean;
					$naturalSearchString .= ' ' . $searchWord . $trailNatural;
			}
		...
		if ($searchType == self::SENTENCE) {
				// FIXME See above for the bad regexp
	 ...
	 */
	public function execFinalQuery_fulltext($searchData, $freeIndexUid = -1) {
		...
		if (($hookObj = &$this->pObj->hookRequest('execFinalQuery_idList'))) {
			...
			$pageWhere = ' ISEC.page_id IN (' . implode(',', $id_list) . ')';
		} else {
