		...
		if ($this->pObj->sObj) {
			...
			if (is_array($tmpl->sections)) {
				...
				foreach ($tmpl->sections as $key => $val) {
					$out .= '<tr><td nowrap class="tsob-conditions"><input class="checkbox" type="checkbox" name="conditions[' . $key . ']" id="check' . $key . '" value="' . htmlspecialchars($val) . '"' . ($this->pObj->MOD_SETTINGS['tsbrowser_conditions'][$key] ? ' checked' : '') . ' />';
