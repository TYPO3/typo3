...
class DocumentationService {
	...
	public function getOfficialDocuments() {
		...
		if ($json) {
			...
			$absoluteCacheFilename = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName('typo3temp' . DIRECTORY_SEPARATOR . 'documents.json');
			\TYPO3\CMS\Core\Utility\GeneralUtility::writeFile($absoluteCacheFilename, $json);
		}
		return $documents;
	}
	...
	public function getLocalExtensions() {
		...
		foreach ($GLOBALS['TYPO3_LOADED_EXT'] as $extensionKey => $extensionData) {
			$absoluteExtensionPath = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName($extensionData['siteRelPath']);
	...
	public function fetchDocument($url, $key, $version = 'latest', $language = 'default') {
		...
		$absolutePathToZipFile = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName('typo3temp' . DIRECTORY_SEPARATOR . $packageName);
		...
		if (!$hasArchive) {
			...
			$http = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Core\\Http\HttpRequest', $packageUrl);
			$response = $http->send();
			if ($response->getStatus() == 200) {
				\TYPO3\CMS\Core\Utility\GeneralUtility::writeFile($absolutePathToZipFile, $response->getBody());
			}
		}

		if (is_file($absolutePathToZipFile)) {
			$absoluteDocumentPath = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName('typo3conf' . DIRECTORY_SEPARATOR . 'Documentation' . DIRECTORY_SEPARATOR);
			...
			foreach ($documents as $document) {
				if ($document['key'] === $key) {
					...
					$absoluteComposerFilename = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName('typo3conf' . DIRECTORY_SEPARATOR . 'Documentation' . DIRECTORY_SEPARATOR . $relativeComposerFilename);
					\TYPO3\CMS\Core\Utility\GeneralUtility::writeFile($absoluteComposerFilename, json_encode($composerData));
					break;
				}
			}
		}
	...
	protected function getAvailablePackages($url) {
		...
		$remote = \TYPO3\CMS\Core\Utility\GeneralUtility::getUrl($indexUrl);
		if ($remote) {
