		...
		foreach ($this->slots[$signalClassName][$signalName] as $slotInformation) {
			...
			if ($slotReturn) {
				if (!is_array($slotReturn)) {
					...
					throw new Exception\InvalidSlotReturnException('The slot method ' . get_class($object) . '->' . $slotInformation['method'] . '() returned more ('
						. count($slotReturn) . ') arguments, than it recieved (' . count($signalArguments) . ').', 1376683066);
				} else {
