...
class UpdateFromTerController extends \TYPO3\CMS\Extensionmanager\Controller\AbstractController {
	...
	public function updateExtensionListFromTerAction($forceUpdateCheck = FALSE) {
		...
		$lastUpdate = $repository->getLastUpdate();
		if(get_class($lastUpdate) != 'DateTime') {
			$lastUpdate = new \DateTime( "2001-01-01" );
		}
		if ($lastUpdate->getTimestamp() < $GLOBALS['EXEC_TIME'] - 24 * 60 * 60 || $forceUpdateCheck) {
			try {
