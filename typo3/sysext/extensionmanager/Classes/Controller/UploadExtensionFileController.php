...
class UploadExtensionFileController extends \TYPO3\CMS\Extensionmanager\Controller\AbstractController {
	...
	protected function getExtensionFromT3xFile($file, $overwrite = FALSE) {
		...
		if (!$overwrite && \TYPO3\CMS\Core\Extension\ExtensionManager::isLoaded($extensionData['extKey'])) {
			throw new \TYPO3\CMS\Extensionmanager\Exception\ExtensionManagerException('Extension is already loaded and overwriting is disabled.', 1342864310);
		}
	...
	protected function getExtensionFromZipFile($file, $fileName, $overwrite = FALSE) {
		...
		if (!$overwrite && \TYPO3\CMS\Core\Extension\ExtensionManager::isLoaded($extensionKey)) {
			throw new \TYPO3\CMS\Extensionmanager\Exception\ExtensionManagerException('Extension is already loaded and overwriting is disabled.', 1342864311);
		}
