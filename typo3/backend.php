...
';
	...
	protected function getLoggedInUserLabel() {
		...
		$title = $GLOBALS['BE_USER']->user['username'];
		...
		if ($GLOBALS['BE_USER']->user['ses_backuserid']) {
			$css .= ' su-user';
			$title = $GLOBALS['LANG']->getLL('switchtouser') . ': ' . $title;
			$label = $GLOBALS['LANG']->getLL('switchtousershort') . ' ' . $label;
		}

		return '<div id="username" class="' . $css . '">' . $link . $icon .
				'<span title="' . $title . '">' . htmlspecialchars($label) . '</span>' .
				($link ? '</a>' : '') . '</div>';
	}
