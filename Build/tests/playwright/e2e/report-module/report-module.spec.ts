...
test.describe('See reports sub modules', () => {
  let modules = ['Status Report', 'Record Statistics'];
...
async function checkCountOfRecordStatisticEntry(contentFrame: FrameLocator, entryName: string, expectedCount: number) {
  let value: number = parseInt(await contentFrame.locator('//td[contains(text(),"' + entryName + '")]/following-sibling::td[1]').innerText());
