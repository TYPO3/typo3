...
export class ContextMenuElement extends LitElement {
  ...
  private readonly show = (event: CustomEvent<ContextMenuEventDetail>): void => {
    ...
    if (detail.originalEvent !== null) {
      ...
      if (detail.originalEvent.pointerId === -1 && detail.originalEvent.target) {
        const rect = (detail.originalEvent.target as Element).getBoundingClientRect();
        y = rect.top + rect.height / 2;
        // @todo remove positioning offset in `this.updateContextMenuPosition()`
        //       and add rect.width/2 here
        x = rect.left;
      } else {
        y = detail.originalEvent.clientY;
        x = detail.originalEvent.clientX;
      }
      this.rootPositionY = y + offset.y;
      this.rootPositionX = x + offset.x;
    }
