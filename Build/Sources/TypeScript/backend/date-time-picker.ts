...
class DateTimePicker {
  ...
  private initializeField(inputElement: HTMLInputElement, locale: flatpickr.Options.LocaleKey): void {
    ...
    if (legacyHiddenField) {
      inputElement.value = legacyHiddenField.value;
      console.warn('The hidden field for "@typo3/backend/date-time-picker.js" shall not be used. Please remove this field and supply the ISO8601 date in the user-facing field instead.');
    }
  ...
  private getDateOptions(inputElement: HTMLInputElement): flatpickr.Options.Options {
    ...
    const options: flatpickr.Options.Options = {
      ...
      onChange: (dates: Date[], currentDateString: string, self: flatpickr.Instance): void => {
        ...
        if (legacyHiddenField) {
          legacyHiddenField.value = self.input.value;
        }
      },
      maxDate: '',
      minDate: '',
