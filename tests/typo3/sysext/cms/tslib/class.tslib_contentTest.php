	...
	public function cropHtmlDataProvider() {
		...
		foreach ($charsets as $charset) {
			$data = array_merge($data, array(
				...
				$charset . ' text with dash in html-element 50|...|1' => array(
					'50|...|1', 'Some text with an invalid attribute in link to <link email.address@example.org -="wrong" mail "Open email window">my email.address@example.org</link> and text after it', 'Some text with an invalid attribute in link to <link email.address@example.org -="wrong" mail "Open email window">my...</link>', $charset
				),
			));
		}
		return $data;
