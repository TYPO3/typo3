...
class t3lib_divTest extends tx_phpunit_testcase {
	...
	public function fixPermissionsCorrectlySetsGroup() {
		if (TYPO3_OS == 'WIN') {
			$this->markTestSkipped('fixPermissionsCorrectlySetsGroup() tests not available on Windows');
		}
		if (!function_exists('posix_getegid')) {
			$this->markTestSkipped('Function posix_getegid() not available, fixPermissionsCorrectlySetsGroup() tests skipped');
		}
		if (posix_getegid() === -1) {
