...
class t3lib_tceforms_flexformsTest extends tx_phpunit_testcase {
	 ...
	 * @return mixed
	 */
	public function callInaccessibleMethod($className, $name, $sheetConf, $fieldName) {
		$class = new \ReflectionClass($className);
		$object = new $className;
		$method = $class->getMethod($name);
		$method->setAccessible(TRUE);
		return $method->invoke($object, $sheetConf, $fieldName);
	}
	...
	public function sheetDataProvider() {
		return array(
			'Empty sheetConf' => array(array(), 'field', array()),
			'Empty fieldName' => array(array(), '', array()),
			'Get configuration on fieldName without dot' => array(
				array('propertyA' => array(
						'keyA' => array(
							'valueA' => 1,
						),
						'keyB' => 2,
			...
			'Get configuration on fieldName with dot' => array(
				array('propertyA' => array(
					...
					'firstsetting' => array('firstsetting' => 3),
				), 'settings.firstsetting', array('firstsetting' => 3)
			),
		);
	}
	...
	public function getFlexFormFieldConfigurationBySheetConfAndFieldName($sheetConf, $fieldName, $expected) {
		$fieldConf = $this->callInaccessibleMethod('t3lib_TCEforms_Flexforms', 'getFlexFormFieldConfiguration', $sheetConf, $fieldName);
